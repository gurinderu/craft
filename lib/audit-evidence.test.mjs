...
import { EVIDENCE_MARKER, EVIDENCE_LINE, GREEN_VERDICT, hasEvidence, demoteUnsupportedGreen } from './audit-evidence.mjs'
...
test('hasEvidence is true when a LINE begins with the marker and carries content after it', () => {
  assert.equal(hasEvidence('Evidence: ran cargo-audit, cargo-deny; both clean'), true)
  // A marker beginning ANY line (after optional whitespace), with content after it, counts —
  // mirroring the rubric "emit one line beginning `Evidence:`".
  assert.equal(hasEvidence('## Verdict\nApprove\nEvidence: read src/db.rs'), true)
  assert.equal(hasEvidence('cargo-audit clean.\n  Evidence: ran cargo-audit over 214 crates'), true)
})

test('hasEvidence rejects the marker buried inside a line of prose (line-anchored, not a substring)', () => {
  // The gate reads a LINE that begins with the marker, not any occurrence: a finding that quotes the
  // marker in prose ("no `Evidence:` of bounds") names nothing this pass actually did, and must not
  // wave a no-work green through. This is the RED before the line-anchoring fix (indexOf would pass).
  assert.equal(hasEvidence('cargo-audit found no `Evidence:` of bounds checking'), false)
  assert.equal(hasEvidence('The rubric asks agents to write an Evidence: line before the verdict.'), false)
})

test('hasEvidence tolerates ordinary markdown decoration on the Evidence line (the VERDICT_LINE decor class)', () => {
  // MAYATNIK correction (node #38): round 2 line-anchored the scan to the first NON-WHITESPACE column,
  // which then rejected the markdown emphasis models routinely put on a labelled line — `**Evidence:**`,
  // `- Evidence:`, `> Evidence:`, `` `Evidence:` `` — so an honest green that showed its work was demoted
  // to INCOMPLETE. The fix reuses the SAME cosmetic-decoration class the sibling scanners already strip
  // (VERDICT_LINE / INCOMPLETE_LINE: `[ \t>*_`#-]`), not a third strictness: the marker is tested at the
  // first NON-decoration column. RED before that change — the strict line-anchored scan returns false.
  assert.equal(hasEvidence('**Evidence:** ran cargo test'), true)
  assert.equal(hasEvidence('- Evidence: ran cargo-audit, cargo-deny'), true)
  ...
  // A decorated marker with NO real content after it is still empty — the gate is not weakened: the
  // trailing decoration is cosmetic, not work named.
  assert.equal(hasEvidence('**Evidence:**'), false)
  assert.equal(hasEvidence('- Evidence:   '), false)
  // And the buried-marker protection stands: a marker after real prose (decorated or not) is rejected.
  assert.equal(hasEvidence('*note* Evidence: buried after a word'), false)
  assert.equal(hasEvidence('cargo-audit found no `Evidence:` of bounds checking'), false)
})

test('hasEvidence tolerates case and a stray space before the colon (the case/whitespace axis)', () => {
  // The mirror of the decoration tolerance (node #38) on the axis models ALSO vary: an all-caps or
  // lower-case label, and a stray space before the colon, are the SAME marker and the same realistic
  // model variance — they must not demote an honest, work-showing green. RED before the case- and
  // whitespace-insensitive fix: an exact-case `startsWith(EVIDENCE_MARKER, i)` rejects every line here.
  assert.equal(hasEvidence('EVIDENCE: ran cargo test'), true)
  assert.equal(hasEvidence('evidence: ran cargo test'), true)
  assert.equal(hasEvidence('Evidence : ran cargo test'), true)
  assert.equal(hasEvidence('**EVIDENCE:** ran cargo test'), true)      // case AND decoration together
  assert.equal(hasEvidence('- evidence : ran cargo-audit'), true)
  // The gate is NOT weakened on the other axis: a marker buried after real prose and a decorated-but-
  // empty marker stay false regardless of case.
...
test('an EMPTY evidence field falls back to the summary (|| not ??)', () => {
  // FINDINGS_SCHEMA marks `evidence` required, so an agent that put its Evidence line in the report
  // BODY (as the rust-security-scanner / rust-miri rubrics instruct — "emit one line beginning
  // `Evidence:`", not "fill the field") leaves the STRUCTURED field present-but-empty (''). `??` does
  // not fall back on '', so the summary — where the agent's line may live — was never consulted and an
  // honest, work-showing green was demoted. This is the RED before the `||` fix.
  const kept = demoteUnsupportedGreen({ verdict: 'Approve', evidence: '', summary: 'Evidence: ran cargo-audit; read src/db.rs' })
  assert.equal(kept.verdict, 'Approve', "an empty evidence field falls back to the summary's Evidence line")
  // The control: an empty field AND a summary with no marker still demotes — '' is the value the
  // fallback exists to skip past, not a reason to trust nothing.
  const demoted = demoteUnsupportedGreen({ verdict: 'Approve', evidence: '', summary: 'looks fine' })
  assert.match(demoted.verdict, /INCOMPLETE \(no evidence/, 'no marker anywhere still demotes')
})
...
// hasEvidence no longer references EVIDENCE_MARKER directly — it anchors with EVIDENCE_LINE, which is
// case- and whitespace-tolerant — so nothing structural ties the reader to the canon any more. This
// closes that gap: whatever the reader accepts, it must accept the exact string writers are told to
// emit. Reword EVIDENCE_MARKER without teaching the regex and this fails first, before writer and
// reader can diverge about the canon.
test('the reader regex accepts the canonical writer marker (reader-tolerant-of-writer stitch)', () => {
