...
import { shardLedger, assembleLedgerShards, LEDGER_SHARD_MAX_BYTES, LEDGER_SHARD_MAX_SHARDS, LEDGER_SHARD_PHASE } from './ledger-shards.mjs'
...
test('REQUIREMENT: the volume crossing a model in one step is bounded — asserted by threshold, not by an exact number', () => {
  ...
  const whole = JSON.stringify(bigLedger).length
  const worst = Math.max(...shards.map(s => JSON.stringify(s).length))
