...
import { shardLedger, assembleLedgerShards, pruneTombstones, tombstoneRound, tombstoneBudget, LEDGER_SHARD_MAX_BYTES, LEDGER_SHARD_MAX_SHARDS, LEDGER_SHARD_PHASE, LEDGER_COPY_REFUSAL_ENTRIES, LEDGER_COPY_SAFETY_ROWS, LEDGER_TOMBSTONE_MAX } from './ledger-shards.mjs'
...
test('LEDGER_TOMBSTONE_MAX is DERIVED from the copy-refusal border, not the shard budget', () => {
  // The single-call copy of the final record refuses on the WHOLE ledger (~170 entries), live rows
  // AND tombstones together — so the tombstone ceiling is taken from THAT border, minus a live-row
  // margin, not from the far looser shard budget (which never bites first and let the memory cross the
  // real border on tombstones-plus-live).
  assert.equal(LEDGER_TOMBSTONE_MAX, LEDGER_COPY_REFUSAL_ENTRIES - LEDGER_COPY_SAFETY_ROWS,
    'the ceiling is the copy-refusal border less the live-row margin, so it moves with the border it protects')
  assert.ok(LEDGER_TOMBSTONE_MAX < LEDGER_COPY_REFUSAL_ENTRIES, 'and it sits under the refusal point with margin for live rows')
})

test('REQUIREMENT: tombstoneBudget bounds the COMBINED ledger under the copy-refusal border, not tombstones alone', () => {
  // The finding this fixes: with ~150 tombstones and ~30 live rows, a tombstones-only cap of 150 let
  // the combined ledger reach 180 — past the ~170-entry single-call copy the final logger refuses,
  // which inverts the memory into a per-round full rescan. The budget must shed tombstones for the live
  // rows so tombstones + live stay under the border.
  const live = 30
  const tombstones = Array.from({ length: 150 }, (_, i) => ({ fp: `fp${i}`, why: `resolved in round ${i + 1}` }))
  const kept = pruneTombstones(tombstones, { max: tombstoneBudget(live) })
  assert.ok(kept.length + live <= LEDGER_TOMBSTONE_MAX, 'tombstones + live stay within the combined ceiling')
  assert.ok(kept.length + live < LEDGER_COPY_REFUSAL_ENTRIES, 'and therefore under the single-call copy-refusal border, with margin')
})

test('tombstoneBudget yields 0 when live findings already fill the ceiling — tombstones cannot deepen an overflow', () => {
  assert.equal(tombstoneBudget(LEDGER_TOMBSTONE_MAX + 5), 0)
  assert.equal(tombstoneBudget(LEDGER_TOMBSTONE_MAX), 0)
  assert.equal(tombstoneBudget(0), LEDGER_TOMBSTONE_MAX, 'with no live rows the whole ceiling is available to tombstones')
})
