...
import { absorbAcross, ATTACK_MAX, sanitizeAttack, baseWhy, isHighSeverity, classifyRedTeam, adjudicateOne, shouldRedTeam, carriedKey, alreadyCarried, findCarrier, absorbInto, ABSORBED_MAX, splitAbsorbed, withoutAbsorbed, absorbedPromptBlock, partitionAbsorbed, clampField, ABSORB_FILE_MAX, ABSORB_TITLE_MAX, markTrackedUnverified, TRACKED_MARK } from './review-adjudicate.mjs'
...
// `fingerprint` plus the two round-local sentinels — the only free identifiers in its body — to
// exercise the real code, not a copy. TRACKED_MARK is passed REAL (imported above) and THINNED_CLAUSE
// is read out of the engine source, because the entry's whole job on that field is to strip both: a
// stubbed sentinel would let either strip be deleted with this test green.
function loadToLedgerEntry() {
  ...
  const t = src.match(/^const THINNED_CLAUSE = (\/.*\/g)$/m)
  assert.ok(t, 'THINNED_CLAUSE literal found in workflows/review.js')
  return new Function('fingerprint', 'TRACKED_MARK', 'THINNED_CLAUSE', `return ${m[0].replace(/^const toLedgerEntry = /, '')}`)(() => 'fp0', TRACKED_MARK, new Function(`return ${t[1]}`)())
}

test('toLedgerEntry strips the tracking mark — a statement about THIS round may not persist', () => {
  const toLedgerEntry = loadToLedgerEntry()
  const e = toLedgerEntry({ severity: 'High', why: `the rationale${TRACKED_MARK}` }, 'open', 'unverified')
  assert.equal(e.why, 'the rationale', 'the mark is gone and the rationale is intact')
})
...
test('tierFromVotes is identical for one batched verdict and one individual vote', () => {
  ...
  // Dead verification: NEVER DROPS, and never asserts an inspection either. This line used to
  // expect `suspected`, which pinned the defect it was guarding against: `suspected` is defined in
  // the report as "a verifier looked and the claim did not stand up", so handing it to a finding
  // nothing looked at made a dead panel demote a Critical to a non-gating finding. Both halves of
  // the old assertion stay assertable — the finding survives, and its tier now says what is true.
  const dead = tierFromVotes({ ...f, severity: 'Critical', file: 'a.rs', line: 1 }, [])
  assert.equal(dead.tier, 'unverified', 'dead verification does not assert an inspection')
  assert.equal(dead.title, 't', 'never drops: the finding itself survives')
  assert.equal(dead.severity, 'Critical', 'and keeps its severity — a death is not a demotion')
  assert.match(dead.why, /NOT VERIFIED/, 'and says in its own words that nothing checked it')
})
...
// Falsifier for the branch's defect #1: after a stall the operator re-runs on the SAME commit, so
// priorRound.head === current HEAD and a delta scan (head...HEAD) would be empty. journalSourced
// must force a full re-scan on its own, even when the ledger itself is healthy (degraded: false).
test('shouldFullRescan: a journal-sourced prior round forces a full re-scan regardless of cadence, independent of degraded', () => {
  assert.equal(shouldFullRescan({ priorRound: { round: 1 }, thisRound: 2, fullEvery: 0, degraded: false, journalSourced: true }), true, 'journalSourced overrides fullEvery=0 on its own — not riding on degraded')
  assert.equal(shouldFullRescan({ priorRound: { round: 1 }, thisRound: 3, fullEvery: 3, degraded: false, journalSourced: true }), true)
  assert.equal(shouldFullRescan({ priorRound: { round: 1 }, thisRound: 2, fullEvery: 0, degraded: false, journalSourced: false }), false, 'control: without journalSourced, same inputs stay incremental')
})
...
test('re-review wiring: lensBase widens to baseRef on a full re-scan, prior head on incremental', () => {
  ...
  assert.ok(src.includes('shouldFullRescan({ priorRound, thisRound, fullEvery, degraded: priorLedgerDegraded, journalSourced: priorRoundJournalSourced })'), 'fullRescan is computed from the guards, including the journal-sourced one')
  assert.ok(src.includes('Re-review DEGRADED'), 'a degraded prior ledger is logged loudly')
})

test('re-review wiring: new findings matching a still-live prior are absorbed, resolved priors excepted', () => {
  assert.ok(src.includes('const livePriors = [...adjudicated.stillOpen, ...adjudicated.regressed, ...adjudicated.carried, ...adjudicated.retired]'), 'dedup set excludes resolved priors and includes retired ones')
  // TWO tracks, not three. Routing the unverified track through absorption put a finding NOTHING
  // checked onto a prior's `why`, where absorbedPromptBlock tells the next adjudicator that
  // "resolved" requires it to be gone — so unchecked gated the re-review verdict. It is marked in
  // place instead; the executed guard is in lib/review-unverified.test.mjs.
  assert.ok(src.includes('absorbAcross([confirmed, suspected], livePriors, retired, matchesPrior)'), 'the JUDGED tracks go through ONE threaded partition (tested by execution below), which decides absorb-or-keep')
  // The hosts are `trackingHosts`, not `livePriors`: a prior carrying the unverified tier is never
  // adjudicated, so it is absent from `livePriors` while still being a live ledger row — and it is
  // the host that matters for a site that stays unchecked round after round. The executed guards are
  // in lib/review-unverified.test.mjs.
  assert.ok(src.includes('markTrackedUnverified(unverified.filter(f => !f.carriedUnverified), trackingHosts, retired, matchesPrior)'), 'and the unverified track is marked in place, never absorbed into a host it would then hold open')
  assert.ok(src.includes('for (const [host, why] of updates) host.why = why'), 'the cumulative host-why updates are applied exactly once')
  assert.ok(src.includes('if (keptAtRetired) log('), 'findings kept at a retired prior are counted and logged separately from absorptions')
})

test('re-review wiring: the carried track has an exit and the exit is not persisted', () => {
  assert.ok(src.includes('carried: [], retired: [] }'), 'the adjudicate tracks carry a retired bucket')
  assert.ok(src.includes('else adjudicated.retired.push(f)'), 'an unchanged carry-check retires the dismissed prior')
  assert.ok(src.includes("log(`⚠️ carry-check for ${f.file}:${f.line} died — kept as carried by default`); adjudicated.carried.push(f)"), 'an indeterminate carry-check retires nothing')
  assert.ok(!/\.\.\.adjudicated\.retired\.map\(f => toLedgerEntry/.test(src), 'retired priors are not re-persisted')
  // Retired priors must still reach the report — but NOT folded into `carried`, whose section says
  // "re-checked again next round". Folding them made the report assert that of a prior that leaves
  // the ledger, and on a full rescan the same defect could sit under both that heading and New.
  assert.ok(src.includes('retired: adjudicated.retired'), 'retired priors reach the report as their own key')
  assert.ok(!src.includes('carried: [...adjudicated.carried, ...adjudicated.retired]'), 'retired priors are NOT folded into the carried track')
  assert.ok(src.includes('## 🏁 Retired'), 'the report gives retired priors their own section')
  assert.ok(src.includes('they leave the ledger and are NOT re-checked again'), 'that section says what retirement means, rather than calling it "carried forward unchanged"')
})

test('pr-comments wiring: the outcome is captured and logged, never discarded', () => {
  assert.ok(src.includes('const posted = await ragent('), 'the poster return value is bound')
  assert.ok(src.includes('schema: PR_COMMENTS_SCHEMA'), 'the outcome comes back structured')
  assert.ok(src.includes('if (posted == null) log('), 'a dead poster is logged rather than read as success')
  assert.ok(/log\(`⚠️ PR comments: nothing was posted/.test(src), 'posting nothing is reported')
})
...
// A dead preflight used to render as an empty string — the downstream prompt then looked exactly
// like one whose preflight had nothing to report, and "we could not establish it" silently became
// the permissive reading. The absence must be stated.
test('preflightBrief says the step is UNAVAILABLE when preflight died, never renders blank', () => {
  const { preflightBrief } = loadPreflightBrief()
  for (const dead of [null, undefined]) {
    const out = preflightBrief(dead)
    assert.match(out, /PREFLIGHT UNAVAILABLE/, 'a dead preflight is named, not omitted')
    assert.match(out, /provenance/i, 'and it must reach the record, not just the prompt')
  }
})
...
test('preflight probes for a tool both inside and outside the dev shell', () => {
  const m = src.match(/function preflightPrompt[\s\S]*?\n\}/)
  const p = m[0]
  assert.match(p, /run the same loop once more under it/i, 'the second, dev-shell pass is still asked for')
  assert.match(p, /PRESENT if EITHER pass found it/i)
...
// The deadline is squeezed from both sides and has been wrong in both directions. Too high (7min
// behind a 3min ceiling) and the budget is advisory — nothing stops a pass that ignores it. Too low
// and it cuts below real work: `ragent` does not cancel on a deadline, it abandons the wait and
// re-dispatches, so 210000ms against a pass measured at 207s (with the clock running from DISPATCH,
// covering queue wait too) turned a slow-but-working preflight into two timeouts and no preflight.
test('the preflight dispatch deadline clears the measured cost and stays under twice the declared ceiling', () => {
  const m = src.match(/label: `preflight:\$\{profile\.id\}`[^)]*deadlineMs: (\d+)/)
  assert.ok(m, 'preflight dispatch carries an explicit deadlineMs')
  const ms = Number(m[1])
  assert.ok(ms >= 240000, `preflight deadline ${ms}ms leaves no margin over the 207s measured cost — a miss costs two dispatches`)
  assert.ok(ms < 360000, `preflight deadline ${ms}ms is twice the prompt's declared 3min ceiling or more, which makes the budget advisory`)
})

// A preflight that dies or times out must be legible as such — everywhere a reader looks. This is
// the recurring shape: "could not establish" defaulting to the reading a clean run would produce.
test('a dead or partial preflight is reported, never rendered as a clean one', () => {
  assert.match(src, /Preflight unavailable \(failed or passed its deadline\)/, 'the log says it out loud')
  assert.match(src, /status: 'unavailable'/, 'the checkpoint distinguishes it from a preflight that found nothing')
  assert.match(src, /PARTIAL: <field> not established/, 'the prompt is told how to mark an unfinished field')
  assert.match(src, /PARTIAL — see notes/, 'and a partial result is flagged in the run log')
})

// Partial-ness must not depend on the SHAPE of prose. The detector was anchored (`/^\s*PARTIAL\b/`),
// so one clause of preamble before the marker produced status 'ok' over unfinished fields — a
// partial rendered as clean. The schema now carries an explicit boolean, and the note is only a
// fallback for a model that writes the prose but forgets the flag.
test('a partial preflight is detected from a schema field, not from where PARTIAL sits in the notes', () => {
  assert.match(src, /partial: \{ type: 'boolean'/, 'the schema declares the flag')
  assert.match(src, /required: \['runner', 'blockers', 'missingTools', 'ciCovers', 'probes', 'partial', 'notes'\]/,
    'and requires it — an optional flag a model omits is the permissive default again')
  assert.doesNotMatch(src, /\^\\s\*PARTIAL/, 'no anchored PARTIAL test survives in the detection path')
  const m = src.match(/function preflightIsPartial[\s\S]*?\n\}/)
  assert.ok(m, 'preflightIsPartial is the single detector')
  assert.match(m[0], /pf\.partial === true/, 'the flag decides')
  assert.match(m[0], /\\bPARTIAL:/, 'and an unanchored note test is the fallback')
  // The same PR routes CI CHECK NAMES into `notes`, so the fallback must not fire on a name: a green
  // check called `partial-build` matched the old case-insensitive bare-word test and flipped a
  // complete preflight to 'partial'. Only the marker the prompt mandates counts.
  assert.doesNotMatch(m[0], /PARTIAL\\b\/i|\\bPARTIAL\\b\//, 'the fallback is not a case-insensitive bare word')
  const detector = new Function('flattenField', `${m[0]}; return preflightIsPartial`)(x => String(x ?? ''))
  assert.equal(detector({ partial: false, notes: 'green: partial-build, Partial checks' }), false,
    'a CI check name in notes does not flip a complete preflight to partial')
  assert.equal(detector({ partial: false, notes: 'Ran out of time. PARTIAL: ciCovers not established (gh unavailable)' }), true,
    'the mandated marker still trips the fallback wherever it sits in the note')
  // Both readers go through it, or one of them still reads a partial as clean.
  assert.ok((src.match(/preflightIsPartial\(preflight\)/g) || []).length >= 2,
    'both the run log and the checkpoint status use the shared detector')
})

// An unopened green check may never claim coverage: `ciCovers` is consumed downstream as
// "do NOT re-run this locally", so a name nobody verified silently skips a real gate signal.
test('the hard cap on reading workflows keeps unopened checks out of ciCovers', () => {
  const m = src.match(/function preflightPrompt[\s\S]*?\n\}/)
  const p = m[0]
  assert.match(p, /HARD CAP/, 'the cap itself stays')
  assert.match(p, /NEVER in \\`ciCovers\\`/, 'and past the cap the checks are excluded from ciCovers by name')
  assert.match(p, /Only a signal you read the workflow for goes in \\`ciCovers\\`/)
})

// The cap makes `ciCovers` deliberately INCOMPLETE — at most two signals — so the gate may not read
// a non-empty list as "the detection is done". It said exactly that ("that list IS the detection"),
// which on a repo with five green checks skipped detection for the three checks the cap left in
// `notes`. Costly, never falsely clean, but the sentence was false.
test('the gate reads ciCovers as the verified subset and is told what to do with checks named only in notes', () => {
  const g = src.match(/function rustGate[\s\S]*?\n\}/)[0]
  assert.doesNotMatch(g, /that list IS the detection/, 'the false sentence is gone')
  assert.match(g, /VERIFIED SUBSET/, 'ciCovers is described as the subset it is')
  assert.match(g, /named in \\`notes\\` but NOT in ciCovers/, 'and the gate is told to act on the leftovers')
  assert.match(g, /A bare check name is never coverage/, 'a name alone may not be trusted as coverage')
})

// The step's lead sentence used to say ONE call while the block below issued two; a model reading
// the lead drops one, and the one it drops is the commit-status call a follow-up deliberately added.
test('the CI-coverage step asks for both SHA-scoped calls in its lead', () => {
  const m = src.match(/function preflightPrompt[\s\S]*?\n\}/)
  const p = m[0]
  const lead = p.match(/4\. CI COVERAGE\.[^\n]*/)[0]
  assert.match(lead, /TWO calls/, 'the lead states the count the code block actually issues')
  assert.doesNotMatch(lead, /ONE call/)
  assert.match(p, /Both calls, once each/, 'and the block still says so too')
})

// The deadline does not kill the agent — it abandons the wait and re-dispatches. A prompt claiming
// it is cut off teaches the model the wrong cost model for overrunning.
test('the preflight budget describes what the deadline actually does', () => {
  const m = src.match(/function preflightPrompt[\s\S]*?\n\}/)
  const p = m[0]
  assert.doesNotMatch(p, /the dispatch is cut off/, 'nothing cuts the agent off')
  assert.match(p, /STOPS WAITING/, 'the caller abandons the wait')
  assert.match(p, /dispatches a second preflight/, 'and pays for a second pass')
})
...
test('the deadline sentinel cannot be confused with a real agent result', () => {
  ...
  // ragent must strip its OWN options — deadlineMs and the re-dispatch breaker — before handing opts
  // to agent(), or the harness sees unknown options (and `breaker` would be a function-bearing
  // object crossing the harness boundary).
  assert.match(src, /const \{ deadlineMs: _deadlineMs, breaker, \.\.\.agentOpts \} = opts/)
})

// ---- bounded verification dispatch ----
test('batched and individual verification share one ordered list, dispatched through a bounded window', () => {
  ...
  // The merged list is dispatched through a sliding window bounded by AGENTS: the per-agent deadline
  // runs from DISPATCH, so an unbounded fan-out makes it fire on queue wait rather than on hanging.
  assert.match(src, /const settledVerdicts = await weightedWindow\(entries, VERIFY_WINDOW_AGENTS, run => parallel\(\[run\]\)\.then\(rs => rs\[0\]\)\)/)
  assert.doesNotMatch(src, /await parallel\(batchThunks\.concat\(individualThunks\)\)/, 'the unbounded single fan-out is gone')
  // A barrier per wave is what this replaced: each wave waited for its slowest member before the
  // next was dispatched, against verification agents measured up to 811s.
  assert.doesNotMatch(src, /for \(const wave of waves\)/, 'the per-wave barrier is gone')
  ...
  // INDIVIDUALS LEAD the list, and the split back out must use THEIR count — the ordering is what
  // lets a batch ask the verdict floor about a Critical that has already come back confirmed
  // (lib/verify-economy.mjs), and the slice boundary must move with it or verdicts get attributed to
  // the wrong findings.
  assert.match(src, /const entries = route\.individual\.map\(\(f, i\) => \(\{ run: withFloor\(individualThunks\[i\]\)/)
  assert.match(src, /settledVerdicts\.slice\(0, route\.individual\.length\)/)
  assert.match(src, /settledVerdicts\.slice\(route\.individual\.length\)/)
})

test('the merged verification list attributes every verdict to the finding it belongs to', () => {
  ...
  // Individuals lead, as they do in the engine — the ordering is what lets a batch consult the
  // verdict floor, and a simulation that mirrored the old order would teach the wrong boundary.
  const settled = individualThunks.concat(batchThunks).map(t => t())
  const judged = settled.slice(0, individualThunks.length)
  const batched = settled.slice(individualThunks.length).filter(Boolean).flat()
  ...
  // A dead batch agent resolves to null and must not shift the batch slice or reach the individual one.
  const withDead = individualThunks.map(t => t()).concat([null], batchThunks.slice(1).map(t => t()))
  assert.deepEqual(withDead.slice(0, individualThunks.length).map(f => f.id), ['i1', 'i2', 'i3'])
  assert.deepEqual(withDead.slice(individualThunks.length).filter(Boolean).flat().map(f => f.id), ['b3'], 'a null batch keeps its slot')
})
...
// ---- absorption: the coarse key drops nothing on the floor ----
// The failure this guards: B is absorbed into prior A in round N and recorded nowhere; in round N+1
// A adjudicates `resolved` and leaves the ledger (resolved is deliberately not re-persisted); B is
// then unrecorded AND untracked, and on an incremental round its site is out of the lens base.

test('findCarrier returns the prior that absorbs a finding, so the caller can record it there', () => {
  const a = { file: 'src/a.rs', ruleId: 'RUST-001', title: 'lock held across await', why: 'the guard is alive at the await' }
  assert.equal(findCarrier({ file: 'src/a.rs', ruleId: 'RUST-001' }, [a], matchesPrior), a)
  assert.equal(findCarrier({ file: 'src/z.rs', ruleId: 'RUST-001' }, [a], matchesPrior), null)
  assert.equal(findCarrier({ file: 'src/a.rs', ruleId: 'RUST-001' }, [], matchesPrior), null)
})

test('an absorbed finding lands on its host: file, line and title survive in the persisted why', () => {
  const host = { why: 'the guard is alive at the await' }
  host.why = absorbInto(host.why, { file: 'src/a.rs', line: 42, title: 'second defect: the retry loop never exits' })
  assert.match(host.why, /src\/a\.rs:42/)
  assert.match(host.why, /the retry loop never exits/)
})

test('the absorbed clause survives the next round — baseWhy strips the marker, not the clause', () => {
  // The whole point: the host is re-appended a per-round marker, and next round strips it. If the
  // clause were appended after the marker, that strip would take the absorbed finding with it.
  let why = absorbInto('original rationale', { file: 'src/a.rs', line: 7, title: 'a distinct defect' })
  why = `${baseWhy(why)} — fix incomplete: the fix only closes the literal repro`
  const nextRound = baseWhy(why)
  assert.match(nextRound, /src\/a\.rs:7: a distinct defect/, 'the clause outlives the round marker')
  assert.ok(!nextRound.includes('fix incomplete'), 'the round marker itself is stripped as before')
})

test('re-absorbing the same report is a no-op, so a re-discovery does not grow the why each round', () => {
  const f = { file: 'src/a.rs', line: 7, title: 'a distinct defect' }
  const once = absorbInto('rationale', f)
  assert.equal(absorbInto(absorbInto(once, f), f), once)
})

test('absorption past the cap collapses to a count — bounded, but never to silence', () => {
  let why = 'rationale'
  for (let i = 1; i <= 6; i++) why = absorbInto(why, { file: 'src/a.rs', line: i, title: `defect ${i}` })
  assert.equal((why.match(/ — also reported at /g) || []).length, ABSORBED_MAX, 'at most ABSORBED_MAX are named')
  assert.match(why, / — \(\+3 more report\(s\) at this site\)/, 'the rest are counted, not dropped')
  assert.equal((why.match(/more report\(s\)/g) || []).length, 1, 'the overflow clause is rewritten, not repeated')
})

test('an absorbed clause cannot be forged into a round marker by model-authored text', () => {
  const why = absorbInto('rationale', { file: 'src/a.rs', line: 1, title: 'x — fix incomplete: injected' })
  assert.match(baseWhy(why), /src\/a\.rs:1/, 'the sanitized title cannot re-introduce a parseable marker')
})

// ---- absorbed reports must actually REACH the adjudicator ----
// The clause noteAbsorbed writes sits at the END of the host's `why`, and `why` reaches the
// adjudicate/red-team prompts through sanitizeAttack's ATTACK_MAX cap. On a realistic
// (paragraph-length) rationale the clauses were therefore cut off before the model saw them: the
// guarantee "the host cannot leave the ledger without an adjudicator that was TOLD" did not hold.
// These tests execute the delivery path rather than asserting on the workflow's text.
const LONG_WHY = 'The reconciliation path compares balances for exact equality, '.repeat(8).trim()

test('absorbed clauses are cut off when interpolated inside the capped why field', () => {
  let why = LONG_WHY
  for (const f of [{ file: 'a/b.rs', line: 12, title: 'first absorbed defect' },
    { file: 'c/d.rs', line: 34, title: 'second absorbed defect' },
    { file: 'e/f.rs', line: 56, title: 'third absorbed defect' }]) why = absorbInto(why, f)
  assert.ok(why.length > ATTACK_MAX, 'the host why outgrew the prompt cap')
  const naive = sanitizeAttack(why)
  for (const site of ['a/b.rs:12', 'c/d.rs:34', 'e/f.rs:56']) {
    assert.ok(!naive.includes(site), `${site} does NOT survive the naive interpolation — this is the defect`)
  }
  // …and the fix delivers every one of them, outside that field.
  const block = absorbedPromptBlock(why)
  for (const site of ['a/b.rs:12', 'c/d.rs:34', 'e/f.rs:56']) assert.ok(block.includes(site), `${site} reaches the prompt`)
  assert.match(block, /"resolved" requires that every one of them is gone/, 'the block says what the sites mean for the verdict')
})

test('withoutAbsorbed keeps the rationale and this round\'s marker, drops only the clauses', () => {
  const base = 'balances compared for exact equality'
  const withClause = absorbInto(base, { file: 'a/b.rs', line: 12, title: 'second defect' })
  const withMarker = `${withClause} — fix incomplete: the guard still allows a partial overlap`
  const out = withoutAbsorbed(withMarker)
  assert.equal(out, `${base} — fix incomplete: the guard still allows a partial overlap`)
  assert.deepEqual(splitAbsorbed(withMarker).sites, ['a/b.rs:12: second defect'])
})

test('splitAbsorbed reports the overflow count and absorbedPromptBlock renders it', () => {
  let why = 'rationale'
  for (let i = 0; i < ABSORBED_MAX + 2; i++) why = absorbInto(why, { file: `f${i}.rs`, line: i, title: `t${i}` })
  const { sites, more } = splitAbsorbed(why)
  assert.equal(sites.length, ABSORBED_MAX)
  assert.equal(more, 2)
  assert.match(absorbedPromptBlock(why), /\(\+2 further report\(s\) at this site/)
})

test('absorbedPromptBlock is empty when nothing was absorbed', () => {
  assert.equal(absorbedPromptBlock('plain rationale — fix incomplete: attack'), '')
  assert.equal(absorbedPromptBlock(undefined), '')
})

// ---- a retired host absorbs nothing ----
test('partitionAbsorbed keeps a finding whose only carrier retired this round', () => {
  const retiredHost = { file: 'a/b.rs', ruleId: 'r1', why: 'dismissed rationale' }
  const livePriors = [retiredHost]
  const fresh = { file: 'a/b.rs', ruleId: 'r1', line: 90, title: 'a genuinely new defect elsewhere in the file' }
  const { kept, absorbed, keptAtRetired, updates } = partitionAbsorbed([fresh], livePriors, new Set([retiredHost]), matchesPrior)
  assert.deepEqual(kept, [fresh], 'the finding survives rather than vanishing into a host that leaves the ledger')
  assert.equal(absorbed, 0)
  assert.equal(keptAtRetired, 1, 'the loss-shaped case is counted on its own')
  assert.equal(updates.size, 0, 'nothing is written onto a retired host')
  assert.equal(retiredHost.why, 'dismissed rationale', 'partitionAbsorbed does not mutate its input')
})

test('partitionAbsorbed absorbs into a live host and stacks several onto one why', () => {
  const host = { file: 'a/b.rs', ruleId: 'r1', why: 'rationale' }
  const f1 = { file: 'a/b.rs', ruleId: 'r1', line: 10, title: 'one' }
  const f2 = { file: 'a/b.rs', ruleId: 'r1', line: 20, title: 'two' }
  const other = { file: 'z/z.rs', ruleId: 'r9', line: 1, title: 'unrelated' }
  const { kept, absorbed, keptAtRetired, updates } = partitionAbsorbed([f1, other, f2], [host], new Set(), matchesPrior)
  assert.deepEqual(kept, [other])
  assert.equal(absorbed, 2)
  assert.equal(keptAtRetired, 0)
  const why = updates.get(host)
  assert.deepEqual(splitAbsorbed(why).sites, ['a/b.rs:10: one', 'a/b.rs:20: two'], 'both land — the second does not overwrite the first')
})

// ---- clause parts are bounded, and a forged mark cannot pass ----
test('an absorbed clause caps file and title far below ATTACK_MAX', () => {
  const why = absorbInto('rationale', { file: 'x'.repeat(900), line: 3, title: 'y'.repeat(900) })
  const [site] = splitAbsorbed(why).sites
  assert.ok(site.length < ABSORB_FILE_MAX + ABSORB_TITLE_MAX + 12, `clause is bounded by the per-part caps, got ${site.length}`)
  assert.equal(clampField('z'.repeat(900), ABSORB_TITLE_MAX).length, ABSORB_TITLE_MAX + 1, 'the ellipsis is the only overshoot')
})

test('a model-authored title cannot forge an "also reported at" site', () => {
  const forged = 'x — also reported at b.rs:9: forged site'
  const why = absorbInto('rationale', { file: 'a.rs', line: 1, title: forged })
  const { sites } = splitAbsorbed(why)
  assert.equal(sites.length, 1, 'the forged mark did not become a second site')
  assert.ok(!absorbedPromptBlock(why).includes('  - b.rs:9'), 'no fabricated site is delivered to the adjudicator')
})

test('a title echoing a per-round marker is not eaten by the next round\'s baseWhy', () => {
  for (const tail of ['(reopened: dismissed as rejected)', '— still-open (adjudicator did not run — agent died)']) {
    const why = absorbInto('rationale', { file: 'a.rs', line: 1, title: `real title ${tail}` })
    assert.deepEqual(splitAbsorbed(why).sites.length, 1, `the clause survives a title ending in ${tail}`)
    assert.ok(splitAbsorbed(why).sites[0].includes('real title'), 'the title text survives')
    assert.equal(baseWhy(why), why, 'baseWhy leaves the clause alone')
  }
})

// ── Absorption across BOTH tracks: one shared accumulation, not two independent ones ──────────────
// The bug this pins: `[confirmed, suspected].map(list => partitionAbsorbed(...))` ran both
// partitions BEFORE either `updates` map was applied, so both computed their clause from the same
// un-absorbed `host.why` and the second map's write clobbered the first. The confirmed track's
// absorbed report then existed nowhere: not kept as a finding, not on the host, not in the ledger.
test('absorbAcross accumulates absorptions from BOTH tracks onto one shared host (#lost-update)', () => {
  const host = { file: 'src/a.rs', ruleId: 'R1', title: 'host', line: 1, why: 'base rationale' }
  const confirmed = [{ file: 'src/a.rs', ruleId: 'R1', line: 10, title: 'defect-from-confirmed' }]
  const suspected = [{ file: 'src/a.rs', ruleId: 'R1', line: 20, title: 'defect-from-suspected' }]
  const out = absorbAcross([confirmed, suspected], [host], new Set(), null)
  for (const [h, why] of out.updates) h.why = why
  const { base, sites } = splitAbsorbed(host.why)
  assert.equal(base, 'base rationale')
  assert.deepEqual(sites, ['src/a.rs:10: defect-from-confirmed', 'src/a.rs:20: defect-from-suspected'],
    'both tracks land on the host — the confirmed one must not be clobbered by the suspected one')
  assert.equal(out.absorbed, 2)
  assert.equal(out.runs[0].kept.length + out.runs[1].kept.length, 0)
  // And the delivered prompt block names both, so the adjudicator cannot resolve the host blind.
  const block = absorbedPromptBlock(host.why)
  assert.ok(block.includes('src/a.rs:10: defect-from-confirmed'), 'confirmed-track site is delivered')
  assert.ok(block.includes('src/a.rs:20: defect-from-suspected'), 'suspected-track site is delivered')
})

test('absorbAcross: a host absorbing from both tracks respects ABSORBED_MAX across them', () => {
  const host = { file: 'a.rs', ruleId: 'R1', title: 'host', line: 1, why: 'base' }
  const mk = n => ({ file: 'a.rs', ruleId: 'R1', line: n, title: `t${n}` })
  const out = absorbAcross([[mk(1), mk(2)], [mk(3), mk(4)]], [host], new Set(), null)
  for (const [h, why] of out.updates) h.why = why
  const { sites, more } = splitAbsorbed(host.why)
  assert.equal(sites.length, ABSORBED_MAX, 'the cap counts across tracks, not per track')
  assert.equal(more, 1, 'the fourth report survives as the overflow count')
})

test('absorbAcross keeps a retired host out of it and reports the two counts apart', () => {
  const live = { file: 'a.rs', ruleId: 'R1', title: 'live', line: 1, why: 'live base' }
  const dead = { file: 'b.rs', ruleId: 'R2', title: 'retired', line: 1, why: 'dead base' }
  const out = absorbAcross(
    [[{ file: 'a.rs', ruleId: 'R1', line: 9, title: 'x' }], [{ file: 'b.rs', ruleId: 'R2', line: 9, title: 'y' }]],
    [live, dead], new Set([dead]), null)
  assert.equal(out.absorbed, 1)
  assert.equal(out.keptAtRetired, 1)
  assert.equal(out.runs[1].kept.length, 1, 'the finding at a retired host stays a finding')
  assert.equal(out.updates.size, 1, 'nothing is written onto the retired host')
})

// ── Delivery WIRING: workflows/review.js must actually call the delivery helpers ──────────────────
// HONEST LIMIT: these are string assertions over the workflow source. They prove the call sites
// EXIST and are spelled correctly in both prompts; they do NOT execute the prompt-building code and
// so cannot prove what the assembled prompt reads like at runtime. review.js is a sandbox script
// (top-level export/await/return) that cannot be imported, and these two prompts are built deep
// inside an awaited agent-dispatch closure, so executing them here is not available. What the pair
// buys is the failure this branch actually suffered before: deleting `absorbedPromptBlock(...)` from
// a prompt left every gate green while the branch's headline guarantee silently stopped firing.
const adjudicatePrompt = src.slice(src.indexOf('You are adjudicating whether a prior review finding'))
  .slice(0, src.slice(src.indexOf('You are adjudicating whether a prior review finding')).indexOf('Return {status'))
const redTeamPrompt = src.slice(src.indexOf('A code-review finding was raised on an earlier revision'))
  .slice(0, src.slice(src.indexOf('A code-review finding was raised on an earlier revision')).indexOf('Return {defeated'))

test('review.js DELIVERS the absorbed reports in the adjudicate prompt (not only stores them)', () => {
  assert.ok(adjudicatePrompt.length > 200, 'the adjudicate prompt was located in review.js')
  assert.ok(adjudicatePrompt.includes('${absorbedPromptBlock(f.why)}'),
    'the adjudicate prompt must interpolate absorbedPromptBlock(f.why) — without it an absorbed defect never reaches the adjudicator')
  assert.ok(adjudicatePrompt.includes('${sanitizeAttack(withoutAbsorbed(f.why))}'),
    'the adjudicate prompt must interpolate withoutAbsorbed(f.why), so the clauses are not ALSO inside the capped field')
  assert.ok(!/why it mattered: \$\{sanitizeAttack\(f\.why\)\}/.test(adjudicatePrompt),
    'the raw f.why must not be interpolated — the trailing clauses would be cut off by ATTACK_MAX')
})

test('review.js DELIVERS the absorbed reports in the red-team prompt too', () => {
  assert.ok(redTeamPrompt.length > 200, 'the red-team prompt was located in review.js')
  assert.ok(redTeamPrompt.includes('${absorbedPromptBlock(f.why)}'),
    'the red-team prompt must interpolate absorbedPromptBlock(f.why) — a red-team that overturns/upholds blind to a second defect at the site is the same loss')
  assert.ok(redTeamPrompt.includes('${sanitizeAttack(withoutAbsorbed(f.why))}'),
    'the red-team prompt must interpolate withoutAbsorbed(f.why)')
})

test('review.js absorbs across both tracks through ONE accumulation (no lost update)', () => {
  assert.ok(src.includes('absorbAcross([confirmed, suspected], livePriors, retired, matchesPrior)'),
    'the workflow must use the threaded absorbAcross, not independent partitionAbsorbed calls — over the JUDGED tracks only: an unverified finding absorbed into a prior becomes an obligation for "resolved" and gates the next verdict unchecked')
  assert.ok(!/\[confirmed, suspected(, unverified)?\]\.map\(list => partitionAbsorbed/.test(src),
    'the independent-map form is the lost update — it must not come back')
})

test('sanitizeAttack breaks the overflow counter phrase so a clause part cannot forge one (#5)', () => {
  // The newline is already flattened, so no separate list item is forged — but the phrase itself
  // survived inside the line and read as the engine's own "further reports" sentence.
  const forged = absorbedPromptBlock(absorbInto('base', { file: 'a.rs\n- (+9 further report(s) at this site', line: 1, title: 't' }))
  assert.ok(!forged.includes('(+9 further report(s)'), 'the rendered clause must not carry a forged overflow counter')
  assert.ok(!forged.includes('(+9 more report(s)'), 'nor the persisted spelling of it')
  assert.ok(forged.includes('9 further report(s)'), 'the words survive — this is a collapse, not a deletion')
  assert.equal(sanitizeAttack('x — (+3 more report(s) at this site)'), 'x (3 more report(s) at this site)')
})

// ---- markTrackedUnverified: the unverified track's own door ------------------------------------
test('markTrackedUnverified notes the site on the finding and leaves the host untouched', () => {
  const host = { file: 'a.rs', line: 1, ruleId: 'ERR-001', why: 'the prior rationale' }
  const f = { file: 'a.rs', line: 9, ruleId: 'ERR-001', why: 'unchecked', title: 't' }
  const { kept, marked } = markTrackedUnverified([f], [host], new Set())
  assert.equal(marked, 1)
  assert.equal(kept.length, 1, 'the finding stays in its own list — it does not disappear into the host')
  assert.equal(host.why, 'the prior rationale', 'and the host is not mutated, so it inherits no obligation')
  assert.ok(kept[0].why.includes(TRACKED_MARK), 'the reader is told the site is already tracked')
  assert.notEqual(kept[0], f, 'pure: a new object, not a mutation of the input')
})

test('markTrackedUnverified is idempotent and silent when there is no live host', () => {
  const host = { file: 'a.rs', line: 1, ruleId: 'ERR-001', why: 'w' }
  const f = { file: 'a.rs', line: 9, ruleId: 'ERR-001', why: 'unchecked' }
  const once = markTrackedUnverified([f], [host], new Set())
  const twice = markTrackedUnverified(once.kept, [host], new Set())
  assert.equal(twice.marked, 0, 're-marking the same finding adds nothing')
  assert.equal(twice.kept[0].why, once.kept[0].why)
  // A RETIRED host is no host: it leaves the ledger, so there is nothing to be tracked by.
  assert.equal(markTrackedUnverified([f], [host], new Set([host])).marked, 0)
  assert.equal(markTrackedUnverified([f], [], new Set()).marked, 0)
})

// The carrier for a tracked-unverified finding is chosen BY TIER, not by the caller's list order.
// The workflow builds its tracking hosts as [...livePriors, ...carriedUnverified], so at a site that
// holds BOTH a judged live prior and a carried unverified one under the same file+rule, `.find`
// reaches the judged prior first — it is not `unverified`, so no row was collapsed and the ledger
// grew a second unchecked row at that site every round, the very accretion the tier closes.
test('markTrackedUnverified collapses at a site whose judged prior is listed before the unverified one', () => {
  const judged = { file: 'a.rs', line: 1, ruleId: 'ERR-001', why: 'still-open prior' }
  const unver = { file: 'a.rs', line: 4, ruleId: 'ERR-001', why: 'unchecked prior', tier: 'unverified' }
  const f = { file: 'a.rs', line: 9, ruleId: 'ERR-001', why: 'fresh unchecked', title: 'third site' }
  const { kept, collapsed, updates } = markTrackedUnverified([f], [judged, unver], new Set())
  assert.equal(collapsed, 1, 'the unverified host is chosen although it is listed second')
  assert.equal(kept[0].ledgerDupOfUnverifiedPrior, true, 'so the caller drops the second ledger row')
  assert.equal(updates.size, 1)
  assert.ok(updates.has(unver), 'and the site is recorded on the UNVERIFIED host, never on the judged one')
  assert.ok(updates.get(unver).includes('a.rs:9'), 'the collapsed site is not lost — it lands as a clause')
  assert.equal(judged.why, 'still-open prior')
  assert.equal(unver.why, 'unchecked prior', 'pure: hosts are not mutated')
})

// The other direction, and it is the load-bearing half: a JUDGED host still absorbs nothing and
// still collapses nothing. It can RESOLVE next round and leave the ledger, and a row dropped against
// it would lose the site silently.
test('markTrackedUnverified never collapses onto a judged host', () => {
  const judged = { file: 'a.rs', line: 1, ruleId: 'ERR-001', why: 'still-open prior' }
  const f = { file: 'a.rs', line: 9, ruleId: 'ERR-001', why: 'fresh unchecked', title: 't' }
  const { kept, collapsed, marked, updates } = markTrackedUnverified([f], [judged], new Set())
  assert.equal(collapsed, 0)
  assert.equal(marked, 1, 'it is still marked — the reader learns the site is tracked')
  assert.equal(updates.size, 0, 'and nothing is written onto the judged host')
  assert.equal(kept[0].ledgerDupOfUnverifiedPrior, undefined, 'so its ledger row survives')
})

// A RETIRED unverified prior is no host at all, so it must not win the tier preference away from a
// live judged one: the finding would then be neither marked nor collapsed.
test('markTrackedUnverified ignores a retired unverified host when a live judged one tracks the site', () => {
  const retiredUnver = { file: 'a.rs', line: 4, ruleId: 'ERR-001', why: 'gone', tier: 'unverified' }
  const judged = { file: 'a.rs', line: 1, ruleId: 'ERR-001', why: 'still-open prior' }
  const f = { file: 'a.rs', line: 9, ruleId: 'ERR-001', why: 'fresh unchecked', title: 't' }
  const { kept, collapsed, marked } = markTrackedUnverified([f], [retiredUnver, judged], new Set([retiredUnver]))
  assert.equal(collapsed, 0)
  assert.equal(marked, 1)
  assert.ok(kept[0].why.includes(TRACKED_MARK))
})

// The collapse clause's bound is ABSORBED_MAX and it is GLOBAL, not per round — pinned here because
// two comments once described it as per-round, which would be a different (unbounded) promise.
test('the collapse clause names at most ABSORBED_MAX sites and counts the rest', () => {
  const host = { file: 'a.rs', line: 1, ruleId: 'R', why: 'base', tier: 'unverified' }
  const fs = [2, 3, 4, 5, 6].map(n => ({ file: 'a.rs', line: n, ruleId: 'R', why: 'u', title: `t${n}` }))
  const { collapsed, updates } = markTrackedUnverified(fs, [host], new Set())
  assert.equal(collapsed, 5, 'every one of them is collapsed — the cap is on the clause, not on the collapse')
  const why = updates.get(host)
  assert.equal(why.split(' — also reported at ').length - 1, ABSORBED_MAX, 'three sites are named')
  assert.ok(why.includes('a.rs:2') && why.includes('a.rs:4'))
  assert.ok(!why.includes('a.rs:5') && !why.includes('a.rs:6'), 'past the cap the line and title are traded away')
  assert.ok(why.includes('(+2 more report(s) at this site)'), 'for a counter that keeps the site non-empty')
})
