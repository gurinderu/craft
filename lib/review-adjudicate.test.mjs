...
import { ATTACK_MAX, sanitizeAttack, baseWhy, isHighSeverity, classifyRedTeam, adjudicateOne, shouldRedTeam, carriedKey, alreadyCarried, findCarrier, absorbInto, ABSORBED_MAX, splitAbsorbed, withoutAbsorbed, absorbedPromptBlock, partitionAbsorbed, clampField, ABSORB_FILE_MAX, ABSORB_TITLE_MAX } from './review-adjudicate.mjs'
...
test('re-review wiring: new findings matching a still-live prior are absorbed, resolved priors excepted', () => {
  ...
  assert.ok(src.includes('partitionAbsorbed(list, livePriors, retired, matchesPrior)'), 'the partition (tested by execution below) decides absorb-or-keep')
  assert.ok(src.includes('const runs = [confirmed, suspected].map('), 'both tracks go through the same partition')
  assert.ok(src.includes('for (const { updates } of runs) for (const [host, why] of updates) host.why = why'), 'the host why updates the partition computed are applied')
