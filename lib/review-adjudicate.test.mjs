...
// A budget nothing enforces is advice. The dispatch deadline used to be 7min behind a prompt
// declaring a 3min ceiling — more than twice over, so a pass could ignore the ceiling entirely and
// still be paid for. Keep the allowance within a return trip of what the prompt actually declares.
test('the preflight dispatch deadline does not exceed the ceiling its prompt declares', () => {
  const m = src.match(/label: `preflight:\$\{profile\.id\}`[^)]*deadlineMs: (\d+)/)
  assert.ok(m, 'preflight dispatch carries an explicit deadlineMs')
  const ms = Number(m[1])
  assert.ok(ms >= 180000 && ms <= 240000, `preflight deadline ${ms}ms is not within a return trip of the declared 3min ceiling`)
})
