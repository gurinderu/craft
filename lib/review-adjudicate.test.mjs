...
test('re-review wiring: new findings matching a still-live prior are absorbed, resolved priors excepted', () => {
  ...
  assert.ok(src.includes('absorbAcross([confirmed, suspected], livePriors, retired, matchesPrior)'), 'both tracks go through ONE threaded partition (tested by execution below), which decides absorb-or-keep')
...
test('review.js absorbs across both tracks through ONE accumulation (no lost update)', () => {
  assert.ok(src.includes('absorbAcross([confirmed, suspected], livePriors, retired, matchesPrior)'),
    'the workflow must use the threaded absorbAcross, not two independent partitionAbsorbed calls')
  assert.ok(!/\[confirmed, suspected\]\.map\(list => partitionAbsorbed/.test(src),
