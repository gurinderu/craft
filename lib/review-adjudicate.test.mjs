...
function loadVerifyRouting() {
  const cut = src.indexOf("phase('Scout')")
  const prefix = src.slice(0, cut).replace(/^export const meta/m, 'const meta')
  const m = src.match(/const BLOCKING_RULE_FAMILY[\s\S]*?\nfunction tierFromVotes\([^)]*\) \{[\s\S]*?\n\}/)
  ...
  return new Function('args', 'agent', 'parallel', 'pipeline', 'phase', 'log', 'budget', 'workflow',
    `${prefix}\n${m[0]}\n;return { verifyTier, tierFromVotes, BATCH_SIZE };`)({}, stub, stub, stub, stub, stub, budget, stub)
}
...
test('verifyTier never batches or skips a finding that can block — Critical/High stay individual', () => {
  ...
  // A lens that UNDER-calls severity is the one real risk of skipping; the blocking rule families
  // are the escape hatch, so a SAF/ERR/CON finding is verified individually at ANY severity.
  for (const id of ['SAF-001', 'ERR-002', 'CON-003']) {
    assert.equal(verifyTier({ severity: 'Info', ruleId: id }), 'individual', `${id} at Info is still verified individually`)
  }
})
