...
function loadHelpers() {
  ...
  const factory = new Function(
    ...
    `${prefix}\n;return { sanitizeAttack, baseWhy, ATTACK_MAX, classifyRedTeam, adjudicateOne, redTeamInvariant, shouldRedTeam, promptFields, isHighSeverity };`,
  )
  return factory({}, stub, stub, stub, stub, stub, budget, stub)
}

const { sanitizeAttack, baseWhy, ATTACK_MAX, classifyRedTeam, adjudicateOne, redTeamInvariant, shouldRedTeam, promptFields, isHighSeverity } = loadHelpers()
...
test('promptFields flattens model-authored title/symbol/ruleId for prompt interpolation, keeps fallbacks', () => {
  const pf = promptFields({ title: 'a\nb ```c```', symbol: 'sym\n```x```', ruleId: 'rule\n```y```' })
  assert.ok(!/\n/.test(pf.title) && !pf.title.includes('`'), 'title flattened')
  assert.ok(!/\n/.test(pf.symbol) && !pf.symbol.includes('`'), 'symbol flattened')
  assert.ok(!/\n/.test(pf.ruleId) && !pf.ruleId.includes('`'), 'ruleId flattened')
...
test('promptFields flattens a drifted/tampered severity for prompt safety, lossless for enum values (round2 #4)', () => {
  ...
  assert.equal(promptFields({ severity: 'Critical' }).severity, 'Critical', 'enum-ish value lossless under sanitizeAttack')
})
