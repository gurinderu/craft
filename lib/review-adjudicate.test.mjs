...
// `fingerprint` and `TRACKED_MARK` — the only free identifiers in its body — to exercise the real
// code, not a copy. TRACKED_MARK is passed REAL (imported above), because the entry's whole job on
// that field is to strip it: a stubbed sentinel would let the strip be deleted with this test green.
function loadToLedgerEntry() {
  ...
  return new Function('fingerprint', 'TRACKED_MARK', `return ${m[0].replace(/^const toLedgerEntry = /, '')}`)(() => 'fp0', TRACKED_MARK)
}
