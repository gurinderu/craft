...
test('pr-comments wiring: the outcome is captured and logged, never discarded', () => {
  // "Best-effort" bounds what may FAIL, not what may be SILENT. The caller asked for comments, so
  // whether any landed is part of the answer — a discarded return value made even the prompt's own
  // "report that" reach nobody.
