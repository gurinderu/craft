...
function loadHelpers() {
  ...
  const factory = new Function(
    ...
    `${prefix}\n;return { sanitizeAttack, baseWhy, ATTACK_MAX, classifyRedTeam, adjudicateOne, redTeamInvariant, shouldRedTeam, promptFields, isHighSeverity, canonicalSeverity, flattenField, shq, isCommitish, ledgerDegraded, shouldFullRescan, FINDING_ITEM, VERDICT_SCHEMA };`,
  )
  return factory({}, stub, stub, stub, stub, stub, budget, stub)
}

const { sanitizeAttack, baseWhy, ATTACK_MAX, classifyRedTeam, adjudicateOne, redTeamInvariant, shouldRedTeam, promptFields, isHighSeverity, canonicalSeverity, flattenField, shq, isCommitish, ledgerDegraded, shouldFullRescan, FINDING_ITEM, VERDICT_SCHEMA } = loadHelpers()
...
function loadVerifyPrompt() {
  const cut = src.indexOf("phase('Scout')")
  const prefix = src.slice(0, cut).replace(/^export const meta/m, 'const meta')
  const m = src.match(/function verifyPrompt\(f, idx, isTool, gateProvenance\) \{[\s\S]*?\n\}/)
