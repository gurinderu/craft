...
// still lives there. `canonicalSeverity` no longer does: it moved to lib/review-coverage.mjs and is
// pasted back into the workflow by the craft-inline gate, so it is imported here and exercised as
// the real module rather than as a copy of the workflow's text.
...
function loadHelpers() {
  ...
  const factory = new Function(
    ...
    `${prefix}\n;return { sanitizeAttack, baseWhy, ATTACK_MAX, classifyRedTeam, adjudicateOne, redTeamInvariant, shouldRedTeam, promptFields, isHighSeverity, flattenField, shq, isCommitish, ledgerDegraded, shouldFullRescan, ADJUDICATE_SCHEMA, FINDING_ITEM, VERDICT_SCHEMA, PROFILES };`,
  )
  return factory({}, stub, stub, stub, stub, stub, budget, stub)
}

const { sanitizeAttack, baseWhy, ATTACK_MAX, classifyRedTeam, adjudicateOne, redTeamInvariant, shouldRedTeam, promptFields, isHighSeverity, flattenField, shq, isCommitish, ledgerDegraded, shouldFullRescan, ADJUDICATE_SCHEMA, FINDING_ITEM, VERDICT_SCHEMA, PROFILES } = loadHelpers()
...
test('re-review wiring: new findings matching a still-live prior are dropped, resolved priors excepted', () => {
  ...
  assert.ok(src.includes('confirmed = confirmed.filter(f => !livePriors.some(p => matchesPrior(f, p)))'), 'confirmed deduped against live priors')
})
