...
test('re-review wiring: new findings matching a still-live prior are absorbed, resolved priors excepted', () => {
  ...
  assert.ok(src.includes('absorbAcross([confirmed, suspected, unverified], livePriors, retired, matchesPrior)'), 'all three tracks go through ONE threaded partition (tested by execution below), which decides absorb-or-keep')
...
test('review.js absorbs across both tracks through ONE accumulation (no lost update)', () => {
  assert.ok(src.includes('absorbAcross([confirmed, suspected, unverified], livePriors, retired, matchesPrior)'),
    'the workflow must use the threaded absorbAcross, not independent partitionAbsorbed calls — and the unverified track is in it too, or an unchecked Low duplicating a live prior would be listed twice')
