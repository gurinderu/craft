...
test('every phase deadline sits above the measured maximum of legitimate work', () => {
  const { deadlineMsFor, DEFAULT_DEADLINE_MS } = loadDeadline()
  // Maxima measured across two real runs (213 agents). A deadline at or below these would cut work
  // that was going to succeed, and the retry would cost MORE than the stall it replaced.
  ...
  for (const [phase, secs] of Object.entries(measuredMaxSeconds)) {
    ...
    assert.ok(ms > secs * 1000, `${phase}: deadline ${ms}ms must exceed the ${secs}s longest real agent`)
    assert.ok(ms <= 60 * 60 * 1000, `${phase}: a deadline over an hour cannot rescue a stalled run`)
  }
...
// ---- one verification wave ----
test('batched and individual verification are dispatched in a single parallel', () => {
  ...
  assert.match(src, /const settledVerdicts = await parallel\(batchThunks\.concat\(individualThunks\)\)/)
