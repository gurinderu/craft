...
import { ATTACK_MAX, sanitizeAttack, baseWhy, isHighSeverity, classifyRedTeam, adjudicateOne, shouldRedTeam, carriedKey, alreadyCarried, findCarrier, absorbInto, ABSORBED_MAX } from './review-adjudicate.mjs'
...
test('re-review wiring: new findings matching a still-live prior are absorbed, resolved priors excepted', () => {
  ...
  assert.ok(src.includes('const host = findCarrier(f, livePriors, matchesPrior)'), 'the match returns the host, not just a boolean')
  assert.ok(src.includes('if (!retired.has(host)) host.why = absorbInto(host.why, f)'), 'an absorbed finding is recorded on its persisted host')
  assert.ok(src.includes('confirmed = confirmed.filter(keep)') && src.includes('suspected = suspected.filter(keep)'), 'both tracks go through the same absorb-or-keep')
})
