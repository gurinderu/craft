...
// ========= adversarial-review: the THIRD budget floor — semantic dedup (PATH coverage, not a carrier) =========
// The two floors above each gate a VERDICT: skipped lenses and an unverified coverage gap both downgrade
// the run to INCOMPLETE. This third `budget.remaining() > BUDGET_FLOOR` gate — the semantic-dedup
// clusterer at :931 — is deliberately NOT a verdict carrier: skipping it keeps MORE findings (both
// cross-vocabulary formulations of one defect survive), which is the conservative direction, and it
// writes no notRun. So this pins PATH COVERAGE, not a downgrade — the finding is explicit that asserting
// a verdict change here would be wrong. What a regression would do silently: flip the comparison and run
// the haiku clusterer precisely when the budget cannot afford it (or skip it always), with nothing else
// in the suite reddening. The property: below the floor at the dedup point, the clusterer is never
// dispatched and the pool passes through untouched.
//
// DEDUP_THRESHOLD gates whether the block is reached at all — read it from the source (a copy here would
// not move with it) and drive one lens to return THRESHOLD+1 findings, two of them the SAME defect in
// different words. Under an unlimited budget the clusterer merges those two; below the floor both must
// live. All findings sit in DISTINCT files so tier-1 mechanical dedup merges none of them and the whole
// pool reaches the semantic gate.
const ADV_DEDUP_A = { title: 'TOCTOU window on the balance debit path', file: 'src/pay.rs', line: 40, severity: 'medium', description: 'the debit reads then writes without a lock', fix: 'take the row lock across the debit', whereChecked: '' }
const ADV_DEDUP_B = { title: 'concurrent update race on the account balance', file: 'src/ledger.rs', line: 900, severity: 'medium', description: 'two writers update the balance without serialization', fix: 'serialize the balance write', whereChecked: '' }

test('adversarial-review: below BUDGET_FLOOR at the dedup point the semantic clusterer is never dispatched — the pool survives untouched (PATH coverage, NOT a verdict carrier)', async () => {
  const src = engineSource('adversarial-review')
  const tm = src.match(/DEDUP_THRESHOLD\s*=\s*([\d_]+)/)
  assert.ok(tm, 'DEDUP_THRESHOLD must be readable from the engine source — its declaration shape moved')
  const THRESHOLD = Number(tm[1].replace(/_/g, ''))
  const fm = src.match(/BUDGET_FLOOR\s*=\s*([\d_]+)/)
  assert.ok(fm, 'BUDGET_FLOOR must be readable from the engine source — its declaration shape moved')
  const FLOOR = Number(fm[1].replace(/_/g, ''))

  // THRESHOLD+1 findings, so `kept.length > DEDUP_THRESHOLD` holds at the gate: the two same-defect
  // findings plus fillers, each in its own file so mechanical dedup leaves the whole pool intact.
  const fillers = Array.from({ length: THRESHOLD - 1 }, (_, i) => ({
    title: `unrelated defect ${i} in module ${i}`, file: `src/mod${i}.rs`, line: 10 + i, severity: 'medium', description: `d${i}`, fix: `f${i}`, whereChecked: '',
  }))
  const findings = [ADV_DEDUP_A, ADV_DEDUP_B, ...fillers] // THRESHOLD + 1 total
  const script = {
    scout: { baseRef: 'main', sizeBucket: 'small', lenses: ['correctness'], changedFiles: ['src/lib.rs'], notes: 'x' },
    'index-warmup': { indexed: false, notes: 'x' },
    review: { findings },
    // Reached ONLY by the unlimited-budget control, where the clusterer runs and merges findings 0 and
    // 1 — the divergence that proves the walled run skipped it on budget alone.
    'dedup-semantic': { clusters: [[0, 1]] },
    'coverage-critic': { findings: [] },
    'verify-batch': ({ prompt }) => ({
      verdicts: [...prompt.matchAll(/--- FINDING (\d+) ---/g)].map(mm => ({
        index: Number(mm[1]), refuted: false, citedLineMatches: true, reachable: true, premiseSupported: true, reason: 'ok',
      })),
    }),
    'log-run': { ok: true },
    '*': null,
  }
  const titlesOf = r => [...r.reportValue.confirmed, ...r.reportValue.suspected].map(f => f.title)

  // The control: unlimited budget. The clusterer is dispatched and merges the two same-defect findings,
  // so the pool drops by one and only the head formulation survives as a finding of its own.
  const control = await runEngine('adversarial-review', { args: {}, script })
  assert.ok(control.calls.some(c => c.label === 'dedup-semantic'),
    'the control must dispatch the semantic clusterer — the run the floor skips is this one')
  assert.ok(control.logs.some(l => /Semantic dedup: merged/.test(l)),
    'and the clusterer must actually merge the two same-defect findings under an unlimited budget')
  const controlTitles = titlesOf(control)
  assert.ok(controlTitles.includes(ADV_DEDUP_A.title) && !controlTitles.includes(ADV_DEDUP_B.title),
    'the merge keeps the head formulation and folds the duplicate into it — one of the two leaves the pool')

  // The wall: `remaining` sits one CALL_SPEND BELOW the floor at the dedup gate but AT the floor when the
  // lens spawns. scout + index-warmup + the one finder lens spend 3×CALL_SPEND before the gate reads
  // `remaining`; FLOOR + 2×CALL_SPEND puts spawn-time remaining at exactly FLOOR (>= floor, so the lens
  // runs) and dedup-time remaining at FLOOR - CALL_SPEND (below the floor, so the clusterer is skipped).
  // No dispatch is ever refused — a SOFT skip at the floor, not the exhaustion wall.
  const walled = await runEngine('adversarial-review', { args: {}, script, budgetTotal: FLOOR + 2 * CALL_SPEND })

  assert.ok(walled.calls.some(c => String(c.label).startsWith('review:') && !c.threw),
    'the finder lens must have run above the floor — the pool the dedup gate reads exists only because the lens looked')
  assert.ok(walled.logs.some(l => new RegExp(`Review: ${THRESHOLD + 1} raw findings -> ${THRESHOLD + 1} after mechanical dedup`).test(l)),
    'all THRESHOLD+1 findings must reach the semantic gate — mechanical dedup merged none of them')
  assert.ok(!walled.calls.some(c => c.label === 'dedup-semantic'),
    'the semantic clusterer must NEVER be dispatched below the floor — the path this test covers')
  assert.ok(!walled.logs.some(l => /Semantic dedup/.test(l)),
    'and the semantic-dedup block must not execute at all — neither a "merged" nor a "no duplicates" line')
  assert.ok(!walled.calls.some(c => c.threw),
    'the skip is at the soft floor, before any dispatch is refused — not at the exhaustion wall')

  // Both cross-vocabulary formulations survive in the pool — the conservative outcome the finding names
  // explicitly: skipping dedup keeps MORE findings, it does not lower the verdict. No verdict assertion
  // here on purpose: this path carries none.
  const walledTitles = titlesOf(walled)
  assert.ok(walledTitles.includes(ADV_DEDUP_A.title) && walledTitles.includes(ADV_DEDUP_B.title),
    'both same-defect formulations survive the skipped dedup — the clusterer that would have merged them never ran')
})
