...
test('a lost write is appended to the report, naming what failed and why', () => {
  ...
  assert.match(report, /^## Verdict\n⛔ Block — one Critical\./, 'the verdict must survive untouched, first')
  assert.match(report, /## ⚠️ Telemetry lost/)
  assert.match(report, /the run record — craft-log-run FAILED: Cannot find module/)
  assert.match(report, /is NOT evidence that the review never ran/)
})

test('every report the engine can return passes through out()', () => {
  // A new exit that forgets the wrapper reintroduces the silence for exactly that path.
  const unwrapped = src.split('\n').filter(l => /^\s*return \[`## Verdict`/.test(l) && !l.includes('return out('))
  assert.deepEqual(unwrapped, [], 'a report return that skips out() loses the marker on that path')
  assert.ok(src.includes('return out(report || fallbackReport())'), 'the normal exit must be wrapped')
  assert.equal((src.match(/return out\(/g) || []).length, 8, 'all eight report exits wrapped')
})
...
test('a failed write is asserted by the logger, not inferred from silence', () => {
  ...
  // The decision this guards: reporting, never failing the run.
  assert.ok(!/throw .*telemetry/i.test(src), 'a lost record must never abort the review')
})
