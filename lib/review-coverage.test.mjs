...
test('every logger command resolves its script path before changing directory', () => {
  ...
  assert.match(plugin.LOGGER_PRELUDE, /^CRAFT_LOGGER="\$\{CLAUDE_PLUGIN_ROOT:\?craft-log-run FAILED[^}]*\}\/lib\/craft-log-run\.mjs"\n$/)
...
// ---- tripwires for the two review.js properties no gate can reach ----
// Same standing as the source-text assertions above: a revert is caught, a rewording is not.

test('the checkpoint notices a directory it did not ask for', () => {
  // A refused --dir does not fail the checkpoint: the logger mints a fresh directory and returns a
  // valid runDir, so every slice written into the directory we asked for is stranded and finalize
  // folds only the new one. The engine compares asked-for against came-back; without that comparison
  // the run reports clean. Nothing else in the repo can execute this path.
  assert.match(src, /const asked = runDir/, 'the directory asked for must be captured before the call')
  assert.match(src, /asked && res\.runDir !== asked/, 'and compared against what came back')
  assert.ok(
    /asked && res\.runDir !== asked[\s\S]{0,400}?noteTelemetryLoss/.test(src),
    'a minted directory must reach noteTelemetryLoss, not be adopted in silence',
  )
})
...
test('a landed-but-degraded run reaches the banner instead of reading as clean', () => {
  // The record can land while its run directory does not. That is neither a lost record nor silence,
  // and both halves are needed for it to surface: the schema description has to ask for the WARNING
  // line (the model steers by it), and the engine has to have a branch for `ok && reason`.
  const logging = fs.readFileSync(path.join(root, 'lib', 'run-logging.mjs'), 'utf8')
  assert.match(logging, /ok is true AND the script printed a craft-log-run WARNING/,
    'the error field must ask for the WARNING line on a successful write')
  assert.match(src, /else if \(landed\.reason\)/, 'and review.js must have somewhere to put it')
  assert.ok(
    /else if \(landed\.reason\)[\s\S]{0,300}?noteTelemetryLoss/.test(src),
    'a landed-but-degraded run must reach the telemetry banner',
  )
})
...
test('the landed-directory sentinel reads the same at every site that writes or matches it', () => {
  ...
  const engines = ['review.js', 'adversarial-review.js', 'rust-audit.js', 'triage-findings.js']
