...
test('uncovered coverage-gap files reach the verdict — without polluting notRun', () => {
  ...
  assert.ok(
    !/notRun\.push\(\.\.\.savedByFloor\)/.test(src),
    'a saving must never enter notRun unconditionally — only the revoked-premise path may convert it',
  )
})
...
test('every report the engine can return passes through out()', () => {
  ...
  // The normal exit wraps the synthesized report AND appends two sections to it, for the same
  // reason in both cases: the synthesis prompt is composed before either fact exists, so the model
  // cannot carry them. A scope the caller asked for and did not get; and a verification saving
  // whose justifying verdict never arrived.
  assert.ok(
    /return out\(\(report \|\| fallbackReport\(\)\)( \+ floorPremiseSection\(\))?( \+ scopeSection\(\))?\)/.test(src),
    'the normal exit must be wrapped',
  )
})
