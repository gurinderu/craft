...
test('every logger command resolves its script path before changing directory', () => {
  ...
  // Two sites remain in review.js — the checkpoint and the prior-round read. The finalize command
  // moved into the shared write path (lib/run-logging.mjs, inlined here), where the same property
  // is asserted by EXECUTING it: see lib/run-logging.test.mjs, 'loggerPrelude resolves the logger
  // BEFORE any cd'.
  const sites = src.match(/^.*cd \$\{shq\(repoArg \|\| '\.'\)\} && node .*$/gm) || []
  assert.equal(sites.length, 2, 'expected the checkpoint and prior-round logger commands')
...
test('a failed write is asserted by the logger, not inferred from silence', () => {
  ...
  const ckptIdx = src.indexOf('LOGGER_PATH} checkpoint --phase')
  assert.ok(ckptIdx > 0, 'review.js must still emit a `checkpoint --phase` command')
  const ckptLine = src.slice(ckptIdx, src.indexOf('\n', ckptIdx))
  assert.ok(
    ckptLine.includes("!runDir && checkpointFailed ? '--rejoin ' : ''"),
