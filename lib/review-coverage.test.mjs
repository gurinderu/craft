...
function loadHelpers(workflowArgs = {}, agentStub = null) {
  ...
  const factory = new Function(
    ...
    `${prefix}\n;return { PROFILES, requestedLangs, LOGGER_PATH, repoArg,
      LOGGER_PRELUDE: typeof loggerPreludeNow === 'function' ? loggerPreludeNow() : null,
...
test('uncovered coverage-gap files reach the verdict — without polluting notRun', () => {
  ...
  const recordVerdictIdx = src.indexOf("verdict: recordVerdict + (incompleteNotes.length || !floorPremiseHeld ? ' (INCOMPLETE)' : '')")
  ...
  // The second road to INCOMPLETE, and it cannot go through `incompleteNotes`: a verification
  // skipped as a saving becomes a coverage hole only if the verdict it was justified by fails to
  // arrive, which is not known until long after `incompleteNotes` is built.
  assert.ok(
    /const floorPremiseHeld = !savedByFloor\.length \|\| recordVerdict === 'Block'/.test(src),
    'the saving\'s premise must be re-read against the verdict that actually came out',
  )
  // Matched on the GUARD rather than on one spelling of the offence: a negative match for a single
  // literal is passed by `notRun.push(...savedByFloor, x)` or by a loop, so it cannot detect the
  // regression it names. What must hold is that every conversion sits under the revoked premise.
  // COUNTED FIRST, because a loop over zero matches asserts nothing and is green exactly when the
  // conversion has been deleted or rewritten into a shape this pattern does not see. That is the
  // same vacuity the previous spelling had, wearing a different coat — a tripwire whose comment
  // describes a guard it does not provide is worse than none, because it stops the next reader
  // looking. The behavioural guarantee lives in lib/review-medium-economy.test.mjs; this one checks
  // only WHERE a conversion may sit, and it must fail if there is no conversion to check.
  const conversions = [...src.matchAll(/notRun\.push\([^\n]*savedByFloor/g)]
  assert.equal(conversions.length, 1, 'exactly one conversion of a saving into notRun must exist')
  for (const m of conversions) {
    const before = src.slice(0, m.index)
    const guard = before.lastIndexOf('if (!floorPremiseHeld) {')
    const closed = before.lastIndexOf('\n}')
    assert.ok(guard > closed, 'a saving may enter notRun ONLY inside the revoked-premise guard')
  }
})
...
test('every logger command resolves its script path before changing directory', () => {
  ...
  // One site remains hand-written in review.js — the prior-round read. The finalize AND checkpoint
  // commands moved into the shared write path (lib/run-logging.mjs, inlined here), where the same
  // property is asserted by EXECUTING it: see lib/run-logging.test.mjs, 'loggerPrelude resolves the
  // logger BEFORE any cd'. The checkpoint prompt used to be a hand-copy of the record one, and every
  // fix to the shared path had to be applied twice — the copy was the one nearly missed each time.
  const sites = src.match(/^.*cd \$\{shq\(repoArg \|\| '\.'\)\} && node .*$/gm) || []
  assert.equal(sites.length, 1, 'expected the prior-round logger command')
  for (const line of sites) {
    assert.ok(line.startsWith('${loggerPreludeNow()}cd '), `logger prelude must precede the cd: ${line}`)
    assert.ok(line.includes('node ${LOGGER_PATH}'), `logger must run via the captured path: ${line}`)
  }

  // And the two emitted shapes themselves. The comment here used to say "no `.` left to resolve
  // against the reviewed repo" while the assertion pinned `${CLAUDE_PLUGIN_ROOT:-.}` — which is
  // exactly that `.`, resolved BEFORE the cd, i.e. against the repository under review. A repo
  // shipping its own lib/craft-log-run.mjs would have been executed. The fallback is gone: with
  // neither craftRoot nor the env var, the command fails loudly, and a record that cannot be written
  // is already a reported, non-fatal outcome.
  // The prelude AS THIS ENGINE EMITS IT — version and all. Asserting a versionless one was the hole
  // the review found: the search block simply was not in the string under test, so a candidate added
  // to it could not be seen here at all, and this test's stated job is exactly that no candidate may
  // be the working directory.
  const plugin = loadHelpers({ repo: '/other/checkout' })
  assert.equal(plugin.LOGGER_PATH, '"$CRAFT_LOGGER"')
  const prelude = plugin.LOGGER_PRELUDE
  assert.ok(prelude && /plugins\/cache\/craft/.test(prelude), 'the emitted prelude must be the real one, search block included')
  assert.match(prelude, /CLAUDE_PLUGIN_ROOT/, 'the environment is consulted first')
  // Every shape that names the current directory, not just the literal `./` the old assertion knew:
  // `$PWD`, `$(pwd)`, a `:-` default that supplies a path, and a bare relative candidate all put the
  // reviewed repository on the search path once the command has cd'd into it.
  for (const cwdish of [/\$PWD/, /\$\(pwd\)/, /`pwd`/, /\$\{CLAUDE_PLUGIN_ROOT:-[^}]/, /(^|[^/\w."$])lib\/craft-log-run/m]) {
    assert.ok(!cwdish.test(prelude), `the working directory must never be a candidate (${cwdish})`)
  }
  // Same dead branch removed here: `|:\?craft-log-run FAILED` is the pre-fix spelling and would
  // have kept this green through a revert.
  assert.match(prelude, /craft-log-run FAILED[\s\S]*exit 1/, 'an unresolved logger must stop the command')
  // Same correction here: this pinned the early-return spelling, which is exactly the shape that
  // skipped the absoluteness check and the refusal. What must hold is that an explicit root is
  // consulted and is guarded like everything else.
  const pinned = loadHelpers({ repo: '/other/checkout', craftRoot: '/opt/craft' })
  assert.match(pinned.LOGGER_PRELUDE, /\/opt\/craft/, 'an explicit craftRoot is consulted')
  assert.match(pinned.LOGGER_PRELUDE, /craft_usable "\$CRAFT_TRY"/, 'and guarded by the same predicate')
  assert.match(pinned.LOGGER_PRELUDE, /craft-log-run FAILED/, 'and still able to refuse')
})
...
test('every report the engine can return passes through out()', () => {
  ...
  // The normal exit wraps the synthesized report AND amends it three ways, for one reason: the
  // synthesis prompt is composed before any of these facts exists, so the model cannot carry them.
  // A scope the caller asked for and did not get; a verification saving whose justifying verdict
  // never arrived; and the verdict LINE itself, which is what a reader and rust-audit's roll-up
  // actually read — a section at the foot of the document does not take back a green verdict above.
  // Each is pinned NON-optionally here: the previous spelling made them optional, so deleting one
  // left this assertion green, which is the shape of a tripwire that has stopped being one.
  assert.ok(
    /return out\(markVerdictIncomplete\(report \|\| fallbackReport\(\)\) \+ floorPremiseSection\(\) \+ scopeSection\(\)\)/.test(src),
    'the normal exit must be wrapped AND must carry all three amendments',
  )
})
...
test('a failed write is asserted by the logger, not inferred from silence', () => {
  // The ok/dead/malformed distinction now lives in `logRunOutcome` (lib/run-logging.mjs), where it
  // is EXECUTED rather than string-matched — see lib/run-logging.test.mjs. What review.js still owes
  // is asking it and reporting the answer.
  assert.ok(
    /const landed = logRunOutcome\(res\)\s*\n\s*if \(!landed\.ok\) noteTelemetryLoss\('the run record', landed\.reason\)/.test(src),
    'logRun must route its result through logRunOutcome and report a write that did not land',
  )
  assert.ok(
    /checkpointFailed = true\s*\n\s*noteTelemetryLoss\(`phase checkpoint/.test(src),
    'a checkpoint that returns no runDir must be reported, and must arm the rejoin for the rest of the run',
  )
  // Arming it is half the wire. The previous assertion stopped there, so DELETING the flag from the
  // command the logger agent actually runs left this test green — the rejoin would have been armed
  // and never asked for. Both emission sites are pinned: a run whose runDir was lost has to rejoin
  // on the checkpoints AND on the finalize, or it files its record and leaves the directory orphaned.
  // The checkpoint command now comes from the shared builder, so what review.js owes is the ARGUMENT:
  // it must hand the armed rejoin to `checkpointPrompt`. The emitted flag itself is pinned where the
  // builder lives — lib/run-logging.test.mjs — by executing it.
  const ckptIdx = src.indexOf('checkpointPrompt({ payload, craftRoot: craftRootArg')
  assert.ok(ckptIdx > 0, 'review.js must build its checkpoint through the shared prompt')
  const ckptLine = src.slice(ckptIdx, src.indexOf('\n', ckptIdx))
  assert.ok(
    ckptLine.includes('rejoin: armRejoin'),
    `the checkpoint command must EMIT --rejoin once the rejoin is armed, not merely arm it: ${ckptLine}`,
  )
  // The finalize command is built by the shared write path, so what review.js owes is HANDING it the
  // armed state — a `logRunPrompt` call that forgot `rejoin` would file the record and orphan the
  // directory. The flag's emission from those arguments is executed in lib/run-logging.test.mjs.
  assert.ok(
    /logRunPrompt\(\{[\s\S]{0,300}?command: 'finalize', dir: runDir, rejoin: rejoinArmed,/.test(src),
    'logRun must pass this run\'s runDir and armed rejoin into the shared finalize command',
  )
  // The structured ok/error schema now rides in logRunDispatch (asserted against LOGRUN_SCHEMA by
  // execution in lib/run-logging.test.mjs); review.js must actually use it.
  assert.ok(src.includes("logRunDispatch(record, { phase: 'Synthesize' })"), 'the logger agent must be dispatched with the shared schema and payload sizing')
})
...
// `--rejoin` can be armed ONLY after a checkpoint already failed, so it is never armed on the first
// (`-plan`) payload — and `-plan` used to be the only one carrying branch/head. The rejoin search
// therefore saw no identity on exactly the paths where it fires, and matched on the repository root
// alone: any concurrently live review of the same repo was an equally valid candidate to adopt.
test('every checkpoint payload carries the run identity, not just the first', () => {
  const payloads = [...src.matchAll(/await checkpoint\(`\$\{profile\.id\}-(\w+)`, \{([\s\S]{0,200})/g)]
  assert.ok(payloads.length >= 3, `expected the three checkpoints, saw ${payloads.length}`)
  for (const [, phase, head] of payloads) {
    assert.match(head, /branch/, `the ${phase} checkpoint must carry branch`)
    // `head` is shorthand now and carries the run's HEAD; the diff base rides under its own
    // `baseRef` key. They shared the `head` key before, which is what stopped checkpoints being
    // folded into the record at all — so both keys are required here, not just one of them.
    assert.match(head, /\bhead\b(?!:\s*baseRef)/, `the ${phase} checkpoint must carry head`)
    assert.match(head, /\bbaseRef\b/, `the ${phase} checkpoint must carry the diff base under its own key`)
  }
})

// ---- tripwires for what remains out of the engine harness's reach ----
// Same standing as the source-text assertions above: a revert is caught, a rewording is not. Two
// tripwires that used to live here are gone, superseded rather than dropped: the minted-directory
// comparison and the landed-but-degraded SOURCE-TEXT tripwire are now EXECUTED in
// lib/engine-harness.test.mjs,
// which fails against the defect itself instead of against the deletion of a line. What stays here
// is what running the engine cannot see — an escaper compared byte for byte, and a sentinel string
// that must read the same at four producer sites and two matchers.

test("review.js's shq stays byte-identical to the one the gate regenerates elsewhere", () => {
  // review.js's shq sits outside the craft-inline fence (excluded to avoid a duplicate declaration),
  // so check-workflows byte-compares the escaper in the other three engines and is blind here. This
  // is that comparison, done by hand — a hardening that lands on the source must land here too.
  const line = /^function shq\(s\) \{.*$/m
  const mine = src.match(line)
  const source = fs.readFileSync(path.join(root, 'lib', 'run-logging.mjs'), 'utf8').match(/^export function shq\(s\) \{.*$/m)
  assert.ok(mine && source, 'both escapers must still be single-line declarations')
  assert.equal(mine[0], source[0].replace(/^export /, ''), 'review.js shq has drifted from lib/run-logging.mjs')
})

test('a landed record is not counted as one that could not be confirmed', () => {
  // A banner that cries "lost" over a record sitting on disk is a banner readers learn to skip —
  // which costs the runs where it means what it says. Both facts stay in one section; the heading
  // and the count follow whichever is true.
  //
  // This one is NOT superseded by the executed engine tests, and the distinction is the whole reason
  // it survives here: those script ONE condition per run, so they cover lost-only and degraded-only.
  // The third block below is the MIXED axis — a lost record and a refused directory in the same run —
  // and it is the axis where the classifier can invert. Losing it let `unconfirmed` be zeroed
  // whenever any line was landed, so a run whose record was lost rendered a serene
  // "Telemetry incomplete — the record is in the store", with every gate green.
  const degraded = telemetryLostSection(['the run directory (the record itself landed) — refused'])
  assert.match(degraded, /## ⚠️ Telemetry incomplete/)
  assert.ok(!/could not be confirmed/.test(degraded), 'a landed record must not read as unconfirmed')
  assert.match(degraded, /record is in the store/)

  const lost = telemetryLostSection(['the run record — the logger agent returned no result'])
  assert.match(lost, /## ⚠️ Telemetry lost/)
  assert.match(lost, /^1 record write/m, 'a genuinely lost record still says so')

  const both = telemetryLostSection(['the run record — gone', 'the run directory (the record itself landed) — refused'])
  assert.match(both, /## ⚠️ Telemetry lost/, 'a real loss in the mix keeps the louder heading')
  assert.match(both, /^1 record write/m, 'but only the unconfirmed ones are counted as unconfirmed')
  assert.ok(both.includes('- the run directory (the record itself landed) — refused'), 'and both lines are still listed')
})

test('the landed-directory sentinel reads the same at every site that writes or matches it', () => {
  // The prefix is hand-written at four producer sites and matched by two independent regexes, and
  // nothing binds them: reword one producer and its own run — record safely on disk — starts
  // reporting `Telemetry lost`, the false alarm this split exists to remove, with every gate green.
  const SENTINEL = 'the run directory (the record itself landed)'
  const engines = RECORD_FILING_FILES
  for (const e of engines) {
    const text = fs.readFileSync(path.join(root, 'workflows', e), 'utf8')
    assert.ok(text.includes(SENTINEL), `${e} must write the sentinel verbatim`)
  }
  // Both matchers, spelled as regex sources, must contain the same words in the same order.
  const asRegexSource = SENTINEL.replace(/[()]/g, c => `\\${c}`)
  const matchers = [
    fs.readFileSync(path.join(root, 'lib', 'review-coverage.mjs'), 'utf8'),
    fs.readFileSync(path.join(root, 'workflows', 'adversarial-review.js'), 'utf8'),
  ]
  for (const m of matchers) {
    assert.ok(m.includes(asRegexSource), 'a classifier must match the sentinel the producers write')
  }
})
