...
test('every report the engine can return passes through out()', () => {
  ...
  // The normal exit wraps the synthesized report AND appends the dropped-scope section to it: a
  // scope the caller asked for and did not get is not something the synthesis prompt may forget.
  assert.ok(
    /return out\(\(report \|\| fallbackReport\(\)\)( \+ scopeSection\(\))?\)/.test(src),
...
test('a failed write is asserted by the logger, not inferred from silence', () => {
  ...
  assert.ok(
    ckptLine.includes('rejoin: !runDir && checkpointFailed'),
  ...
  assert.ok(
    /logRunPrompt\(\{[\s\S]{0,300}?command: 'finalize', dir: runDir, rejoin: checkpointFailed,/.test(src),
