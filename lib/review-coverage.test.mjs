...
test('every logger command resolves its script path before changing directory', () => {
  ...
  assert.ok(!/:-\./.test(plugin.LOGGER_PRELUDE), 'no cwd fallback may resolve the logger')
  // The PROPERTY, not the spelling: with neither craftRoot nor the env var the prelude may consult
  // the user's own plugin cache, but it must never fall back to the working directory, and it must
  // end in a refusal that stops the command. Pinning the exact former one-liner made a legitimate
  // change to HOW the logger is found look like a regression in WHETHER it is found safely.
  assert.match(plugin.LOGGER_PRELUDE, /CLAUDE_PLUGIN_ROOT/, 'the environment is still consulted first')
  assert.ok(!/(^|[^\w])\.\/lib\/craft-log-run/.test(plugin.LOGGER_PRELUDE), 'and the working directory is never a candidate')
  assert.match(plugin.LOGGER_PRELUDE, /craft-log-run FAILED[\s\S]*exit 1|:\?craft-log-run FAILED/, 'an unresolved logger must stop the command, not continue')
