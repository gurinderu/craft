...
test('every logger command resolves its script path before changing directory', () => {
  ...
  assert.match(pinned.LOGGER_PRELUDE, /case "\$CRAFT_TRY" in \/\*\)/, 'and guarded by the same absoluteness check')
...
test('every checkpoint payload carries the run identity, not just the first', () => {
  ...
  for (const [, phase, head] of payloads) {
    ...
    assert.match(head, /head:/, `the ${phase} checkpoint must carry head`)
  }
})
