...
test('a failed write is asserted by the logger, not inferred from silence', () => {
  ...
  for (const cmd of ['LOGGER_PATH} checkpoint --phase', 'LOGGER_PATH} finalize ']) {
    const i = src.indexOf(`${cmd}`)
    assert.ok(i > 0, `review.js must still emit a \`${cmd}\` command`)
    const line = src.slice(i, src.indexOf('\n', i))
    assert.ok(
      line.includes("!runDir && checkpointFailed ? '--rejoin ' : ''"),
      `the ${cmd.trim()} command must EMIT --rejoin once the rejoin is armed, not merely arm it: ${line}`,
    )
  }
