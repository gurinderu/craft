...
function loadHelpers(workflowArgs = {}) {
  ...
  const factory = new Function(
    ...
    `${prefix}\n;return { PROFILES, supportedLangLabel, resolveProfilePin, unknownPinMessage, noLanguageMessage, nothingToReviewMessage, materialUncovered, coverageGapFiles, isAncillaryConfig, resolveCoverage, uncoveredNotRunNote, noChangedFilesMessage, requestedLangs, LOGGER_PATH, LOGGER_PRELUDE, repoArg };`,
  )
...
test('the coverage outcome is decided from the change set, before any pin can populate active', () => {
  ...
  const rust = H.PROFILES.rust
  const empty = H.resolveCoverage({ changedFiles: [], detectedActive: [], pinnedLangs: ['rust'] })
  ...
  const docs = H.resolveCoverage({ changedFiles: ['README.md', 'Cargo.lock'], detectedActive: [], pinnedLangs: ['rust'] })
  assert.equal(docs.outcome, 'nothing-to-review', 'an inert-only diff is an honest green, pin or no pin')

  const py = H.resolveCoverage({ changedFiles: ['app.py'], detectedActive: [], pinnedLangs: null })
  assert.equal(py.outcome, 'no-profile', 'unreviewable material with no pin is INCOMPLETE')

  const pinnedOverMaterial = H.resolveCoverage({ changedFiles: ['app.py'], detectedActive: [], pinnedLangs: ['rust'] })
  ...
  const detectedRust = H.resolveCoverage({ changedFiles: ['src/lib.rs'], detectedActive: [rust], pinnedLangs: ['rust'] })
...
test('the guard branches consume resolveCoverage and keep their honest verdicts', () => {
  ...
  const resolveIdx = src.indexOf('const coverage = resolveCoverage({ changedFiles, detectedActive, pinnedLangs })')
  ...
  assert.ok(
    src.includes('noLanguageMessage(changedFiles.length, coverage.material.length)'),
...
test('resolveProfilePin dedupes — a repeated pin must not run the pipeline twice', () => {
  ...
  assert.deepEqual(H.resolveProfilePin(['rust', 'Rust']), { pinned: ['rust'], unknown: [] })
  assert.deepEqual(H.resolveProfilePin(['RUST', ' rust ', 'rust']), { pinned: ['rust'], unknown: [] })
  assert.deepEqual(H.resolveProfilePin(['python', 'Python']), { pinned: [], unknown: ['python'] })
  const c = H.resolveCoverage({ changedFiles: ['app.py'], detectedActive: [], pinnedLangs: H.resolveProfilePin(['rust', 'Rust']).pinned })
...
test('project config is reported but does not fire the INCOMPLETE marker; real source still does', () => {
  ...
  for (const f of ['.github/workflows/ci.yml', 'deny.toml', 'rustfmt.toml', 'Dockerfile', 'justfile', 'Makefile', '.editorconfig']) {
    assert.ok(H.isAncillaryConfig(f), `${f} is project config, not reviewed source`)
    assert.deepEqual(H.coverageGapFiles([f]), [], `${f} must not put an otherwise-green verdict into INCOMPLETE`)
    assert.deepEqual(H.materialUncovered([f]), [f], `${f} is still material — it is reported as not reviewed`)
  }
  ...
  for (const f of ['scripts/deploy.sh', 'db/schema.sql', 'api/user.proto', 'app.py', 'main.go', 'ui/app.ts']) {
    assert.ok(!H.isAncillaryConfig(f), `${f} is source, not config`)
    assert.deepEqual(H.coverageGapFiles([f]), [f], `${f} must still fire the marker`)
  }
  assert.deepEqual(H.coverageGapFiles(['README.md', '.github/workflows/ci.yml', 'app.py']), ['app.py'])
})
