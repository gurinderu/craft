...
test('review: a lens answering with a mangled name does not corrupt the ledger or its findings', async () => {
  ...
  // Counted on the run's own findings tally rather than on `findingCount`, which counts CONFIRMED
  // ones — and this fixture's verifier is deliberately dead, so nothing is confirmed. The property
  // under test is attribution, not verification.
