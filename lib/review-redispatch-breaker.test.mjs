...
const findingsOver = (files, source = 'safety') => files.flatMap(f => Array.from({ length: BATCH }, (_u, i) => ({ ...finding(f, 10 + i), source })))
  ...
  const { calls } = await runEngine('review', {
    ...
    script: scriptFor({
      ...
      extra: {
        ...
        scout: ctx => ({
          sizeBucket: 'small',
          lenses: [/^scout:nix$/.test(String(ctx.opts.label)) ? 'purity' : 'safety'],
        ...
        lens: ctx => {
          ...
          rsLens += 1
          return { lens: 'safety', findings: rsLens === 1 ? findingsOver(RS_FILES) : [] }
        },
