...
import { VERIFY_WINDOW_AGENTS, verifyWeight, weightedWindow } from './review-waves.mjs'
...
test('the in-flight budget is set to a value that actually bounds the queue', () => {
  assert.ok(Number.isInteger(VERIFY_WINDOW_AGENTS) && VERIFY_WINDOW_AGENTS > 0)
  ...
  assert.ok(VERIFY_WINDOW_AGENTS >= 4 && VERIFY_WINDOW_AGENTS <= 40, `${VERIFY_WINDOW_AGENTS} keeps dispatch parallel but bounded`)
})
