...
// ---- prior-round transport: short why + whyRef, persisted end to end ----------------------------
// The prior-round transport carries a SHORTENED `why` and a `whyRef` back to the record the FULL one
// lives in. These run the real engine so a carried finding's persisted ledger entry is inspected as
// the engine would file it — a source-text match would catch a deletion, not a defect.
test('review memory: a CARRIED prior keeps its shortened why AND its whyRef through adjudication and the record write', async () => {
  const shortWhy = 'z'.repeat(120)
  const carriedPrior = ledgerRow({
    disposition: 'open', severity: 'Medium', title: 'still broken',
    why: shortWhy, whyRef: { record: '2026-01-01T00-00-00Z-workflow-review.json', fp: 'birthfp' },
  })
  const run = await runEngine('review', {
    args: {},
    script: scriptFor({
      findings: [],
      priorRound: priorWith([carriedPrior]),                 // round 1 → this run is round 2
      adjudicate: { status: 'still-open', note: '', attack: '', currentLine: 50 },
    }),
  })
  const ledger = (filedRecord(run) || {}).ledger || []
  const carried = ledger.find(e => e.disposition === 'open')
  assert.ok(carried, 'the still-open prior is re-persisted as a live row')
  assert.equal(carried.why, shortWhy, 'the carried why stays short — the full one lives in the birth record')
  assert.deepEqual(carried.whyRef, { record: '2026-01-01T00-00-00Z-workflow-review.json', fp: 'birthfp' },
    'the whyRef survives adjudication and the record write, still pointing at the birth record')
  // The transport-integrity guard reads entry COUNT, not field length, so shortening never trips it.
  assert.ok(!run.logs.some(l => /TRUNCATED/.test(l)), 'a shortened why must not be read as a truncated transport')
})

test('review memory: a FRESH finding is persisted with its FULL why and no whyRef (full at birth)', async () => {
  const longWhy = 'q'.repeat(800)
  const run = await runEngine('review', {
    args: {},
    // A first review (no prior round): the finding is unambiguously BORN this round.
    script: scriptFor({
      findings: [finding('High', 51, { ruleId: 'NEW-1', title: 'brand new', why: longWhy })],
    }),
  })
  const ledger = (filedRecord(run) || {}).ledger || []
  const fresh = ledger.find(e => e.ruleId === 'NEW-1')
  assert.ok(fresh, 'the fresh finding reaches the persisted ledger')
  assert.equal(fresh.why, longWhy, 'a fresh finding keeps its FULL why at birth — nothing shortens it in the engine')
  assert.equal('whyRef' in fresh, false, 'a fresh finding carries no whyRef')
})
