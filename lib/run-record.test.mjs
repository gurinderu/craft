...
import {
  ...
  dispositionFromTriage, rereviewVerdict, ENGINE_REVISION, engineKey, engineRevisionToken, isEngineAttributed, GREEN_VERDICT,
} from './run-record.mjs'
...
test('indexProjection does NOT carry engineRevision, and the note saying so is not mirrored', () => {
  ...
  const p = indexProjection({ schemaVersion: 1, ts: 't', kind: 'workflow', name: 'review', project: '/p', verdict: 'Approve', engineRevision: 2 })
...
test('findPriorRound: newest wins, malformed index lines are skipped, mismatches reject', (t) => {
  ...
  assert.match(findPriorRound({ store, project, branch: 'nope' }).reason,
    /index\.jsonl: 4 unparsable line\(s\) skipped/,
...
// sound ONLY within one engine revision (the fp basis is revision-scoped — see ENGINE_REVISION). The
// loader reports whether the prior round is comparable so the workflow can skip the check across a bump
// instead of missing a regression silently.
test('findPriorRound reports sameEngineRevision true only when the prior record matches this engine', (t) => {
  ...
  assert.equal(findPriorRound({ store, project, branch: 'feat/x' }).sameEngineRevision, true)

  // A DIFFERENT (older) revision → not comparable: the fingerprints were computed under another basis.
  detail('2026-07-11T00-00-00Z', { round: 2, head, engineRevision: ENGINE_REVISION - 1, ledger: [{ fp: 'aaaa' }], findings: { total: 1 } })
  fs.writeFileSync(path.join(store, 'index.jsonl'), [row('2026-07-10T00-00-00Z'), row('2026-07-11T00-00-00Z')].join('\n') + '\n')
  assert.equal(findPriorRound({ store, project, branch: 'feat/x' }).sameEngineRevision, false, 'the newest wins, and it is an older revision')
  ...
  assert.equal(findPriorRound({ store, project, branch: 'feat/x' }).sameEngineRevision, false, 'a pre-revision record is not comparable')
})
