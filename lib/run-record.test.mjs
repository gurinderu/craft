...
test('findPriorRound: skips `.` rows, walks past non-ancestor and unreadable candidates, normalizes the ledger', (t) => {
  ...
  assert.deepEqual(findPriorRound({ store: store2, project, branch: 'feat/x' }), PRIOR_ROUND_NONE, 'a `.` row is never attributed to this repo')
})

// The CLI resolves `--project` to an absolute path before calling in, so the end-to-end test above
// can never exercise a relative project. A DIRECT library call can — and that is the path that
// regressed. `project: '.'` must not become a candidate that matches legacy `project: "."` rows.
test('findPriorRound: a direct library call with a relative project never matches a `.` row', (t) => {
  ...
  // Ancestry is checked against path.resolve(project) — here the cwd repo, so use its own HEAD.
  const head = execFileSync('git', ['rev-parse', '--short', 'HEAD'], { encoding: 'utf8' }).trim()
  const branch = 'fix/deterministic-prior-round'
  ...
  assert.deepEqual(
    findPriorRound({ store, project: '.', branch }),
    PRIOR_ROUND_NONE,
