...
// candidate, never to a guess. Well-timed uniqueness is not by itself evidence, though — it is
// satisfied exactly when the stalled run's OWN journal is missing and one neighbour's happens to sit
// in the window (see `branchTies` below), which measured live is the COMMON case for this path, not
// a corner case. So a unique, well-timed candidate is matched only when its journal's own recorded
// `branch` also agrees with the caller's — a positive tie, not the absence of a rival.
...
// Uniqueness alone is NOT evidence: it is satisfied whenever the stalled run's OWN journal is
// missing and exactly one NEIGHBOURING journal happens to sit in the window — which is the measured
// common case (a worktree-run's journal files under a different slug than the repo it reviewed), not
// a corner case. A journal's `base` result entry (`classifyResult` → `'base'` in craft-log-run.mjs)
// carries the reviewed `branch`; a checkpoint carries the same field under the same name (see
// `checkpointIdentity`/`dirIdentity` there). On the live store the two agree exactly on every real
// match. `journalBranch` reads that field straight out of the raw journal lines rather than importing
// craft-log-run.mjs's `classifyResult` — that module already imports `findJournalForRun` from here,
// and a reverse import would be circular. The two definitions of "this is the base entry" (`files` is
// an array and `baseRef` is present) are kept in sync by hand; there is only the one journal shape.
function journalBranch(file) {
  let text
  ...
  for (const line of text.split('\n')) {
    ...
    const r = e && e.type === 'result' ? e.result : null
    if (r && typeof r === 'object' && Array.isArray(r.files) && r.baseRef) {
      return typeof r.branch === 'string' ? r.branch : ''
    }
  }
  return ''
}

// The positive tie itself. `branch` is the CALLING side's own evidence (the stalled run's checkpoint
// branch) — when it is not known, there is nothing to tie against and matching on uniqueness alone is
// exactly the confident-wrongness failure mode this module exists to avoid, so that is refused too,
// not just a mismatch.
function branchTies(candidateFile, branch) {
  if (!branch) return { ok: false, reason: 'no-branch-known' }
  const jBranch = journalBranch(candidateFile)
  if (!jBranch || jBranch !== branch) return { ok: false, reason: 'branch-mismatch' }
  return { ok: true }
}
...
export function findJournalForRun({ project, runStartMs, sessionId = '', branch = '', claudeHome = path.join(os.homedir(), '.claude') } = {}) {
  ...
  if (sessionId) {
    ...
    // Uniqueness within a session is strong evidence but not proof — the session id narrows to
    // "workflow runs in this harness session", which can still hold more than one review of
    // different repos/branches over its lifetime. The branch tie applies here too, same reasoning as
    // the project-slug path below.
    const tie = branchTies(candidates[0].file, branch)
    if (!tie.ok) return { found: false, reason: tie.reason }
  ...
  if (candidates.length > 1) return { found: false, reason: 'ambiguous-journals', count: candidates.length }
  // This is the path that is unique BECAUSE the run's own journal is missing (see the module header
  // and `branchTies` above) — uniqueness here is never accepted alone.
  const tie = branchTies(candidates[0].file, branch)
  if (!tie.ok) return { found: false, reason: tie.reason }
  return { found: true, dir: path.dirname(candidates[0].file), file: candidates[0].file }
}
