...
export function loggerPrelude(craftRoot, version = '') {
...
export function checkpointPrompt({ payload, craftRoot = '', repo = '', phase = '', dir = '', rejoin = false, version = '' } = {}) {
...
\`\`\`
${loggerPrelude(craftRoot, version)}CRAFT_REC="$(mktemp "\${TMPDIR:-/tmp}/craft-ckpt.XXXXXX")"
cat > "$CRAFT_REC" <<'CRAFT_RECORD_EOF'
