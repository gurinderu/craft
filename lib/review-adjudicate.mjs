...
export function partitionAbsorbed(findings, livePriors, retired, fallbackMatch, seed) {
...
export function absorbAcross(lists, livePriors, retired, fallbackMatch) {
  const runs = []
  let updates = new Map()
...
export const TRACKED_MARK = ' — (this site is already tracked by a still-live prior finding; NOT absorbed into it: nothing checked this report against the code, so it may not hold that prior open)'
...
export function markTrackedUnverified(findings, livePriors, retired, fallbackMatch) {
  const isRetired = h => (retired instanceof Set ? retired.has(h) : !!(retired || []).includes(h))
