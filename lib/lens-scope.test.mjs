...
import { sliceDiff, sliceableLens, WHOLE_DIFF_LENSES, pathspecLiteral, decodeGitPath, MAX_SHARED_PER_SLICE } from './lens-scope.mjs'
...
test('lens-scope: a path is handed to git as a literal, not as a pattern', () => {
  ...
  assert.equal(decodeGitPath('"dir/a b.rs"'), 'dir/a b.rs', 'git C-quoting must be undone before handing the name back')
  assert.equal(decodeGitPath('"dir/a\\"b.rs"'), 'dir/a"b.rs')
...
test('lens-scope: git octal escapes are decoded back into the real name', () => {
  ...
  assert.equal(decodeGitPath('"caf\\303\\251.rs"'), 'café.rs')
  assert.equal(decodeGitPath('"\\320\\277\\321\\203\\321\\202\\321\\214.rs"'), 'путь.rs')
  assert.equal(decodeGitPath('"tab\\there.rs"'), 'tab\there.rs')
  assert.equal(decodeGitPath('"nl\\nhere.rs"'), 'nl\nhere.rs')
  ...
  assert.equal(decodeGitPath('"odd\\zhere.rs"'), 'odd\\zhere.rs')
})
...
test('lens-scope: a quoted name with RAW utf-8 survives byte-exactly', () => {
  ...
  assert.equal(decodeGitPath('"a b café.rs"'), 'a b café.rs')
  assert.equal(decodeGitPath('"a b 中.rs"'), 'a b 中.rs')
  // And the two quoting modes must agree on the same file.
  assert.equal(decodeGitPath('"caf\\303\\251.rs"'), decodeGitPath('"café.rs"'))
})
...
test('lens-scope: a malformed escape keeps its backslash instead of becoming another file', () => {
  ...
  assert.equal(decodeGitPath('"a\\30.rs"'), 'a\\30.rs', 'a two-digit tail is not an octal escape')
  assert.equal(decodeGitPath('"dir\\"'), 'dir\\', 'a trailing backslash must not consume past the end')
...
test('lens-scope: nothing ever produces an EMPTY pathspec', () => {
  ...
  assert.notEqual(decodeGitPath('""'), '', 'a pair of quotes decodes to nothing and must fall back')
...
test('lens-scope: an astral character survives a raw-utf-8 quoted name', () => {
  ...
  assert.equal(decodeGitPath('"a b😀.rs"'), 'a b😀.rs')
  assert.equal(decodeGitPath('"x 𝄞.rs"'), 'x 𝄞.rs')
  // And the escaped spelling of the same character must decode to the identical name.
  assert.equal(decodeGitPath('"\\360\\237\\230\\200.rs"'), decodeGitPath('"😀 .rs"').replace(' ', ''))
})

test('lens-scope: a name that is literally quoted is not un-quoted twice', () => {
  // A file called `"q"`. Git returns `"\\"q\\""`, the source decode gives the real name `"q"`, and a
  // second decode would strip the quotes that ARE the name — handing git `:(literal)q`, which
  // reviews nobody's file, or somebody else's if a file `q` exists. The decoder used to fail to
  // fire when it should; this is the same failure inverted.
  assert.equal(decodeGitPath('"\\"q\\""'), '"q"')
  assert.equal(pathspecLiteral('"q"'), ':(literal)"q"', 'pathspecLiteral must not decode — it receives decoded names')
  // Decoding at the source is idempotent for every OTHER shape, which is why this one is the trap.
  for (const n of ['a b.rs', 'café.rs', 'plain.rs', '']) assert.equal(decodeGitPath(decodeGitPath(n)), decodeGitPath(n), n)
})
