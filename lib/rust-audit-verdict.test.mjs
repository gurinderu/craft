...
// ---- a tool that never ran is not clean ----

test('the not-run verdict survives normalisation and never reads as green', () => {
  // The one word every layer has to agree on. normalizeDimensionVerdict() returns any INCOMPLETE
  // verdict UNCHANGED (so it lands in worstVerdict's non-permissive branch); worstVerdict() then
  // reads it as a Warning; lib/analyze-runs.mjs's isIncomplete() buckets it as incomplete rather
  // than as an approve. If any of those three stopped agreeing, an unrun tool would read as clean.
  assert.equal(normalizeDimensionVerdict('INCOMPLETE (not run)'), 'INCOMPLETE (not run)')
  assert.equal(worstVerdict([normalizeDimensionVerdict('INCOMPLETE (not run)')]), 'Warning')
  assert.equal(worstVerdict(['Approve', 'Clean', 'INCOMPLETE (not run)']), 'Warning')
  // and it must not swallow a real finding from another dimension
  assert.equal(worstVerdict(['INCOMPLETE (not run)', 'Block']), 'Block')
})

test('the not-run verdict is in the schema enum the dimensions answer with', () => {
  const m = /verdict: \{[\s\S]*?\n {4}\}/.exec(auditSrc)
  assert.ok(m[0].includes("'INCOMPLETE (not run)'"), 'a dimension must have a way to say it did not run')
})

test('a dimension that could not run marks the whole audit INCOMPLETE', () => {
  // `couldNotRun` (verdict INCOMPLETE) joins `notRun` (no result at all) before auditVerdict, so a
  // missing cargo-semver-checks marks the audit incomplete exactly like a dead agent does.
  assert.equal(auditVerdict('Approve', ['semver']), 'Approve (INCOMPLETE)')
  // ...and a fully healthy audit is NOT marked incomplete — the guard must not fire on green runs.
  assert.equal(auditVerdict('Approve', []), 'Approve')
})

test('the couldNotRun list is anchored, so a partially-covered Block is not filed as "never ran"', () => {
  // `Block (INCOMPLETE)` is a finding that WAS made under partial coverage — it belongs in the red
  // column, not in the "checked nothing" list. Only a verdict that is nothing but INCOMPLETE counts.
  const m = /const couldNotRun = results\.filter\(r => (\/[^/]*\/i)\.test/.exec(auditSrc)
  assert.ok(m, 'expected couldNotRun to filter results by an INCOMPLETE verdict test')
  const re = new RegExp(m[1].slice(1, -2), 'i')
  assert.ok(re.test('INCOMPLETE (not run)'))
  assert.ok(!re.test('Block (INCOMPLETE)'))
  assert.ok(!re.test('Approve'))
})

test('a demoted no-evidence dimension is bucketed apart from tooling-absent', () => {
  // couldNotRun means "the tool was absent, nothing was checked". A green demoted for a missing
  // `Evidence:` line is a different fact — the tool may well have run, the agent just did not show its
  // work — so it must NOT land in couldNotRun (logged and framed as tooling-absent) but in its own
  // noEvidence bucket. Both still feed incompleteDimensions, so the audit is INCOMPLETE either way.
  const cnr = /const couldNotRun = results\.filter\(r => (\/[^/]*\/i)\.test/.exec(auditSrc)
  assert.ok(cnr, 'expected couldNotRun to filter by an INCOMPLETE (not run) test')
  const cnrRe = new RegExp(cnr[1].slice(1, -2), 'i')
  assert.ok(cnrRe.test('INCOMPLETE (not run)'), 'tooling-absent still lands in couldNotRun')
  assert.ok(!cnrRe.test('INCOMPLETE (no evidence — claimed Approve)'), 'a no-evidence demotion does NOT land in couldNotRun')

  const ne = /const noEvidence = results\.filter\(r => (\/[^/]*\/i)\.test/.exec(auditSrc)
  assert.ok(ne, 'expected a noEvidence bucket for the demoted greens')
  const neRe = new RegExp(ne[1].slice(1, -2), 'i')
  assert.ok(neRe.test('INCOMPLETE (no evidence — claimed Approve)'), 'a no-evidence demotion lands in noEvidence')
  assert.ok(!neRe.test('INCOMPLETE (not run)'), 'and tooling-absent does not')

  // incompleteDimensions is a SOFT catch-all over ANY dimension whose verdict leads with INCOMPLETE
  // (plus notRun), so no off-canonical form escapes the accounting — the narrow display buckets above
  // are sub-cases of it, not its whole membership (finding 1).
  const inc = /const incompleteDimensions = \[\.\.\.notRun, \.\.\.results\.filter\(r => (\/[^/]*\/i)\.test\(String\(r\.verdict \|\| ''\)\)\)\.map\(r => r\.dimension\)\]/.exec(auditSrc)
  assert.ok(inc, 'incompleteDimensions must be a soft catch-all over notRun + any INCOMPLETE-leading verdict')
  const incRe = new RegExp(inc[1].slice(1, -2), 'i')
  for (const v of ['INCOMPLETE (not run)', 'INCOMPLETE (no evidence — claimed Approve)', 'INCOMPLETE (tooling absent)', 'INCOMPLETE (not run: cargo-hack absent)', 'INCOMPLETE']) {
    assert.ok(incRe.test(v), `the catch-all counts ${v}`)
  }
  assert.ok(!incRe.test('Block (INCOMPLETE)'), 'but a severity-suffixed verdict is a real finding, not "checked nothing"')
  assert.ok(!incRe.test('Approve'), 'and a clean green is not incomplete')
})

test('the unused-crates zero-candidate branch carries computed evidence for a green', () => {
  // The find prompt has no `Evidence:` clause, so a computed-clean green (no candidates) must be given
  // COMPUTED evidence or demoteUnsupportedGreen would demote an honest clean pass. Pin that the branch
  // injects it, gated on greenness so a non-run (metadata absent → INCOMPLETE) keeps its own words.
  const m = /if \(!candidates\.length\) \{([\s\S]*?)\n {2}\}/.exec(auditSrc)
  assert.ok(m, 'expected the candidates:0 early-return block')
  assert.match(m[1], /GREEN_VERDICT\.test\(String\(found\.verdict/, 'greenness gates the evidence injection')
  assert.match(m[1], /unusedEvidence\(t\)/, 'a green carries the computed tally as its Evidence line')
})

test('every tool dimension is given a way to say it did not run', () => {
  // The defect itself, as a regression guard. Prompt TEXT cannot be unit-tested as behaviour, but
  // its vocabulary can: each dimension that depends on an external tool must offer the agent
  // `INCOMPLETE (not run)` for the absent-tool case. They used to say "return Approve", which
  // printed `semver | Approve` for a check that never happened.
  const prompt = label => {
    const i = auditSrc.indexOf(`label: '${label}'`)
    assert.ok(i > 0, `expected a dimension labelled ${label}`)
    // the prompt is the template literal that opens the tasks.push(() => agent(` … `) above it
    const start = auditSrc.lastIndexOf('tasks.push(', i)
    return auditSrc.slice(start, i)
  }
  for (const label of ['semver', 'build-matrix', 'deps', 'tests-cov', 'unused-crates:find', 'security', 'miri']) {
    assert.match(prompt(label), /INCOMPLETE \(not run\)/, `${label} must be able to report that it did not run`)
  }
  // ...and none of them still carries the old instruction to answer Approve when nothing ran.
  assert.equal(/"Approve" if nothing could run|if nothing runs and the graph is empty, return verdict "Approve"/.test(auditSrc), false)
})
