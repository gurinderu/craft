...
// contract below.
//
// TWO CEILINGS, stated here so neither is read off the header optimistically.
// (1) THE DECLARATION IS ONLY AS GOOD AS ITS COUNTER. The preflight runs on the cheapest model at
// low effort, which is right for reconnaissance and wrong for arithmetic: the count it declares is
// its own tally of what it did, and a miscount looks identical to a lie. The audit is exact about
// what it was given; what it was given is approximate.
// (2) THE BREACH IS CARRIED BY THE LOG AND THE RUN RECORD, NOT BY THE RENDERED REPORT — chosen, not
// overlooked. A probe-budget breach says nothing about the code under review; it is a fact about
// this run's own cost discipline, and its consumer is the later measurement over many records, plus
// the human watching one run in the log. Putting it in the report would address it to the one reader
// who cannot act on it. The cost of the choice is real and belongs here in writing: a breach is
// invisible to anyone who only ever reads a green report.
