...
export function unresolvedSiblings(text, regions, readSourceText = readSource) {
  ...
  for (const r of regions) {
    ...
    for (const name of exported) {
      ...
      const code = r.actual.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/(^|\n)\s*\/\/[^\n]*/g, '$1')
      if (!new RegExp(`(?<![.\\w$])${name}\\s*\\(`).test(code)) continue
      if (new RegExp(`(?:function|const|let|var)\\s+${name}\\b`).test(text)) continue   // declared elsewhere in the workflow
      out.push({ name, source: r.source, line: r.open + 1 })
    }
  }
  return out
}
