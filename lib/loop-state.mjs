...
export function reconcileChain({ complete = null, completeAt = NaN, proven = null, evidence = null } = {}) {
  ...
  if (evidence && (!complete || num(evidence.at) > num(completeAt))) {
    ...
    // found instead of re-discovering it. Falling back to the newest COMPLETE round's ledger is the
    // older behaviour and stays for a directory written before the shards existed.
    //
    ...
    const ledger = sharded || (Array.isArray(complete?.ledger) ? complete.ledger : [])
    return {
      found: true,
      round: num(complete?.round) + 1 || 1,
      head: String(evidence.head || complete?.head || ''),
      ledger,
      ledgerCount: sharded ? Math.max(num(evidence.ledgerTotal), sharded.length) : ledger.length,
