...
test('every engine understands the key=value form its own invocation line advertises', async () => {
  ...
  for (const engine of RECORD_FILING_ENGINES) {
    ...
    // Shared across all four: the string form is UNDERSTOOD, and the repair is said out loud. What
    // each engine then does with `base` is its own business — adversarial-review names the option
    // `diffBase`, triage-findings has no diff to base at all, and rust-audit threads the base its
    // scout resolved rather than the one it was handed. Asserting "base reaches a prompt" for all
    // four would be asserting a wish, which is how two earlier tests in this file went wrong.
    assert.ok(
    ...
    if (engine === 'review') {
      const detect = calls.find(c => c.label === 'detect')
      assert.match(detect.prompt, /v0\.17\.0/, 'and for review the base must reach the agent that resolves the diff')
    }
  }
})
