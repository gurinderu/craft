...
test('an agent that THROWS is a dead agent, not a crashed run', async () => {
  ...
  const { report } = await runEngine('rust-audit', {
    args: {},
    script: {
      security: () => { throw new Error('agent blew up') },
      '*': null,
    },
  })
  assert.ok(report, 'a throwing agent must not take the run down')
  assert.match(report, /INCOMPLETE|not run|did not run/i, 'and the run must not read as complete')
})
