...
test('the logger is never resolved against the reviewed repository', async () => {
  ...
  for (const engine of RECORD_FILING_ENGINES) {
    ...
    assert.ok(
      !/craft-log-run\.mjs"?\s*$/m.test(logger.prompt.split('cd ')[1] ?? ''),
      `${engine}: nothing may resolve the logger after the cd into the reviewed repository`,
    )
    ...
    assert.match(logger.prompt, /craft-log-run FAILED[^\n]*refusing to resolve against the reviewed repository|CLAUDE_PLUGIN_ROOT:\?/,
