...
test('review: an absolute `path` with no `repo` refuses before dispatching a single agent', async () => {
  const { report, calls, logs } = await runEngine('review', { args: { path: OTHER_REPO }, script: ALL_DEAD })
  ...
  assert.ok(
    !calls.some(c => c.label === 'detect' || String(c.label || '').startsWith('scout')),
    'refusing must cost nothing — no detect, no scout, no lenses',
  )
  assert.ok(calls.some(c => c.label === 'log-run'), 'and the refusal must still be recorded in the store')
...
for (const engine of ['rust-audit', 'adversarial-review', 'triage-findings']) {
  test(`${engine}: refuses \`repo\` instead of silently working on the session's own checkout`, async () => {
    ...
    assert.equal(calls.length, 0, 'refusing must cost nothing — not one agent dispatched')
  })
}
