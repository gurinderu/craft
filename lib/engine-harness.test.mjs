...
import {
  runEngine, filedRecord, engineSource,
...
test('review: an absolute `path` is read as the repo instead of a pathspec that matches nothing', async () => {
  // Measured 2026-09-17: dispatched as `path=<another repo>` — the spelling the pins' own argument
  // list invited, because it named `path` and not `repo` — the run reviewed the session's checkout
  // and spent 57 agents / 2.04M tokens having all 14 lenses independently rediscover that the base
  // was unreachable and there were no sources. An absolute pathspec matches nothing, so the failure
  // arrives as an empty diff, which reads like "no changes" rather than like a mis-aimed run.
  const repo = '/Users/someone/projects/other-repo'
  const { calls, logs } = await runEngine('review', { args: { path: repo }, script: ALL_DEAD })
  assert.ok(
    logs.some(l => l.includes(repo) && /ABSOLUTE/.test(l)),
    'the engine must say loudly that it was handed an absolute path where a pathspec belongs',
  )
  const detect = calls.find(c => c.label === 'detect')
  assert.ok(detect, 'the engine must dispatch the base-resolution agent')
  assert.ok(
    !detect.prompt.includes(`-- ${repo}`),
    'and must NOT pass the absolute path to git as a pathspec — that is the empty-diff failure',
  )
  assert.match(
    detect.prompt,
    /WORKING DIRECTORY/,
    'it must instead be treated as the repository to review, so agents cd there',
  )
})

test('review: an absolute `path` alongside an explicit `repo` drops the scope rather than emptying the diff', async () => {
  const repo = '/Users/someone/projects/other-repo'
  const { calls, logs } = await runEngine('review', { args: { repo, path: '/elsewhere/crate' }, script: ALL_DEAD })
  ...
  assert.ok(detect.prompt.includes(repo), 'and the explicitly given repo must still be the target')
})
