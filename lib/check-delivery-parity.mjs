...
// through the parser that actually reads these verdicts on the OpenCode side (`parseVerdict` /
...
// two engines that read these verdicts — every word claimed here is confirmed against `parseVerdict`
// in opencode/plugin/run-record.mjs, but the Claude Code side is also read by a second, independent
// engine, `worstVerdict` in lib/run-record.mjs, and the tripwire's words never run through it. That
// second engine is not merely unchecked — it actively DISAGREES with the one this gate exercises:
// `worstVerdict(['INCOMPLETE (not run)'])` returns `'Warning'`, while `parseVerdict` on the OpenCode
// side reports `'INCOMPLETE (not run)'` for the same input. Nothing proves the ENGINE→TABLE
// direction either: a fifth verdict added to either engine would fall outside this gate entirely,
// with every test here still green. Concretely, `worstVerdict` also matches `Pass` (in
// `Approve|Healthy|Clean|Pass`), a word this OUTCOMES table does not carry at all — a second,
// independent instance of the same gap, not a reason to add `Pass` here. Both are accepted, named
// ceilings; reconciling the two engines' vocabularies is a separate decision, not made here.
