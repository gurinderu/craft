...
// The registry's form gate. Two authoring slips on the extension surface would otherwise degrade
// SILENTLY: a group — or a key's phrase list — written as a bare string is handed out per
// CHARACTER by `for...of`/`Object.entries`, and single-character whole-word regexes read as
// reachable in almost any prose, so the group gates nothing while CI stays green (verified: with
// `{ evidence: 'Evidence of work' }` a one-sided body divergence reported NO problem at all). A
// null/undefined group instead throws a bare TypeError from `Object.entries` that names no entry.
// Both must surface as problems NAMING the group and key, so each entry must be a non-empty
// object whose values are non-empty arrays of non-empty strings.
function requirementsFormProblems(requirements) {
  const problems = []
  for (const [groupName, group] of Object.entries(requirements)) {
    if (typeof group !== 'object' || group === null || Array.isArray(group) || Object.keys(group).length === 0) {
      problems.push(
        `REQUIREMENTS group "${groupName}" is not a non-empty object mapping requirement keys to ` +
          'phrase arrays — the group can gate nothing; fix the registry',
      )
      continue
    }
    for (const [key, phrases] of Object.entries(group)) {
      if (!Array.isArray(phrases) || phrases.length === 0 || phrases.some((w) => typeof w !== 'string' || w === '')) {
        problems.push(
          `REQUIREMENTS group "${groupName}", key "${key}" must list its phrases as a non-empty ` +
            'array of non-empty strings — a bare string would be matched per character and read ' +
            'as reachable in almost any prose',
        )
      }
    }
  }
...
export function checkContentParity(
  ...
  // The registry's own form comes first: a malformed group would either gate nothing or throw
  // mid-loop, so nothing compared through it can be trusted — report the registry and stop.
  const malformed = requirementsFormProblems(requirements)
  ...
  // The mirror sweep for `exemptions`, for the same reason: the group loop below only reaches an
  // (agent, group) cell when the agent is PAIRED and the group is REGISTERED. An entry keyed on a
  // deleted or misspelled agent, on an agent currently unpaired (the loop `continue`s before any
