...
//
// What is measured is the BODY, with the leading YAML frontmatter block stripped first. Measured
// directly: every one of the 8 agent bodies carries the full outcome set on its own — the
// frontmatter strip changes nothing about body coverage. What it changes is what the FRONTMATTER
// ALONE would otherwise mask: the `description:` line alone reaches the full outcome set for 7 of
// the 8 files, and is one outcome short (`bad`, missing from `rust-miri`'s description) for the
// 8th — so measuring the unstripped file would have reported "all clean" almost everywhere even if
// a body had lost an outcome entirely, because the description sitting above it already covers for
// it. That is exactly the failure the frontmatter strip exists to prevent: see the demonstration two
// paragraphs below, where all 7 body occurrences of INCOMPLETE in the real
// opencode/agents/rust-reviewer.md are replaced by APPROVE and the file still reads as fully
// covered — by the description, not the body. The two `description:` lines are therefore now
...
// file. Without this, the `description:` line alone can satisfy nearly every outcome — measured
// across all 8 agent files: the description alone reaches the full outcome set in 7 of 8, missing
// only `bad` in `rust-miri`'s. That near-total coverage is what makes the frontmatter dangerous to
// measure: verified, the real opencode/agents/rust-reviewer.md still reads as fully covered even
// with all 7 body occurrences of INCOMPLETE replaced by APPROVE, because the description line above
// it already carries the words on its own — a body that lost an outcome entirely would still pass.
