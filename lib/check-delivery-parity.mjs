...
export function tableFormProblems(table, { surface, entryNoun, keyNoun, entryRule, leafOk, leafRule }) {
  ...
  for (const [entryName, record] of Object.entries(table)) {
    if (typeof record !== 'object' || record === null || Array.isArray(record) || Object.keys(record).length === 0) {
...
export function checkContentParity(
  ...
  // The configuration's own form comes first, and a malformed table suppresses EVERYTHING below,
  // not only what reads through the broken table — deliberately. Both tables are this checker's
  // own configuration, living in this same file: with either malformed, the sweeps and the
  // pairing walk read data that cannot be trusted, and a report mixing "your configuration is
  // broken" with findings computed around the breakage invites acting on the findings. The run
  // is red either way; the cost is one extra local re-run after fixing the table, and the CLI
  // names the state honestly ("configuration malformed — nothing compared").
  const malformed = configurationFormProblems(requirements, exemptions)
  if (malformed.length) return malformed

  return [
    ...deadUnpairedProblems(unpaired, root, opencode),
    ...deadExemptionProblems(exemptions, root, opencode, requirements),
    ...pairProblems(root, opencode, unpaired, requirements, exemptions),
  ]
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (configurationFormProblems(REQUIREMENTS, REQUIREMENT_EXEMPTIONS).length) {
    // A malformed configuration table means checkContentParity returned before comparing
    // anything — the pairs line would assert a comparison that never ran, the misleading-summary
    // shape this repository names as a defect class.
