...
// The work-evidence requirement (invariant #53): a review agent must SHOW it worked before a
// passing verdict is trusted. One group, one key, one PHRASE — deliberately not a list of synonyms:
// the phrase gate must see only this fixed string, the same one lib/audit-evidence.mjs reads at
// runtime (a tripwire in lib/audit-evidence.test.mjs holds the two identical). Held here in the
// registry so losing the line on one delivery of a pair reds this gate before the merge.
export const EVIDENCE = { field: ['Evidence:'] }
...
// (realm @nick/craft, node #57).
export const REQUIREMENTS = { verdict: OUTCOMES, evidence: EVIDENCE }
...
function stripFrontmatter(text) {
  // The anchored `^---` match is defeated by two invisible authoring slips, and either one silently
  // reopens the false negative this strip exists to prevent — the whole frontmatter, `description:`
  // included, would then be measured as body and mask a lost outcome. `fs.readFileSync(…, 'utf8')`
  // does NOT drop a UTF-8 BOM (Node keeps U+FEFF; verified), and some editors write one; a stray
  // blank line before the block is equally plausible from hand-editing. Both defeat `^---`. So
  // normalize first — drop a leading BOM, then let blank (whitespace-only) lines precede the opening
  // `---` — and only then match. Being MORE willing to strip can only measure less as body, never
  // more, so it cannot manufacture a divergence; it closes the masking hole.
...
// The staleness sweep over `unpaired`, in the directions the pairing walk below cannot reach. That
// walk visits names present in `root`, so it catches a stale excuse only when the port has appeared
// (the paired-now case). Every other direction is decided HERE, because UNPAIRED_BY_DESIGN means
// "root-only ships": the instant the excused name is NOT in `root`, the excuse's premise is false.
// Two such directions, both invisible to the root-keyed walk:
//   • present on NEITHER side — a deleted root file; the excuse now stands for nothing.
//   • present ONLY under opencode/agents/ — the agent moved to OpenCode-only, so "ships only for
//     Claude Code" is now exactly backwards. The pairing walk emits "port of nothing" for the file
//     but never consults the excuse, so its staleness would linger unflagged without this arm.
function deadUnpairedProblems({ unpaired, root, opencode }) {
  const problems = []
  for (const name of Object.keys(unpaired)) {
    if (root.has(name)) continue // a live root-only excuse, or a now-paired one the walk below catches
    if (opencode.has(name)) {
      problems.push(
        `UNPAIRED_BY_DESIGN excuses "${name}" as Claude-Code-only, but it ships only under ` +
          "opencode/agents/ — the excuse's premise is now false; drop the entry",
      )
    } else {
      problems.push(
...
// OpenCode agents with no root counterpart — a port of nothing. Its own concern over the opposite
// map, kept out of the per-root-agent walk below rather than braided into it.
function orphanPortProblems({ root, opencode }) {
  const problems = []
...
// One root agent against its port: it must be ported or excused by name (and the excuse must not
// outlive the port), and once paired, each registered (agent, group) cell goes through
// `compareGroup`. Returns this agent's problem strings, in the fixed order excuse-then-groups.
function agentPairProblems(name, body, { opencode, unpaired, requirements, exemptions }) {
  // `Object.hasOwn`, not a bare `unpaired[name]`: agent names are filenames, so an agent named
  // like an Object.prototype member (`toString`, `constructor`, `hasOwnProperty`) would read the
  // inherited member as a truthy excuse and mask a real root-only divergence as UNPAIRED_BY_DESIGN.
  // Same trap, same guard as deadExemptionProblems above.
  const excuse = Object.hasOwn(unpaired, name) ? unpaired[name] : undefined
  if (!opencode.has(name)) {
    // Root-only: a divergence, unless an explicit excuse says it ships for Claude Code by design.
    return excuse
      ? []
      : [
  ...
  // Same own-property guard on the exemptions lookup, for consistency and to survive a future
  // refactor: the outer `exemptions[name]` is keyed by the same filename-derived name as `excuse`.
  // The inner `?.[groupName]` stays a bare `?.` — `groupName` is a trusted own key of the registry
  // (`Object.entries(requirements)`), and the current second `?.` already dodges the name danger.
  const agentExemptions = Object.hasOwn(exemptions, name) ? exemptions[name] : undefined
  for (const [groupName, group] of Object.entries(requirements)) {
    problems.push(
      ...compareGroup(name, groupName, group, body, opencode.get(name), agentExemptions?.[groupName]),
    )
  }
  return problems
}

// The pairing walk, read as its concerns separate: orphan ports first, then each root agent
// through its own comparison. The braid that once lived inline — two loops plus a third nested in
// one of them — is now three named units, none nested in another.
function pairProblems(cfg) {
  const problems = [...orphanPortProblems(cfg)]
  for (const [name, body] of cfg.root) {
    problems.push(...agentPairProblems(name, body, cfg))
  }
  return problems
}
...
export function readAgents(dir) {
  ...
  let names = []
  try {
    // Sorted, so the walk and every FAIL line the CLI prints are stable across filesystems: POSIX
    // readdir order is implementation-defined (ext4 hashes, other filesystems differ), and this
    // checker's output is diffed and pasted between agents, where a per-machine reordering reads as
    // a spurious change.
