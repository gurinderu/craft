...
// `root` and `opencode` are Maps of agent name → body. Returns a list of problems (empty = clean).
export function checkParity(root, opencode) {
  const problems = []

  // The staleness check on UNPAIRED_BY_DESIGN, in the direction the root-keyed loop below cannot
  ...
  for (const [name, body] of root) {
    const excuse = UNPAIRED_BY_DESIGN[name]
