...
test('agent-deadline: the module never reaches for a clock', async () => {
  ...
  const src = await (await import('node:fs/promises')).readFile(new URL('./agent-deadline.mjs', import.meta.url), 'utf8')
  const code = src.split('\n').filter(l => !l.trim().startsWith('//')).join('\n')
  for (const forbidden of ['Date.now', 'new Date', 'performance.now', 'process.hrtime', 'process.uptime']) {
    assert.ok(!code.includes(forbidden), `${forbidden} is unavailable in the workflow sandbox`)
  }
})
