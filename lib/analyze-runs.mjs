...
export function aggregate(records) {
  ...
  for (const r of recs) {
    ...
    // The four buckets are MUTUALLY EXCLUSIVE. `Approve (INCOMPLETE)` used to land in both
    // `incomplete` and `approve`; now that uncovered files make that suffix common, double-counting
    // would inflate the approve column into a claim of coverage the runs did not have.
    const incomplete = isIncomplete(v)
    if (incomplete) { w.incomplete++; incompleteRuns++ }
    if (/Block|At-risk|UB-found/i.test(v)) w.block++
    else if (/Warning|Concerns/i.test(v)) w.warning++
    else if (incomplete) { /* counted above; an incomplete run is not an approve */ }
