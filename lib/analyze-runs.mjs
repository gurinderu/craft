...
//     downstream are over-firing; their prompt/rubric is the mutation target.
...
export function aggregate(records) {
  ...
  for (const r of recs) {
    ...
    for (const dim of (Array.isArray(r.dimensions) ? r.dimensions : [])) {
      ...
      const agg = byDimension[k] || (byDimension[k] = { runs: 0, findings: 0, bySeverity: { Critical: 0, High: 0, Medium: 0, Low: 0, Info: 0 } })
      agg.runs++
