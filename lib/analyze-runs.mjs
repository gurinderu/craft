...
// no engineRevision. Same stance as engineKey — an absent revision is its own bucket, never folded
// into a known one. (realm @nick/craft, #95)
const revToken = r => (r && Number.isInteger(r.engineRevision) ? `r${r.engineRevision}` : 'r?')
...
export function aggregate(records) {
  ...
  // The colliding versions only — a craftVersion carried by more than one distinct revision. A
  // reader slicing by that version blends two different engines; the report and the `--version`
  // filter both surface this so the trap is visible. (realm @nick/craft, #95)
  const versionCollisions = Object.entries(revsByVersion)
