...
export function aggregate(records) {
  ...
  const dimensions = Object.entries(byDimension).map(([dimension, d]) => {
    ...
    return {
      ...
      // `mixed`: both, and the rate is not a like-for-like comparison at all. The report prints this
      // whenever it is not `strict`, because no reader can recover it from the records.
      refuteBasis: d.tracked && d.legacy ? 'mixed' : d.tracked ? 'strict' : 'legacy',
