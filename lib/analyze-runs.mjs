...
export function aggregate(records) {
  ...
  for (const r of recs) {
    ...
    const w = byWorkflow[name] || (byWorkflow[name] = {
      ...
      // `triage-findings` write `outputTokens` with none, and `review` itself gained `round` after
      // `outputTokens`, so a band of its re-reviews carries a larger pool with no round. Reading an
      // absent round as first-pass folds those structurally-larger totals into the first-pass average
      // unwarned (the same "NOT MEASURED IS NOT ZERO" defect the thinned clause guards). So a
      // round-less run goes to its OWN `Unclassified` pool — never invented as first-pass or
      // re-review, the `savedByFloorPremiseHeld === false` stance, one field over.
    ...
    // Three-state by round PRESENCE, then value. A number > 1 → re-review; a number <= 1 (0 or 1) →
    // first-pass; NOT a number (absent) → Unclassified, never silently first-pass. The discriminator
    // is `typeof r.round === 'number'`, not `r.round > 1`: `undefined > 1` is false and would sweep
    // every round-less run into first-pass, which is the defect this replaces.
...
export function renderReport(a) {
  ...
  for (const w of a.workflows) {
    // The run-token pool clause, ADAPTIVE by whether this workflow stamps `round` at all. A bucket
    // holding ANY round-bearing run (only `review` does) prints its first-pass and re-review pools
    // apart (each only when its own N>0), and — if some of its runs predate the `round` field —
    // names those round-less runs as counted APART from the split, never folded into first-pass (the
    // `refute basis LEGACY` discipline: an absent discriminator is a caveat, not a zero). A bucket
    // that never stamps `round` (adversarial-review, rust-audit, triage-findings) has only round-less
    // runs; labelling their pool `(first-pass)` would imply a `(re-review)` counterpart that can
    // never exist, so it gets ONE plain pool with no run-type label.
    const hasRoundBearing = w.avgRunTokenPoolFirstPass != null || w.avgRunTokenPoolReReview != null
    let poolClause = ''
    if (hasRoundBearing) {
      if (w.avgRunTokenPoolFirstPass != null) poolClause += ` · ~${w.avgRunTokenPoolFirstPass} tok-pool/run (first-pass)`
      if (w.avgRunTokenPoolReReview != null) poolClause += ` · ~${w.avgRunTokenPoolReReview} tok-pool/run (re-review)`
      if (w.poolUnclassifiedN) poolClause += ` · ⚠️ ${w.poolUnclassifiedN} run(s) predate the \`round\` field — counted apart from the split, not folded into first-pass`
    } else if (w.avgRunTokenPoolUnclassified != null) {
      poolClause += ` · ~${w.avgRunTokenPoolUnclassified} tok-pool/run`
    }
