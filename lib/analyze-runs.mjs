...
// over:
//   - no `surfaceGate` object at all -> PRE-GATE. The field did not exist when this run was
//     recorded (or this engine never writes it), so it must not be read as "the gate ran and saved
//     nothing" — that would invent a measurement the run never made. Counted in `sgPreGate`, never
//     folded into the saved tally below.
...
export function aggregate(records) {
  ...
  for (const r of recs) {
    ...
    const w = byWorkflow[name] || (byWorkflow[name] = {
      ...
      sgPreGate: 0, sgGateRuns: 0, sgSavedRuns: 0, sgSavedPasses: 0, sgSavedByLens: {},
    })
    w.runs++
  ...
  const workflows = Object.entries(byWorkflow).map(([name, w]) => ({
    ...
    sgPreGate: w.sgPreGate, sgGateRuns: w.sgGateRuns, sgSavedRuns: w.sgSavedRuns,
...
//
// The SHARE (saved / (saved + gateable lenses that actually ran)) is deliberately NOT computed here.
// It looked reachable from `dimensions` — a dimension named `<lang>:negative-space` etc. only exists
// when that lens survived the gate (workflows/review.js builds `dimensions` from the POST-gate
// `plan.lenses`) — but workflows/review.js also has a path where a mechanical gate failure in ONE
// profile discards `dimensions: []` for the WHOLE run (the top-level `if (gateFailed.length)` branch)
// while `surfaceGate.dropped` is computed from module-level state and survives that branch untouched.
// A run shaped that way can have a lens that genuinely ran in a passing profile with no trace of it
// left in `dimensions` — undercounting "ran" and inflating any share computed from it. That is a real
// gap in what the record carries, not a hypothetical one, so the honest move is to report the raw
// counts and name the gap rather than print a number that can be quietly wrong.
function renderSurfaceGateSection(a) {
  ...
  for (const w of withGate) {
    const preGateNote = w.sgPreGate ? ` (${w.sgPreGate} run(s) predate the surface gate, excluded here)` : ''
    ...
    L.push(`- ${w.name}: ${w.sgGateRuns} run(s) carry the gate${preGateNote} · ${w.sgSavedRuns} run(s) saved ≥1 pass`
      + ` · ${w.sgSavedPasses} whole-repo pass(es) saved total${byLens ? ` (${byLens})` : ''}`)
  }
  L.push('_Share (saved / (saved + gateable lenses that actually ran)) is not reported: a run whose_',
    '_mechanical gate fails in one profile discards `dimensions` for the WHOLE run (workflows/review.js),_',
    '_which can hide a lens that ran in another, passing profile — so "ran" cannot be counted reliably_',
    '_from the record. Needs a dispatched-count field that survives a gate failure; a follow-up, not a_',
    '_number invented here._')
  return L
}
