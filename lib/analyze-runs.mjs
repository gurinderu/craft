...
function tallySurfaceGate(w, r) {
  ...
  // The merged `gate.status` is worst-of across profiles, so on a mixed run a passing profile can
  // drop a surface-gated lens with nothing left to dispatch (dispatched empty) while a failing profile
  // sets the whole record's status to fail — a REAL saving that gate.status + dispatched cannot tell
  // apart from a run where every profile aborted at the gate. `surfaceGate.lensesRan` (realm
  // @nick/craft #103) is the third condition that separates them: true when at least one profile
  // completed its lens phase this run. Exclude ONLY the conjunction: gate failed AND nothing dispatched
  // AND no profile ran lenses. Backward-compat: a record written before this field existed has no
  // `lensesRan`, so `sg.lensesRan !== true` is true and the run is still excluded exactly as before —
  // historical counts do not move.
  if (r.gate && r.gate.status === 'fail' && dispatched && dispatched.length === 0 && sg.lensesRan !== true) { w.sgGateFailedRuns++; return }
...
function renderSurfaceGateSection(a) {
  ...
  for (const w of withGate) {
    ...
    const gateFailedNote = w.sgGateFailedRuns
      ? ` (${w.sgGateFailedRuns} run(s) excluded: gate failed with no surface-gated lens dispatched)`
      : ''
