...
export function aggregate(records) {
  ...
  for (const r of recs) {
    ...
    if (ver && typeof ver === 'object') {
      ...
      if (typeof ver.thinned === 'number' && ver.thinned > 0) { w.thinned += ver.thinned; w.thinnedRuns++ }
...
export function renderReport(a) {
  ...
  for (const w of a.workflows) {
    L.push(`- ${w.name}: ${w.runs} run(s) · B/W/A ${w.verdicts.block}/${w.verdicts.warning}/${w.verdicts.approve}`
      ...
      + `${w.thinned ? ` · ⚠️ ${w.thinned} verdict(s) on a THINNED panel across ${w.thinnedRuns} run(s) — votes answered off-schema and were discarded before the arithmetic` : ''}`
