...
import { containsPath, insideStore } from './craft-log-run.mjs'
...
//
// One input is deliberately absent from the table and named here instead: a candidate that does not
// EXIST. The shell refuses it at `[ -f ]` — scored "contained" by the reader below — while the JS
// side answers lexically, so the two genuinely disagree there. Every row therefore builds a real
// file. Saying so is the honest form; a row asserting agreement that does not hold would not be.
const CASES = [
  {
...
test('what is handed to node is the path that was CHECKED, not the one that was named', () => {
  // The predicate validates the fully resolved path, so that is what must be used. Assigning the
  // unresolved candidate instead leaves a window spanning the mktemp, the whole heredoc of a record
  // that can be hundreds of kilobytes, and the cd — long enough for a symlink component to be
  // re-pointed into the reviewed repository between the check and the use. The guard existed for a
  // round with no test: reverting it left all 476 tests green, which is the shape this branch keeps
  // meeting — a fix whose own correctness nothing observes.
  const root = scratch()
  try {
    const real = path.join(root, 'real')
    fs.mkdirSync(real, { recursive: true })
    fs.writeFileSync(path.join(real, 'logger.mjs'), 'console.log("RAN")\n')
    const plugin = path.join(root, 'plugin')
    fs.mkdirSync(path.join(plugin, 'lib'), { recursive: true })
    fs.symlinkSync(path.join(real, 'logger.mjs'), path.join(plugin, 'lib', 'craft-log-run.mjs'))
    const repo = path.join(root, 'repo')
    fs.mkdirSync(repo, { recursive: true })

    const out = execFileSync('bash', ['-c', `${loggerPrelude(plugin, '1.0.0', repo)}echo "USED=$CRAFT_LOGGER"`], {
      encoding: 'utf8', env: { PATH: process.env.PATH, HOME: '/tmp/nonexistent-home' },
    })
    assert.equal(
      out.trim(), `USED=${path.join(real, 'logger.mjs')}`,
      'the resolved target must be handed over, not the symlink that was merely validated',
    )
  } finally {
    fs.rmSync(root, { recursive: true, force: true })
  }
...
test('a path that cannot be resolved gets no verdict', () => {
  // `resolveAsFarAsItExists` walks past a path that does not exist yet, which is ordinary for a run
  // directory about to be created. It used to walk past EVERY error, so a symlink loop degraded into
  // a lexical answer and `insideStore` called a path "inside the store" that resolves nowhere near
  // it — the same fail-open as a bound that keeps going when it runs out.
  const store = scratch()
  try {
    fs.mkdirSync(path.join(store, '.partial'), { recursive: true })
    fs.symlinkSync(path.join(store, '.partial', 'a'), path.join(store, '.partial', 'b'))
    fs.symlinkSync(path.join(store, '.partial', 'b'), path.join(store, '.partial', 'a'))
    assert.equal(
      insideStore(path.join(store, '.partial', 'a', 'run'), store), false,
      'a path whose resolution loops must not be reported as inside anything',
    )
    // And the ordinary not-yet-created case still resolves, or the fix would have closed the door on
    // the guard's actual purpose.
    assert.equal(insideStore(path.join(store, '.partial', 'fresh-run'), store), true)
  } finally {
