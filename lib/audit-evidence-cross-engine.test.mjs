...
test('both audit engines tolerate case and a stray space before the colon IDENTICALLY', () => {
  // The case/whitespace fix is the mirror of the decoration fix on a second axis, and — like the
  // decoration fix — it must land on BOTH engine copies together. RED before the fix on both sides:
  // 'EVIDENCE:', 'evidence:' and 'Evidence :' each failed the exact-case startsWith.
  for (const line of ['EVIDENCE: ran x', 'evidence: ran x', 'Evidence : ran x', '**EVIDENCE:** ran x']) {
    assert.equal(hasEvidenceCC(line), true, `Claude engine reads a case/space marker: ${line}`)
    assert.equal(hasEvidenceOC(line), true, `opencode engine reads a case/space marker: ${line}`)
  }
  // Not weakened on the other axis: buried and decorated-but-empty stay false on both, in any case.
  for (const line of ['the Evidence: is buried', '**Evidence:**', '**EVIDENCE:**']) {
    assert.equal(hasEvidenceCC(line), false, `Claude engine rejects: ${line}`)
    assert.equal(hasEvidenceOC(line), false, `opencode engine rejects: ${line}`)
  }
})
