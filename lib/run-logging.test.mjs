...
// engine's finalize call is `logRunPrompt({ …, command: 'finalize', dir: runDir, rejoin:
// checkpointFailed })`, and `checkpointFailed` is exactly "one of this run's checkpoints failed, so
// its runDir may have been adopted". Reconstructed here from the engine source so a change to that
// call site — or to the builder underneath it — cannot quietly stop transmitting the discriminator.
test('the finalize command review.js builds carries the flags the ownership modes stand on', () => {
  ...
  assert.match(engine, /command: 'finalize', dir: runDir, rejoin: checkpointFailed/,
    'review.js must still hand finalize both its runDir and whether a checkpoint failed')
  assert.match(engine, /dir: runDir, rejoin: !runDir && checkpointFailed/,
    'and its checkpoints must still say which of the two they are')
