...
import { LOGRUN_SCHEMA, shq, loggerPrelude, logRunPrompt, logRunDispatch, logRunOutcome, quietly } from './run-logging.mjs'
...
// Run the prompt's own command the way the logger agent would: record on stdin, into a temp store.
function runPrompt(prompt, { repo, store, record }) {
  const cmd = loggerCommand(prompt)
    .replace('< /tmp/craft-rec.json', `--store ${shq(store)} < ${shq(path.join(store, 'in.json'))}`)
  fs.mkdirSync(store, { recursive: true })
  fs.writeFileSync(path.join(store, 'in.json'), JSON.stringify(record))
  const script = `${loggerPrelude(ROOT)}${cmd}`
...
const RECORD_FILING = ['review.js', 'adversarial-review.js', 'rust-audit.js', 'triage-findings.js']

test('every record-filing engine is still visible to the CRAFT_VERSION gate', () => {
  // workflow-version.mjs derives its set from a `logRun(` match. Renaming or wrapping logRun in any
  // engine would drop it from that gate silently — this is what makes that impossible to miss.
