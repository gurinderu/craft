...
test('loggerPrelude resolves the logger BEFORE any cd', () => {
  ...
  assert.match(explicit, /craft_usable "\$CRAFT_TRY"/, 'and it goes through the same predicate as every other source')
  ...
  // Anchored on the command line that actually enters the repo, not on the first `cd ` in the text:
  // the containment preamble contains `$(cd <repo> && pwd -P)` in a subshell, which is a different
  // `cd` and comes first. Splitting on the first one tested the wrong thing.
  const p = logRunPrompt({ record: {}, craftRoot: '/a/craft', repo: '/b/repo' })
  const cdLine = p.split('\n').find(l => /^cd .*&& node /.test(l)) || ''
  assert.ok(cdLine, 'expected the cd-into-repo command line')
  assert.ok(p.indexOf('CRAFT_LOGGER=') < p.indexOf(cdLine), 'the logger must be resolved before the command enters the repo')
})

test('finalize mode is available for the engine that checkpoints, write for the rest', () => {
  assert.match(loggerCommand(logRunPrompt({ record: {}, command: 'finalize', dir: '/d/run' })), /finalize --dir '\/d\/run' \$\{CLAUDE_CODE_SESSION_ID:\+--session "\$CLAUDE_CODE_SESSION_ID"\} --project/)
  assert.match(loggerCommand(logRunPrompt({ record: {}, command: 'finalize', rejoin: true })), /finalize --rejoin \$\{CLAUDE_CODE_SESSION_ID:\+--session "\$CLAUDE_CODE_SESSION_ID"\} --project/)
  assert.match(loggerCommand(logRunPrompt({ record: {} })), /"\$CRAFT_LOGGER" write \$\{CLAUDE_CODE_SESSION_ID:\+--session "\$CLAUDE_CODE_SESSION_ID"\} --project/)
})

// The literal-string assertions above (`/\$\{CLAUDE_CODE_SESSION_ID:\+--session "\$CLAUDE_CODE_SESSION_ID"\}/`)
// catch a DELETION of the fragment, not a defect in it: changing `:+` to `:-` would regress the
// empty-string case (an EMPTY session id would then produce `--session ""` instead of no flag at
// all) and still match that regex. This runs the actual fragment through a real shell and checks
// the four cases the property is supposed to hold for, including a hostile value.
test('the shell-expanded session flag actually behaves correctly when executed, not just when pattern-matched', () => {
  const FRAGMENT = '${CLAUDE_CODE_SESSION_ID:+--session "$CLAUDE_CODE_SESSION_ID"}'
  // Prints one argv entry per line via a marker so we can tell "zero args" from "one empty-string arg".
  const script = `f() { for a in "$@"; do echo "ARG:$a"; done; }\nf ${FRAGMENT}`
  const argvOf = (env) => {
  ...
  // unset -> no flag at all
  assert.deepEqual(argvOf({ ...baseEnv }), [], 'an unset session id must expand to nothing')

  // empty -> no flag at all (this is exactly what :+ (not :-) is for, and what a :- typo would break)
  assert.deepEqual(argvOf({ ...baseEnv, CLAUDE_CODE_SESSION_ID: '' }), [], 'an empty session id must expand to nothing')

  // set -> exactly two argv entries: the flag and the value, as separate words
  assert.deepEqual(argvOf({ ...baseEnv, CLAUDE_CODE_SESSION_ID: 'abc-123' }), ['--session', 'abc-123'],
    'a set session id must expand to exactly two argv entries')

  // hostile value containing a command separator -> still exactly ONE value argument, never executed
  const hostile = 'abc; rm -rf /; echo pwned'
  assert.deepEqual(argvOf({ ...baseEnv, CLAUDE_CODE_SESSION_ID: hostile }), ['--session', hostile],
    'a hostile session id must arrive as one literal argv entry, not be interpreted by the shell')
})
...
test('the checkpoint prompt is the same builder, and emits what it is asked to', () => {
  ...
  assert.ok(!/--dir /.test(armed), 'with no directory to name, only the rejoin is asked for')
  ...
  assert.match(pinned, /--rejoin /, 'a named directory does not cancel the rejoin — see THE FLAGS ARE INDEPENDENT below')
...
test('with no craftRoot and no CLAUDE_PLUGIN_ROOT the block fails loudly and stages nothing', () => {
  ...
  // realpath: the prelude now hands over the RESOLVED path, and on macOS /var is a symlink to
  // /private/var — comparing a resolved actual against an unresolved expected reports a difference
  // the code did not make.
  const home = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'craft-home-')))
...
test('a session with CLAUDE_CONFIG_DIR set is searched where its plugins actually live', () => {
  ...
  const cfg = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'craft-cfg-')))
...
test('EVERY way of naming the logger is refused when it is not absolute', () => {
  ...
  // there after the cd.
  // The `repo` passed to the prelude is DELIBERATELY not the cwd, and that is what makes this test
  // mean what it says. With `repo` omitted it defaults to `.`, so containment resolves it to the
  // cwd — the planted directory — and every refusal below comes from the containment check instead
  // of the absoluteness one. Deleting the absoluteness guard entirely then left this test, and the
  // whole suite, green: the branch's only assertion against defect #45's own class could not see it.
  // Here the relative candidates land in `cwd`, which is a DIFFERENT directory from the declared
  // repo, so containment has nothing to say and only absoluteness can refuse them.
  const cwd = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'craft-cwd-')))
  fs.mkdirSync(path.join(cwd, 'lib'), { recursive: true })
  fs.writeFileSync(path.join(cwd, 'lib', 'craft-log-run.mjs'), 'console.log("PWNED")\n')
  fs.mkdirSync(path.join(cwd, 'evil', 'plugins', 'cache', 'craft', 'craft', '1.0.0', 'lib'), { recursive: true })
  fs.writeFileSync(path.join(cwd, 'evil', 'plugins', 'cache', 'craft', 'craft', '1.0.0', 'lib', 'craft-log-run.mjs'), 'console.log("PWNED")\n')
  const declaredRepo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'craft-declared-')))
  const cases = [
    ['explicit craftRoot', loggerPrelude('.', '1.0.0', declaredRepo), {}],
    ['explicit craftRoot climbing out', loggerPrelude('..', '1.0.0', declaredRepo), {}],
    ['relative CLAUDE_PLUGIN_ROOT', loggerPrelude('', '1.0.0', declaredRepo), { CLAUDE_PLUGIN_ROOT: '.' }],
    ['relative CLAUDE_CONFIG_DIR', loggerPrelude('', '1.0.0', declaredRepo), { CLAUDE_CONFIG_DIR: 'evil' }],
  ]
  for (const [what, prelude, extraEnv] of cases) {
    ...
    assert.ok(refused, `${what}: a non-absolute path must be refused, never resolved in the working directory`)
    assert.ok(!out.split('\n').some(l => l.trim() === 'PWNED'), `${what}: and nothing in the cwd may execute`)
  }
  fs.rmSync(cwd, { recursive: true, force: true })
  fs.rmSync(declaredRepo, { recursive: true, force: true })
})
...
test('a logger that resolves INSIDE the reviewed repository is refused, however it got there', () => {
  // Absolute is not enough. `craftRoot` arrives in the model-composed args string, so an absolute
  // path naming the repository under review was accepted and executed — the same class as a
  // model-supplied `--dir`, which this project already constrains. Three routes to the same place,
  // all executed: the repo itself, a `..` climb back into it, and a symlink pointing at it. `pwd -P`
  // on both sides is what makes the last two indistinguishable from the first.
  const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'craft-evil-')))
  fs.mkdirSync(path.join(repo, 'lib'), { recursive: true })
  fs.writeFileSync(path.join(repo, 'lib', 'craft-log-run.mjs'), 'console.log("PWNED")\n')
  const link = path.join(os.tmpdir(), `craft-link-${process.pid}`)
  fs.rmSync(link, { force: true })
  fs.symlinkSync(repo, link)
  for (const [what, root] of [['the repo itself', repo], ['a climb back into it', path.join(repo, 'lib', '..')], ['a symlink to it', link]]) {
    let out = ''
    let refused = false
    try {
      out = execFileSync('bash', ['-c', `${loggerPrelude(root, '1.0.0', repo)}node "$CRAFT_LOGGER"`], {
        encoding: 'utf8', cwd: repo, env: { PATH: process.env.PATH, HOME: '/tmp/nonexistent-home' },
      })
    } catch (e) {
      refused = true
      out = String(e.stdout || '') + String(e.stderr || '')
    }
    assert.ok(refused, `${what}: must refuse`)
    assert.ok(!out.split('\n').some(l => l.trim() === 'PWNED'), `${what}: the reviewed repo's script must never run`)
    assert.match(out, /no usable logger|refusing to resolve against the reviewed repository/, `${what}: and say why`)
  }
  fs.rmSync(link, { force: true })
...
test('the record builder derives its version from the record, not from a caller who may forget', () => {
  // The twin of the checkpoint assertion below, and it was the half left open: every loggerPrelude
  // test passes a literal version, so nothing observed the one wiring that takes it off the record.
  // Replacing that derivation with '' left all 478 tests and every gate green while the plugin-cache
  // search silently stopped being emitted for `write` and `finalize` in all four engines — defect
  // #45 returning by the front door of the branch that exists to close it.
  const withVersion = logRunPrompt({ record: { schemaVersion: 1, kind: 'workflow', name: 'review', craftVersion: '5.5.5' } })
  assert.match(withVersion, /plugins\/cache\/craft/, 'a record carrying a version yields the search block')
  assert.match(withVersion, /5\.5\.5/, 'and searches for that version')
  const without = logRunPrompt({ record: { schemaVersion: 1, kind: 'workflow', name: 'review' } })
...
// The whole own-directory/found-directory distinction in `finalizeRun` stands on `--rejoin` arriving
// at the CLI, and for the life of this builder it could not arrive together with `--dir`: the
// expression was `${!dir && rejoin ? '--rejoin ' : ''}`, while review.js finalizes with
// `{ dir: runDir, rejoin: checkpointFailed }`. So on every run that had a runDir at all — that is,
// every run whose checkpoints worked at least once, including one whose runDir was ADOPTED by an
// earlier `--rejoin` — the CLI read `rejoin: false`, `ownDir` was true, and ownership collapsed to
// "the same repository". Nothing failed; the guard simply was not there. These tests exist so a
// flag the guard depends on can never again be dropped silently between the engine and the CLI.
test('logRunPrompt sends --dir and --rejoin together when the engine asks for both', () => {
  const both = logRunPrompt({ record: { kind: 'workflow', name: 'review' }, command: 'finalize', dir: '/s/.partial/x', rejoin: true })
  assert.match(both, /finalize --dir '\/s\/\.partial\/x' --rejoin /, 'the rejoin must survive a named directory')
  const dirOnly = logRunPrompt({ record: { kind: 'workflow', name: 'review' }, command: 'finalize', dir: '/s/.partial/x', rejoin: false })
  assert.ok(!/--rejoin/.test(dirOnly), 'and must not appear when the engine did not ask for it')
  const rejoinOnly = logRunPrompt({ record: { kind: 'workflow', name: 'review' }, command: 'finalize', rejoin: true })
  assert.match(rejoinOnly, /finalize --rejoin /)
})

// The property above is about a helper; THIS is about the command the engine actually builds. The
// engine's finalize call is `logRunPrompt({ …, command: 'finalize', dir: runDir, rejoin: rejoinArmed
// })`, and `rejoinArmed` is exactly "this run ASKED the logger to rejoin, so its runDir may have been
// adopted". It used to be `checkpointFailed`, which is a strictly coarser thing — a checkpoint died
// at all — and a run that minted its own directory first and lost a LATE checkpoint then declared an
// adoption it never made, spending the `ownDir` shortcut that keeps a mid-review branch move from
// costing the run its phases. The behaviour of both spellings is executed in lib/rejoin-arming.test.mjs;
// reconstructed here from the engine source so a change to that call site — or to the builder
// underneath it — cannot quietly stop transmitting the discriminator.
test('the finalize command review.js builds carries the flags the ownership modes stand on', () => {
  const engine = fs.readFileSync(new URL('../workflows/review.js', import.meta.url), 'utf8')
  assert.match(engine, /command: 'finalize', dir: runDir, rejoin: rejoinArmed/,
    'review.js must still hand finalize both its runDir and whether it ever asked to rejoin')
  assert.match(engine, /const armRejoin = !runDir && checkpointFailed/,
    'and its checkpoints must still arm on the state where adoption is actually possible')
  assert.match(engine, /dir: runDir, rejoin: armRejoin/,
    'emitting that armed state, not merely computing it')
  // A run whose first checkpoint died, rejoined into some directory, and then finalizes: the CLI has
  // to see BOTH flags, or finalizeRun treats an adopted directory as this run's own and deletes it.
  const adopted = logRunPrompt({
    record: { kind: 'workflow', name: 'review', craftVersion: '9.9.9' },
    craftRoot: '/opt/craft', repo: '/repos/mine',
    command: 'finalize', dir: '/s/.partial/2026-01-01T00-00-00Z-workflow-review', rejoin: true,
  })
  const line = adopted.split('\n').find(l => l.includes('$CRAFT_LOGGER'))
  assert.match(line, /finalize --dir '[^']+' --rejoin \$\{CLAUDE_CODE_SESSION_ID/,
    'both flags, in the one line the logger agent actually runs')
})
