...
test('writeCostRecordAtomically leaves no temp behind when the rename fails', () => {
  ...
  assert.throws(() => writeCostRecordAtomically(target, { cost: { total: 1 } }), /EISDIR|directory/i)
  const leaked = fs.readdirSync(dir).filter(n => /\.cost-.*\.tmp$/.test(n))
  assert.deepEqual(leaked, [], 'a failed atomic write must leave no .cost-*.tmp behind')
})
