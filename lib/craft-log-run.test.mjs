...
test('finalize refuses a --dir that is not inside the store', () => {
  const store = tmpStore()
  const outsider = fs.mkdtempSync(path.join(os.tmpdir(), 'craft-not-a-run-'))
  fs.writeFileSync(path.join(outsider, 'keep.txt'), 'precious')
  assert.throws(
    () => finalizeRun({ schemaVersion: 1, kind: 'workflow', name: 'review', verdict: 'Approve' }, { store, dir: outsider }),
    /not inside/,
  )
  assert.ok(fs.existsSync(path.join(outsider, 'keep.txt')), 'and it must still be there')
  fs.rmSync(outsider, { recursive: true, force: true })

  // The store root itself is not a run directory either.
  assert.throws(() => finalizeRun({ schemaVersion: 1, kind: 'workflow', name: 'review' }, { store, dir: path.join(store, '.partial') }), /not inside/)
  // A real run directory still works.
  ...
  assert.equal(finalizeRun({ schemaVersion: 1, kind: 'workflow', name: 'review' }, { store, project: '/repos/mine', dir: real }).folded, 1)
})
