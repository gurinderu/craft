...
import {
  ...
  gitIdentity, applyGitIdentity, sumTranscriptUsage, enrichCost, writeCostRecordAtomically,
  writeFileAtomically, WHY_TRANSPORT_MAX,
} from './craft-log-run.mjs'
import { ENGINE_REVISION } from './run-record.mjs'
import { slugifyProjectPath } from './journal-link.mjs'
import { logRunPrompt } from './run-logging.mjs'
...
test('checkpoints survive a run that never finalizes, and recover turns them into a partial record', () => {
  const store = tmpStore()
  const dir = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: store, now: new Date('2026-08-02T14:26:15Z') })
  // `round` rides on the plan checkpoint, as workflows/review.js stamps it: a recovered record with
  // no round would be indexed as round 0 (no round at all), so `recover` KEEPS such a directory as
  // the chain's last witness instead of consuming it — the case its own test below covers. The
  // ordinary path, and the one asserted here, is a directory whose round IS readable.
  writeCheckpoint(dir, 'rust-plan', { branch: 'pr-1203-review', head: 'dd95346d', round: 3, gate: { status: 'pass' } })
  ...
  assert.equal(rec.round, 3, 'and the round it stood at, so the repair does not reset the chain')
...
// WHAT THIS ONE ACTUALLY PINS, said plainly because the paragraph above overstated it: `--dir`
// without `--rejoin` is the OWN-directory mode, whose check is `attested.project === project`. So
// this test is answered by the repository shortcut and would stay green with `identityAgrees`
// returning a constant true (measured). The proof itself is pinned by 'a same-repo, same-branch
// directory at another head is refused once the rejoin is armed' further down, and the flag that
// selects between the two modes by the tests in lib/run-logging.test.mjs.
...
test('a stalled run whose verify verdict could not be linked to a transcript refuses ledgerSource:journal', (t) => {
  ...
  // findPriorRound must then refuse its LEDGER exactly like a partial with no ledger at all — but
  // the round still happened, so it comes back as a DEGRADED round (empty ledger, the finding count
  // that makes the loss loud) rather than as `found:false`, which renders the next run as a first review.
  ...
  assert.equal(hit.ledger.length, 0, 'the untrustworthy journal ledger is still refused')
  // journalSourced now says "this head came from a run that STOPPED, do not diff off it" — and that
  // is this shape exactly: the round is read off the surviving checkpoint directory, whose head is
  // where the run stalled and may equal this run's HEAD. The engine reads the flag to force a full
  // base...HEAD re-scan (lib/loop-state.test.mjs pins that seam). It is NOT a claim that the refused
  // journal ledger was used: the assertion above still holds it at zero.
  assert.equal(hit.journalSourced, true)
})
...
test('findPriorRound still refuses a checkpoint-only partial record\'s ledger, but reports it as a degraded round', (t) => {
  const store = tmpStore()
  const { dir: project, head } = tinyRepo()
  t.after(() => fs.rmSync(project, { recursive: true, force: true }))
  const ts = '2026-07-10T00-00-00Z'
  ...
  const hit = findPriorRound({ store, project, branch: 'feat/x' })
  // The ledger is refused (a checkpoint-only recovery holds counts, not findings) — but the round is
  // reported, because `found:false` here is what makes the next run render as a first review.
  assert.equal(hit.found, true)
  assert.equal(hit.ledger.length, 0)
  assert.equal(hit.priorFindings, 1, 'a non-zero count beside an empty ledger is what review.js reads as DEGRADED')
})
...
// ---- prior-round transport: a SHORT `why`, the full one recoverable by reference ----------------
// The ledger crosses an agent as structured output into the next round; a full rationale is the bulk
// of that payload. `findPriorRound` carries a SHORT `why` (<=WHY_TRANSPORT_MAX) and a `whyRef` back to
// the record the FULL one lives in — and it does this at the READ boundary only, never rewriting the
// record it read.
test('findPriorRound shortens a long transport `why`, points whyRef at the birth record, and leaves the record on disk untouched', (t) => {
  const store = tmpStore()
  const { dir: project, head } = tinyRepo()
  t.after(() => fs.rmSync(project, { recursive: true, force: true }))
  const ts = '2026-07-12T00-00-00Z'
  const recordName = `${ts}-workflow-review.json`
  const longWhy = 'x'.repeat(600)
  const shortWhy = 'y'.repeat(120)
  const item = over => ({ fp: 'fp', file: 'src/a.rs', line: 1, symbol: 's', severity: 'High', tier: 'confirmed', disposition: 'open', source: 'safety', ruleId: 'R', title: 't', why: 'w', ...over })
  const recPath = path.join(store, recordName)
  fs.writeFileSync(recPath, JSON.stringify({
    round: 2, findings: { total: 3 },
    ledger: [
      item({ fp: 'long', why: longWhy }),                                              // full why, no ref → truncate + birth ref
      item({ fp: 'short', why: shortWhy }),                                            // already short, no ref → untouched
      item({ fp: 'carried', why: shortWhy, whyRef: { record: 'older-birth.json', fp: 'orig' } }), // already carried → propagate
    ],
  }))
  ...
  const hit = findPriorRound({ store, project, branch: 'feat/x' })
  assert.equal(hit.found, true)
  const [long, short, carried] = hit.ledger

  // A long why is truncated to EXACTLY the cap and gains a whyRef pointing at THIS record (its birth).
  assert.equal(long.why.length, WHY_TRANSPORT_MAX, 'a >cap why is shortened to exactly the cap')
  assert.equal(long.why, longWhy.slice(0, WHY_TRANSPORT_MAX))
  assert.deepEqual(long.whyRef, { record: recordName, fp: 'long' }, 'whyRef names this record and the item fp')

  // A short why is left as-is and owes no pointer.
  assert.equal(short.why, shortWhy, 'a <=cap why is untouched')
  assert.equal('whyRef' in short, false, 'a <=cap why carries no whyRef')

  // A carried item keeps BOTH its why and its ORIGINAL birth reference — not re-pointed at this record.
  assert.equal(carried.why, shortWhy, 'an already-carried why is untouched')
  assert.deepEqual(carried.whyRef, { record: 'older-birth.json', fp: 'orig' }, 'the birth reference propagates forward unchanged')

  // The transport-integrity guard reads entry COUNT, not field length: shortening must not perturb it.
  assert.equal(hit.ledgerCount, hit.ledger.length, 'ledgerCount matches the array length after shortening')

  // Read-only: the record on disk still holds the FULL why after the read.
  assert.equal(fs.readFileSync(recPath, 'utf8'), before, 'findPriorRound must not rewrite the record it reads')
  assert.equal(JSON.parse(before).ledger[0].why.length, 600, 'the stored full why is intact on disk')
})

// The THIRD transport path: a round recovered from a dead run's surviving checkpoints, with no
// finalized record at all. `partialChainEvidence` reads the sharded ledger and reconcileChain returns
// it verbatim through `answer`, so the `why` must be shortened where the evidence ledger is built or it
// crosses the agent full-length with no `whyRef` — and a later normal-path read would then re-truncate
// an already-markered `why` (cutting the adjudicator marker) and mint a `whyRef` at the wrong record.
test('findPriorRound shortens a long `why` on the EVIDENCE-RECOVERY path and points whyRef at the surviving directory', (t) => {
  const store = tmpStore()
  const { dir: project, head } = tinyRepo()
  t.after(() => fs.rmSync(project, { recursive: true, force: true }))
  const longWhy = 'x'.repeat(600)
  // A dead run's surviving checkpoint directory: no index, no detail record — exactly the loss this
  // path exists for. Its sharded ledger holds one FRESH finding whose `why` is over the transport cap
  // and carries no `whyRef` (it has never been transported before).
  const dir = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project, now: new Date('2026-07-13T00:00:00Z') })
  writeCheckpoint(dir, 'rust-plan', { branch: 'feat/x', head }, { project })
  const item = { fp: 'ev', file: 'src/a.rs', line: 1, symbol: 's', severity: 'High', tier: 'confirmed', disposition: 'open', source: 'safety', ruleId: 'R', title: 't', why: longWhy }
  writeCheckpoint(dir, 'ledger-01', { branch: 'feat/x', head, ledgerShard: { index: 1, of: 1, total: 1 }, ledgerItems: [item] }, { project })

  const hit = findPriorRound({ store, project, branch: 'feat/x' })
  assert.equal(hit.found, true, 'a surviving directory with no complete round before it is the evidence-recovered round')
  assert.equal(hit.ledger.length, 1, 'the dead run\'s sharded ledger is the carried ledger')
  const [ev] = hit.ledger

  // Falsifier: before the evidence path was shortened this `why` crossed at its full 600 chars with no
  // whyRef. It must now be truncated to EXACTLY the cap and point back at the SURVIVING DIRECTORY — its
  // birth on this path — because no finalized record exists here to name.
  assert.equal(ev.why.length, WHY_TRANSPORT_MAX, 'a >cap evidence `why` is shortened to exactly the cap')
  assert.equal(ev.why, longWhy.slice(0, WHY_TRANSPORT_MAX))
  assert.deepEqual(ev.whyRef, { record: path.basename(dir), fp: 'ev' }, 'whyRef names the surviving directory and the item fp')

  // Per-item `why` length changes only; the entry COUNT is untouched, so the round still reads complete.
  assert.equal(hit.ledgerCount, hit.ledger.length, 'ledgerCount matches the array length after shortening')
})
...
// ---- the ownership proof must survive the payload shapes the engine really writes -------------
// `workflows/review.js` once wrote `head: baseRef` (the diff base, a ref NAME) into every checkpoint
// and the run's real HEAD into the final record. Measured before the fix: `finalizeRun` returned
...
// It still folds, but for a stronger reason than "head is ignored": both sides are now stamped by
// the CLI's own `gitIdentity`, so a model-authored `head: 'origin/main'` never reaches the proof.
...
// (The function-level counterpart to the above used to assert that `head` does NOT decide
// ownership. It was retired with the fix that put `head` back — see
// "identityAgrees: `head` decides ownership again" at the end of this file. It was not merely a
// stale expectation: it pinned the decayed proof, under which any directory of the same project
// and branch was adoptable.)

// ---- the ownership proof must stand on what the CODE computes ---------------------------------
// The proof had decayed to "the same repository". Three edges met: `head` was dropped from
// `identityAgrees` because the two writers spelled it differently; the spelling was then fixed
// (checkpoints carry the real head plus `baseRef` under its own key); and the `detect` prompt that
// supplies `branch`/`head` was rewritten to say "these are a fallback only, an empty string is a
// fine answer" — while `checkpoint` was the one command that did NOT apply the code-computed
// identity. A model that believes the prompt writes `{project, '', ''}`, and `identityAgrees` then
// adopts any directory of the same project. `repoKey` deliberately collapses a linked worktree onto
// the main checkout, so "same project" is wider than "same checkout".
test('the checkpoint command stamps the CODE-computed branch and head, not the model\'s', () => {
  const store = tmpStore()
  const repo = initRepo()
  const head = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: repo, encoding: 'utf8' }).trim()
  const cli = (args, stdin) => execFileSync('node', [SCRIPT, ...args, '--store', store, '--project', repo],
    { cwd: repo, input: stdin, encoding: 'utf8' })
  // Exactly what the rewritten `detect` prompt invites: both fields left empty.
  const ck = JSON.parse(cli(['checkpoint', '--phase', 'Gate'], JSON.stringify({
    kind: 'workflow', name: 'review', phase: 'Gate', branch: '', head: '',
  })).trim())
  const phases = readCheckpoints(ck.runDir)
  assert.equal(phases.length, 1)
  assert.equal(phases[0].branch, 'feat/measured', 'the branch is git\'s answer, not the model\'s empty string')
  assert.equal(phases[0].head, head, 'and so is the head — the field the proof stands on')
})

// With the spelling fixed and both sides stamped by code, `head` is a real discriminator again and
// the proof must use it. The concrete loss it closes: a STALE `.partial` left by an abandoned
// earlier run of the same repo and branch passed the weakened proof, so its phases were folded into
// this run's record and its directory deleted — and this file's own header calls that
// irrecoverable ("fragmentation is recoverable, a merged record is not").
test('identityAgrees: `head` decides ownership again, and a self-disagreeing head disqualifies', () => {
  const mine = { project: '/repos/mine', branch: 'feat/x', head: 'abc1234' }
  assert.equal(identityAgrees(mine, { project: '/repos/mine', branch: 'feat/x', head: 'abc1234' }), true,
    'the same project, branch and head is the same run')
  assert.equal(identityAgrees(mine, { project: '/repos/mine', branch: 'feat/x', head: 'def5678' }), false,
    'a DIFFERENT head is a different run — a stale abandoned .partial must not be folded in')
  assert.equal(identityAgrees(mine, { project: '/repos/mine', branch: 'feat/x', head: null }), false,
    'and a directory that cannot agree with ITSELF about the head is nobody\'s')
...
// The consequence at the CLI level, which is the level the damage happens at.
test('finalize refuses a stale .partial of the same repo and branch but an older head', () => {
  const store = tmpStore()
  const repo = initRepo()
  const cli = (args, stdin) => execFileSync('node', [SCRIPT, ...args, '--store', store, '--project', repo],
    { cwd: repo, input: stdin, encoding: 'utf8' })
  // An abandoned run at an EARLIER commit, stamped by the code of its own time.
  const stale = JSON.parse(cli(['checkpoint', '--phase', 'Gate'], JSON.stringify({
    kind: 'workflow', name: 'review', phase: 'Gate',
  })).trim())
  // The tree moves on; this run's identity is a different head.
  fs.writeFileSync(path.join(repo, 'b.txt'), 'b\n')
  execFileSync('git', ['-c', 'user.email=t@t', '-c', 'user.name=t', 'add', 'b.txt'], { cwd: repo })
  execFileSync('git', ['-c', 'user.email=t@t', '-c', 'user.name=t', 'commit', '-q', '-m', 'second'], { cwd: repo })
  const out = cli(['finalize', '--rejoin'], JSON.stringify({ ...RECORD, branch: 'feat/measured' }))
  assert.ok(!/folded/.test(out), `the stale directory must not be folded into this run; got ${out}`)
  assert.equal(fs.existsSync(stale.runDir), true, 'and it must survive for `recover`, not be deleted')
})

// ---- the run's identity is the one observed when it STARTED ------------------------------------
// `gitId` is read at the moment the record is WRITTEN, which for a review is hours after the run
// began. Switch branches mid-review — an ordinary thing to do while a long audit runs — and the
// record is filed under whatever is checked out at the end. That is not a cosmetic field: the
// re-review chain selects on `branch`, so the run's whole ledger is attributed to a branch it never
// looked at, and the branch it DID review has no round for it.
// The run's own first checkpoint already carries the code-computed identity from near its start
// (that is what the ownership proof reads), so finalize prefers what its OWN checkpoints attest and
// falls back to write-time git only when there are none.
test('finalize files the run under the branch it STARTED on, not the one checked out at the end', () => {
  const store = tmpStore()
  const repo = initRepo()
  ...
  // The reviewer switches branches while the review is still running.
  execFileSync('git', ['-c', 'user.email=t@t', '-c', 'user.name=t', 'checkout', '-q', '-b', 'feat/elsewhere'], { cwd: repo })
  fs.writeFileSync(path.join(repo, 'c.txt'), 'c\n')
  execFileSync('git', ['-c', 'user.email=t@t', '-c', 'user.name=t', 'add', 'c.txt'], { cwd: repo })
  execFileSync('git', ['-c', 'user.email=t@t', '-c', 'user.name=t', 'commit', '-q', '-m', 'elsewhere'], { cwd: repo })

  const out = cli(['finalize', '--dir', ck.runDir], JSON.stringify({ ...RECORD }))
  assert.match(out, /folded 1 checkpoint/, 'the run still owns its own directory — the fold must not be collateral')
  const rec = JSON.parse(fs.readFileSync(path.join(store, fs.readdirSync(store).find(f => f.endsWith('.json'))), 'utf8'))
  assert.equal(rec.branch, 'feat/measured', 'the branch the review actually ran against')
  assert.equal(rec.head, startHead, 'and the head it actually reviewed')
  const row = JSON.parse(fs.readFileSync(path.join(store, 'index.jsonl'), 'utf8').trim())
  assert.equal(row.branch, 'feat/measured', 'the INDEX row is what the round chain selects on')
})

// ---- ownership of a run directory, on the path the engine actually takes ------------------------
// The blocking finding this closes: the `--dir`/`--rejoin` mode split in `finalizeRun` was decided by
// a flag that could not reach it. `logRunPrompt` built the rejoin flag as `!dir && rejoin` while
// review.js finalized with `{ dir: runDir, rejoin: checkpointFailed }`, so a run that had a runDir
// sent `--dir` and swallowed `--rejoin` — `ownDir` was true on every such run and ownership reduced
// to `attested.project === project`, i.e. "the same repository", with `repoKey` collapsing a linked
// worktree onto the main checkout on top of that. The damage path: first checkpoint fails → the
// second rejoins and ADOPTS a live directory → finalize names it with `--dir` and no `--rejoin` →
// folded into this record and DELETED while its real owner is still writing.
//
// Driven through the CLI with the argument line the ENGINE's own builder produces, not with
// hand-written flags — the defect lived precisely in the gap between the two.
test('an ADOPTED run directory is neither folded nor deleted, with the flags the engine really sends', () => {
  const store = tmpStore()
  const now = new Date('2026-08-02T14:26:15Z')
  // The victim: a live run of the SAME repository on another branch. Same project is all the broken
  // check ever asked for.
  const theirs = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: '/repos/mine', now })
  writeCheckpoint(theirs, 'rust-plan', { branch: 'feat/theirs', head: 'aaaa1111' }, { project: '/repos/mine' })

  // Exactly what review.js passes to logRunPrompt once one of its checkpoints has failed and a later
  // one adopted a directory: the adopted dir AND the armed rejoin.
  const prompt = logRunPrompt({
    record: { kind: 'workflow', name: 'review' }, command: 'finalize', dir: theirs, rejoin: true,
  })
  const argLine = prompt.split('\n').find(l => l.includes('$CRAFT_LOGGER'))
  const flags = argLine.slice(argLine.indexOf('finalize')).split(/\s+/).slice(1)
    .filter(a => a.startsWith('--dir') || a.startsWith('--rejoin') || a.startsWith("'/"))
    .map(a => a.replace(/^'|'$/g, ''))
  assert.deepEqual(flags, ['--dir', theirs, '--rejoin'],
    'the engine\'s own builder must hand the CLI both flags — this is where the guard was lost')

  execFileSync('node', [SCRIPT, 'finalize', ...flags, '--store', store, '--project', '/repos/mine'],
    { input: JSON.stringify({ ...RECORD, branch: 'feat/mine', head: 'bbbb2222' }), encoding: 'utf8' })
  assert.ok(fs.existsSync(theirs), 'the adopted directory must survive — its owner is still running')
  assert.equal(readCheckpoints(theirs).length, 1, 'with its checkpoints intact')
  const rec = JSON.parse(fs.readFileSync(path.join(store, fs.readdirSync(store).find(f => f.endsWith('.json'))), 'utf8'))
  assert.ok(!rec.phases, 'and nothing of the other run enters this record')
})

// The companion of the test above, and the one finding #2 is about: the committed `--dir` guard went
// through the own-directory shortcut, so its verdict was decided by `attested.project === project`
// alone — it stayed green with `identityAgrees` returning a constant true (measured: gutting the
// function failed six other tests and not that one). This drives the SEARCHED/adopted mode, where the
// full proof is what answers, and it distinguishes on a field the shortcut cannot see: same project,
// same branch, a different head.
test('a same-repo, same-branch directory at another head is refused once the rejoin is armed', () => {
  const store = tmpStore()
  const now = new Date('2026-08-02T14:26:15Z')
  const stale = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: '/repos/mine', now })
  writeCheckpoint(stale, 'rust-plan', { branch: 'feat/x', head: '1111aaaa' }, { project: '/repos/mine' })

  const res = finalizeRun({ ...RECORD, branch: 'feat/x', head: '2222bbbb' },
    { store, project: '/repos/mine', dir: stale, rejoin: true, now: new Date('2026-08-02T14:40:00Z') })
  assert.equal(res.folded, 0, 'a run at another head is another run — project and branch alone cannot say so')
  assert.equal(res.kept, true, 'and it is reported as kept, not silently dropped')
  assert.ok(fs.existsSync(stale), 'nor deleted')
})

// Finding #3. `dirIdentity` poisons a field its checkpoints DISAGREE about to `null`, and the guard
// read `!attested.project` as "this directory attests nothing", which is the legacy-leftover case and
// short-circuits to `mine`. So the single dirtiest directory in the store — one holding checkpoints
// from two different repositories — was the one most certain to be folded and deleted.
test('a directory whose checkpoints disagree about the project is refused, in both modes', () => {
  const store = tmpStore()
  const now = new Date('2026-08-02T14:26:15Z')
  const mixed = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: '/repos/mine', now })
  writeCheckpoint(mixed, 'rust-plan', { branch: 'feat/x' }, { project: '/repos/mine' })
  writeCheckpoint(mixed, 'rust-lenses', { branch: 'feat/x' }, { project: '/repos/elsewhere' })
  assert.equal(dirIdentity(readCheckpoints(mixed)).project, null, 'the contradiction is what poisons the field')

  // The own-directory mode: `--dir`, no rejoin. This is the one that used to accept it.
  const own = finalizeRun({ ...RECORD, branch: 'feat/x' },
    { store, project: '/repos/mine', dir: mixed, now: new Date('2026-08-02T14:40:00Z') })
  assert.equal(own.folded, 0, 'a directory that cannot agree with itself proves nothing')
  assert.equal(own.kept, true)
  assert.ok(fs.existsSync(mixed), 'and must not be deleted — recover can still promote it')

  // …and the searched mode, which already refused, pinned so the two cannot drift apart again.
  const searched = finalizeRun({ ...RECORD, branch: 'feat/x' },
    { store, project: '/repos/mine', dir: mixed, rejoin: true, now: new Date('2026-08-02T14:41:00Z') })
  assert.equal(searched.folded, 0)
  // A leftover attesting NO project at all is still the legacy case, and still foldable.
  const legacy = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: '/repos/mine', now })
  fs.writeFileSync(path.join(legacy, '00-rust-plan.json'), JSON.stringify({ phase: 'rust-plan' }))
  assert.equal(finalizeRun({ ...RECORD }, { store, project: '/repos/mine', dir: legacy, now }).folded, 1,
    'absence of identity is not disagreement — refusing it would strand every pre-identity partial')
})

// Finding #5. The start-of-run identity was preferred WHOLESALE as soon as ONE of its two fields was
// attested: `{ branch: attested.branch || '', head: attested.head || '' }`. `applyGitIdentity` skips
// an empty field, so a run whose checkpoints knew the head but not the branch filed the branch from
// the PAYLOAD — the model-reported value this whole path exists to stop trusting, and exactly the
// inversion the "filed under the identity it reviewed" fix was made to remove.
test('a half-attested directory improves the field it knows without demoting the other', () => {
  const store = tmpStore()
  const now = new Date('2026-08-02T14:26:15Z')
  const dir = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: '/repos/mine', now })
  // Head attested, branch not — a checkpoint written on a detached HEAD, where `gitIdentity` yields
  // no branch by design.
  writeCheckpoint(dir, 'rust-plan', { head: 'startsha' }, { project: '/repos/mine' })

  const res = finalizeRun({ ...RECORD, branch: 'branch-the-model-guessed' }, {
    store, project: '/repos/mine', dir, now: new Date('2026-08-02T14:40:00Z'),
    gitId: { branch: 'feat/observed', head: 'endsha' },
  })
  const rec = JSON.parse(fs.readFileSync(res.file, 'utf8'))
  assert.equal(rec.head, 'startsha', 'the head the run actually reviewed wins over write-time git')
  assert.equal(rec.branch, 'feat/observed',
    'and the branch falls back to OBSERVED git, never to the payload, just because the directory did not attest one')
})

// Finding #4. One record described two working copies: `branch`/`head` were read off `workdir` (the
// path the caller pointed at) while `commit`/`dirty` were read off `project`, which is a `repoKey`
// and deliberately collapses a linked worktree onto its MAIN checkout. Any analysis comparing `head`
// to `commit` was then comparing different checkouts — and on this machine agent worktrees are
// ordinary, so it is the reviewed side that lost.
test('commit and dirty describe the same checkout as branch and head, in a linked worktree', () => {
  const store = tmpStore()
  const main = initRepo()
  const wt = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'craft-wt-')), 'wt')
  execFileSync('git', ['worktree', 'add', '-q', '-b', 'feat/inthetree', wt], { cwd: main })
  try {
    fs.writeFileSync(path.join(wt, 'b.txt'), 'b\n')
    execFileSync('git', ['-c', 'user.email=t@t', '-c', 'user.name=t', 'add', 'b.txt'], { cwd: wt })
    execFileSync('git', ['-c', 'user.email=t@t', '-c', 'user.name=t', 'commit', '-q', '-m', 'in the tree'], { cwd: wt })
    assert.notEqual(
      execFileSync('git', ['rev-parse', 'HEAD'], { cwd: wt, encoding: 'utf8' }).trim(),
      execFileSync('git', ['rev-parse', 'HEAD'], { cwd: main, encoding: 'utf8' }).trim(),
      'the two checkouts must really be at different commits, or this test proves nothing')

    execFileSync('node', [SCRIPT, 'write', '--store', store, '--project', wt],
      { cwd: wt, input: JSON.stringify({ ...RECORD }), encoding: 'utf8' })
    const rec = JSON.parse(fs.readFileSync(path.join(store, fs.readdirSync(store).find(f => f.endsWith('.json'))), 'utf8'))
    assert.equal(rec.project, repoKey(wt), 'the round chain is still keyed to the collapsed repo, deliberately')
    assert.equal(rec.branch, 'feat/inthetree', 'the branch is the worktree\'s')
    assert.ok(rec.head.startsWith(rec.commit),
      `commit must be the short form of the head in the SAME checkout — got head ${rec.head}, commit ${rec.commit}`)
  } finally {
    execFileSync('git', ['worktree', 'remove', '--force', wt], { cwd: main })
  }
})

// A REFUSED `--dir` must not fall through into the rejoin SEARCH. Before the rejoin flag reached the
// CLI at all, `--dir <d> --rejoin` was inexpressible, so a garbled directory ended harmlessly: no
// target, record written, nothing folded. With the flag delivered the same input reaches the search,
// and the search refuses only at TWO or more candidates — so a `rust-audit` sibling (one nested
// review per crate, sharing project, branch AND head) finds EXACTLY ONE match: its live sibling's
// directory, which it then folds into its own record and DELETES while that sibling is still writing.
// Refusing is the only answer available here: the engine's statement about the directory arrived
// corrupted, and a search that lands on the one live neighbour is a guess dressed as a proof.
// Driven through the real CLI with the flags the ENGINE builds — the last defect of this family lived
// exactly in the gap between what review.js meant and what the command line said.
test('a refused --dir refuses the rejoin search too — a sibling\'s live directory is never adopted', () => {
  const store = tmpStore()
  const repo = initRepo()
  const gitId = gitIdentity(repo)

  // The sibling: a live run of the same repo, branch and head, still writing.
  const sibling = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: repoKey(repo) })
  writeCheckpoint(sibling, 'rust-plan', { kind: 'workflow', name: 'review', ...gitId }, { project: repoKey(repo) })

  // What a model garbles `runDir` into: a path outside the store.
  const garbled = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'craft-garbled-')), 'run-dir')

  // The command line is the engine's, not the test's: same builder review.js finalizes through.
  const prompt = logRunPrompt({ record: RECORD, command: 'finalize', dir: garbled, rejoin: true })
  const m = prompt.match(/node "\$CRAFT_LOGGER" finalize (.*?)\$\{CLAUDE_CODE_SESSION_ID/)
  assert.ok(m, 'the finalize command line must be readable out of the prompt the engine sends')
  const flags = m[1].trim().split(/\s+/).map(s => s.replace(/^'|'$/g, ''))
  assert.deepEqual(flags, ['--dir', garbled, '--rejoin'], 'and it must really carry both — this is the combination that was once inexpressible')

  const out = execFileSync('node', [SCRIPT, 'finalize', ...flags, '--store', store, '--project', repo],
    { cwd: repo, input: JSON.stringify(RECORD), encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] })

  assert.ok(fs.existsSync(sibling), 'the sibling\'s directory must survive: deleting it destroys a live run\'s telemetry')
  assert.equal(fs.readdirSync(sibling).filter(f => f.endsWith('.json')).length, 1, 'and its slices with it')
  assert.doesNotMatch(out, /folded/, 'nothing may be folded — the phases belong to another run')

  const rec = JSON.parse(fs.readFileSync(path.join(store, fs.readdirSync(store).find(f => f.endsWith('.json'))), 'utf8'))
  assert.ok(!rec.phases, 'and the record must not carry the sibling\'s phases')
})

test('a partial record\'s sharded ledger is normalized at the read boundary, so one missing key cannot void the load', (t) => {
  const store = tmpStore()
  const { dir: project, head } = tinyRepo()
  t.after(() => fs.rmSync(project, { recursive: true, force: true }))
  const ts = '2026-07-11T00-00-00Z'
  // `recover` copies checkpoints into the record's `phases` unfiltered and then deletes the
  // directory, so this is how a shard survives. The entry is written the way `toLedgerEntry` writes
  // it: `severity` has no fallback there, so a finding that lacks one yields an entry with the key
  // ABSENT — JSON.stringify drops it. Handed on raw, that array reaches the next round as the
  // workflow's structured output, fails its strict schema (additionalProperties:false, 11 required
  // fields), and the prior round zeroes out: a blind review rendered as a first one.
  fs.writeFileSync(path.join(store, `${ts}-workflow-review.json`), JSON.stringify({
    partial: true, findings: { total: 2 }, round: 3,
    phases: [{ phase: 'ledger-01', ledgerShard: { index: 1, of: 1, total: 1 }, ledgerItems: [{ fp: 'aaaa', file: 'src/a.rs', line: 1, title: 't', why: 'w' }] }],
  }))
  fs.writeFileSync(path.join(store, 'index.jsonl'),
    JSON.stringify({ ts, kind: 'workflow', name: 'review', project, branch: 'feat/x', head }) + '\n')
  ...
  const hit = findPriorRound({ store, project, branch: 'feat/x' })
  assert.equal(hit.found, true)
  assert.equal(hit.ledger.length, 1, 'the shard inside the record is the carried ledger')
  for (const key of ['fp', 'file', 'line', 'symbol', 'severity', 'tier', 'disposition', 'source', 'ruleId', 'title', 'why']) {
    assert.ok(Object.hasOwn(hit.ledger[0], key), `every key the strict schema requires must be present — '${key}' was not`)
  }
})

// ---- enrich-cost: the REAL run cost, folded in post-hoc from the per-agent transcripts ---------
// `outputTokens` on a record is `budget.spent()` — the harness pool, under 1% of real spend. The
// real cost is cache_read/cache_write, which lives only in `<run-dir>/agent-*.jsonl` and is out of
// the live sandbox engine's reach, so it is summed after the fact by the launcher.
test('sumTranscriptUsage sums the four token classes over assistant records, skipping non-assistant and malformed lines', () => {
  const text = [
    JSON.stringify({ type: 'assistant', message: { model: 'claude-x', usage: { input_tokens: 10, output_tokens: 3, cache_creation_input_tokens: 100, cache_read_input_tokens: 5000 } } }),
    JSON.stringify({ type: 'user', message: { role: 'user', content: 'hi' } }), // not assistant → skipped
    '{ this is not json',                                                        // malformed → skipped
    JSON.stringify({ type: 'assistant', message: { usage: { input_tokens: 2, output_tokens: 1, cache_creation_input_tokens: 20, cache_read_input_tokens: 4000 } } }),
    '',                                                                          // blank → skipped
    JSON.stringify({ type: 'assistant', message: {} }),                          // assistant, no usage → skipped
  ].join('\n')
  assert.deepEqual(sumTranscriptUsage(text), { output: 4, input: 12, cacheRead: 9000, cacheWrite: 120 })
  // The array form (pre-parsed lines) is tolerated too, and an empty input is a zero sum, not a throw.
  assert.deepEqual(sumTranscriptUsage([]), { output: 0, input: 0, cacheRead: 0, cacheWrite: 0 })
})

test('enrich-cost writes a cost object summed from the run dir\'s agent transcripts, preserves other fields, and is idempotent on re-run', () => {
  const dir = tmpStore()
  const runDir = path.join(dir, 'run')
  fs.mkdirSync(runDir)
  fs.writeFileSync(path.join(runDir, 'agent-a.jsonl'),
    JSON.stringify({ type: 'assistant', message: { usage: { input_tokens: 10, output_tokens: 3, cache_creation_input_tokens: 100, cache_read_input_tokens: 5000 } } }) + '\n' +
    JSON.stringify({ type: 'user', message: { content: 'x' } }) + '\n')
  fs.writeFileSync(path.join(runDir, 'agent-b.jsonl'),
    JSON.stringify({ type: 'assistant', message: { usage: { input_tokens: 2, output_tokens: 1, cache_creation_input_tokens: 20, cache_read_input_tokens: 4000 } } }) + '\n')
  const recordFile = path.join(dir, 'rec.json')
  fs.writeFileSync(recordFile, JSON.stringify({ schemaVersion: 1, name: 'review', outputTokens: 12345, verdict: 'Approve' }, null, 2))

  const out = execFileSync('node', [SCRIPT, 'enrich-cost', '--run-dir', runDir, '--record', recordFile], { encoding: 'utf8' })
  assert.match(out, /enriched/)
  const rec = JSON.parse(fs.readFileSync(recordFile, 'utf8'))
  assert.deepEqual(rec.cost, { output: 4, input: 12, cacheRead: 9000, cacheWrite: 120, total: 9136, agents: 2 })
  // Every other field survives the merge untouched.
  assert.equal(rec.outputTokens, 12345)
  assert.equal(rec.verdict, 'Approve')
  assert.equal(rec.schemaVersion, 1)

  // Idempotent: a second run REPLACES cost from the same transcripts, never accumulates onto a stale one.
  execFileSync('node', [SCRIPT, 'enrich-cost', '--run-dir', runDir, '--record', recordFile], { encoding: 'utf8' })
  const rec2 = JSON.parse(fs.readFileSync(recordFile, 'utf8'))
  assert.deepEqual(rec2.cost, { output: 4, input: 12, cacheRead: 9000, cacheWrite: 120, total: 9136, agents: 2 })

  // The exported helper agrees with the CLI (no drift between the two entry points).
  assert.deepEqual(enrichCost({ runDir, recordFile }).cost, rec2.cost)
})

test('enrich-cost exits non-zero with a clear message when the run dir has no agent transcripts', () => {
  const dir = tmpStore()
  const runDir = path.join(dir, 'empty')
  fs.mkdirSync(runDir)
  const recordFile = path.join(dir, 'rec.json')
  fs.writeFileSync(recordFile, JSON.stringify({ schemaVersion: 1, name: 'review' }))
  let err = null
  try {
    execFileSync('node', [SCRIPT, 'enrich-cost', '--run-dir', runDir, '--record', recordFile], { encoding: 'utf8', stdio: 'pipe' })
  } catch (e) { err = e }
  assert.ok(err, 'enrich-cost must fail when there are no transcripts')
  assert.notEqual(err.status, 0)
  assert.match(String(err.stderr || ''), /no agent-.*\.jsonl/)
})

// enrichCost writes the record atomically (temp + rename). The atomic contract has a failure clause:
// if the write or the rename throws, nothing is left behind. writeFileSync-succeeds/renameSync-fails
// is not portably forcible against a plain record file (POSIX rename over a regular file in a writable
// dir always succeeds; the only rename failures — an unwritable dir, or a directory as the target —
// either also fail the temp write or fail enrichCost's earlier read of the record). So the cleanup is
// pinned here, on the helper enrichCost delegates the write to: renaming onto an existing DIRECTORY
// throws EISDIR with the temp already written, exactly the mid-write window the cleanup covers.
test('writeCostRecordAtomically leaves no temp behind when the rename fails', () => {
  const dir = tmpStore()
  const target = path.join(dir, 'target')
  fs.mkdirSync(target)   // a directory as the rename target → renameSync throws EISDIR after the write
  assert.throws(() => writeCostRecordAtomically(target, { cost: { total: 1 } }), /EISDIR|ENOTEMPTY|directory/i)
  const leaked = fs.readdirSync(dir).filter(n => /\.tmp$/.test(n))
  assert.deepEqual(leaked, [], 'a failed atomic write must leave no .tmp behind')
})

// The shared helper both writeCostRecordAtomically and repairIndex now route through. Its failure
// clause — temp written, rename throws, nothing left behind — is pinned here on the same forcing move
// the cost test uses: renaming a file onto an existing DIRECTORY throws (EISDIR on Linux, ENOTEMPTY
// elsewhere) with the temp already written, exactly the mid-write window the cleanup covers. This is
// the cleanup repairIndex GAINED by delegating here — its own rename cannot be forced to fail portably
// (its target must stay a readable index.jsonl the function reads before it writes), so the contract
// is pinned on the shared helper repairIndex delegates the write to.
test('writeFileAtomically leaves no temp behind when the rename fails', () => {
  const dir = tmpStore()
  const target = path.join(dir, 'target')
  fs.mkdirSync(target)   // a directory as the rename target → renameSync throws after the temp write
  assert.throws(() => writeFileAtomically(target, 'payload'), /EISDIR|ENOTEMPTY|directory/i)
  const leaked = fs.readdirSync(dir).filter(n => /\.tmp$/.test(n))
  assert.deepEqual(leaked, [], 'a failed atomic write must leave no .tmp behind')
})

// One unreadable transcript must not discard every good transcript's cost — the finding: an unguarded
// per-transcript readFileSync let a single EISDIR/ENOENT/EACCES abort the whole sum.
test('enrich-cost skips an unreadable transcript (a subdirectory matching the glob) and sums only the good one', () => {
  const dir = tmpStore()
  const runDir = path.join(dir, 'run')
  fs.mkdirSync(runDir)
  fs.writeFileSync(path.join(runDir, 'agent-good.jsonl'),
    JSON.stringify({ type: 'assistant', message: { usage: { input_tokens: 10, output_tokens: 3, cache_creation_input_tokens: 100, cache_read_input_tokens: 5000 } } }) + '\n')
  // Matches /^agent-.*\.jsonl$/ but is a DIRECTORY: readFileSync on it throws EISDIR (portable).
  fs.mkdirSync(path.join(runDir, 'agent-bad.jsonl'))
  const recordFile = path.join(dir, 'rec.json')
  fs.writeFileSync(recordFile, JSON.stringify({ schemaVersion: 1, name: 'review' }, null, 2))

  const { cost } = enrichCost({ runDir, recordFile })
  // agents counts what was actually summed (1, not the glob's 2); skipped makes the loss legible.
  assert.deepEqual(cost, { output: 3, input: 10, cacheRead: 5000, cacheWrite: 100, total: 5113, agents: 1, skipped: 1 })
  assert.deepEqual(JSON.parse(fs.readFileSync(recordFile, 'utf8')).cost, cost)
})

// The tolerance stops at "nothing readable": every matching transcript unreadable is the no-data case,
// and a cost of all zeros there would read as a real measurement of a free run — so it still throws.
test('enrich-cost still throws when every matching transcript is unreadable', () => {
  const dir = tmpStore()
  const runDir = path.join(dir, 'run')
  fs.mkdirSync(runDir)
  ...
  const recordFile = path.join(dir, 'rec.json')
  fs.writeFileSync(recordFile, JSON.stringify({ schemaVersion: 1, name: 'review' }))
  assert.throws(() => enrichCost({ runDir, recordFile }), /no readable agent-.*\.jsonl/)
  assert.equal(JSON.parse(fs.readFileSync(recordFile, 'utf8')).cost, undefined, 'the record is left untouched')
})

// Readability is not zero-sum: transcripts can parse fine yet carry no assistant usage — an
// empty/truncated transcript, or the runtime renaming a usage field (external surface, node #98).
// That files an all-zeros cost with agents>0, which tallyCost would average as a genuine near-free
// run — silently wrong. enrichCost must still FILE it (a trivial run is theoretically possible) but
// WARN loudly. Without the guard this test is red: no warning reaches stderr for the zero-sum case.
test('enrich-cost warns loudly when readable transcripts sum to zero tokens (a real run is never free)', () => {
  const dir = tmpStore()
  const runDir = path.join(dir, 'run')
  fs.mkdirSync(runDir)
  // Two READABLE transcripts that parse cleanly but carry no assistant usage: one only user/result
  // records, one an assistant record with no `.message.usage`. sumTranscriptUsage returns all zeros
  // for each, so both are summed (agents == 2) yet total == 0.
  fs.writeFileSync(path.join(runDir, 'agent-a.jsonl'),
    JSON.stringify({ type: 'user', message: { content: 'x' } }) + '\n' +
  ...
  let cost
  try {
    cost = enrichCost({ runDir, recordFile }).cost
  } finally {
    console.error = orig
  }
  // Readable → filed, not refused; both transcripts summed, but the sum is zero.
  assert.equal(cost.agents, 2)
  assert.equal(cost.total, 0)
  assert.deepEqual(JSON.parse(fs.readFileSync(recordFile, 'utf8')).cost, cost, 'the all-zeros cost is still filed, not refused')
  // The anomaly is loud on stderr and points at the usage-schema external surface.
  const warned = warnings.join('\n')
  assert.match(warned, /summed to zero tokens/)
  assert.match(warned, /a real run is never free/)
  assert.match(warned, /node #98/)
})
