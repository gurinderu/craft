...
test('a run whose first checkpoint failed still lands in ONE directory', () => {
  ...
  const d2 = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: store, now: new Date('2026-08-02T14:26:15Z') })
  writeCheckpoint(d2, 'rust-lenses', { ranLenses: ['safety'], candidates: { total: 12, bySeverity: {} } })
  // Phase 3 comes with no --dir either, because the workflow still has no runDir.
  const d3 = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: store, now: new Date('2026-08-02T14:41:02Z') })
  writeCheckpoint(d3, 'rust-verify', { verdict: 'Block' })
...
test('a stale leftover is not adopted by the next run', () => {
  ...
  writeCheckpoint(dead, 'rust-plan', { branch: 'old', gate: { status: 'pass' } })
  ...
  const fresh = checkpointDir({ kind: 'workflow', name: 'review' }, { store, project: store, now: new Date('2026-08-02T14:26:15Z') })
