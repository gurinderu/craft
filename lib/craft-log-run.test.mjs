...
test('concurrent writers each keep their record — the name is claimed, not probed', async () => {
  ...
  const src = `
    ...
    const [store, who] = process.argv.slice(2)
    writeRecord({ schemaVersion: 1, kind: 'workflow', name: 'review', verdict: who },
  ...
  const kids = writers.map(who => spawn(process.execPath, [runner, store, who], { stdio: 'ignore' }))
  ...
  assert.equal(files.length, writers.length, 'one record per writer — a lost record is a lost run')
  const verdicts = files.map(f => JSON.parse(fs.readFileSync(path.join(store, f), 'utf8')).verdict).sort()
  assert.deepEqual(verdicts, [...writers].sort(), 'and each writer\'s own record, not five copies of one')
  const index = fs.readFileSync(path.join(store, 'index.jsonl'), 'utf8').trim().split('\n')
  assert.equal(index.length, writers.length)
  ...
  if (named.size && !named.has('')) assert.equal(named.size, writers.length, 'no two index lines may name one file')
})
