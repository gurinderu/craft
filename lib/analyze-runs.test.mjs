...
test('the surface-gate section names pre-gate runs mixed in the same workflow, and never reports a share', () => {
  ...
  assert.match(sgSection, /2 run\(s\) carry the gate \(1 run\(s\) predate the surface gate, excluded here\)/)
  assert.match(sgSection, /1 run\(s\) saved ≥1 pass/)
  assert.match(sgSection, /1 whole-repo pass\(es\) saved total \(compat 1\)/)
  // Honesty over a made-up fraction: no ratio is printed, only the documented reason it is withheld.
  assert.ok(!/share:/i.test(sgSection), 'no share ratio is fabricated')
  assert.match(sgSection, /Share .* is not reported/)
})
