...
// What one ANSWERED dispatch costs of the run's budget — agents and nested workflows alike.
...
export async function runEngine(name, { args = {}, script = {}, budgetTotal = 1e9 } = {}) {
  ...
  const report = await fn(
    ...
    async (...argv) => {
      ...
      // it and an answered one spends like any other call. Without the gate, the one dispatch
      ...
      // Spent at DISPATCH, before the `await` below — the check and the spend must be one
      // synchronous step. With the spend after the await, a concurrent agent() slipped between a
      // nested dispatch's gate check and its spend, and the wall landed one call late with a
      // message naming the wrong dispatch (measured while writing the gate's own test).
      spent += CALL_SPEND
    ...
    if (err && typeof err === 'object') err.calls = calls
    throw err
  })
