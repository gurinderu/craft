...
// The same roster as filenames. Exported rather than re-spelled: this list existed in three unbound
// copies across the test files, which is the "a rule in four files, tested in one" shape twice over —
// a fifth engine added to one copy leaves the other two silently not checking it.
