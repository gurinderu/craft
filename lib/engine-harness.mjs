...
export async function runEngine(name, { args = {}, script = {}, budgetTotal = 1e9 } = {}) {
  ...
  const agent = async (prompt, opts = {}) => {
    ...
    if (spent >= budgetTotal) {
      calls.push({ label, key: null, prompt: String(prompt ?? ''), opts, result: undefined, threw: 'budget exhausted' })
      throw new Error(`agent budget exhausted (spent ${spent} of ${budgetTotal})`)
    }
    const key = keyFor(script, label)
    const callIndex = key === null ? 0 : (keyCounts.get(key) ?? 0)
    if (key !== null) keyCounts.set(key, callIndex + 1)
    // Check + spend + record are ONE synchronous step at dispatch (the model at CALL_SPEND), and
    // a scripted throw is written onto the record before it propagates, mirroring the refusal
    // shape above. With the record made only after the answer, the dying dispatch was the one
    // entry the trail did not contain — err.calls located the death at the previous call — and
    // with the spend there too, the throwing half of the dead-agent class was free while the null
    // half spent, so trail and budget disagreed the moment a test mixed a scripted throw with a
    // wall calibrated as `index × CALL_SPEND`.
    spent += CALL_SPEND
    const call = { label, key, prompt: String(prompt ?? ''), opts, result: undefined }
    calls.push(call)
    let value
    try {
      value = key === null ? undefined : answerFor(script, key, prompt, opts, callIndex)
    } catch (err) {
      call.threw = String(err?.message ?? err)
      throw err
    }
  ...
  const report = await fn(
    ...
    // not there.
    async (...argv) => {
      // Gated exactly like agent(): a nested run is dispatched work, so an exhausted budget refuses
      // it and a dispatched one spends like any other call. Without the gate, the one dispatch
      // surface rust-audit nests through could never run out, and an exhaustion test over a nesting
      // engine would lean on a stub refusing nothing — the same "a stub that could never run out
      // kept every branch unreachable" hole the agent() gate closes. The callers' own
      // `.catch(() => null)` turns the throw into a NOT-RUN entry, same as a nested engine that died.
      if (spent >= budgetTotal) {
        calls.push({ label: 'workflow', key: null, prompt: '', opts: argv[1] ?? {}, argv, result: undefined, threw: 'budget exhausted' })
        throw new Error(`workflow budget exhausted (spent ${spent} of ${budgetTotal})`)
      }
      // Check + spend + RECORD at DISPATCH, before the `await` below — one synchronous step, the
      // same model as agent() (stated at CALL_SPEND). With the spend after the await, a concurrent
      // agent() slipped between a nested dispatch's gate check and its spend, and the wall landed
      // one call late with a message naming the wrong dispatch (measured while writing the gate's
      // own test). With the RECORD after the await, a scripted nested run that threw — the dead
      // nested engine this stub exists to model — was spent but absent from the trail, so budget
      // and trail disagreed by one CALL_SPEND and the argv the record exists to carry died with it;
      // an answered one was recorded at completion, after its concurrent siblings, so the trail
      // order was not the dispatch order the wall arithmetic reads.
      spent += CALL_SPEND
      const call = { label: 'workflow', key: 'workflow', prompt: '', opts: argv[1] ?? {}, argv, result: undefined }
      calls.push(call)
      const answer = Object.prototype.hasOwnProperty.call(script, 'workflow') ? script.workflow : null
      let result
      try {
        result = typeof answer === 'function' ? await answer({ argv }) : answer
      } catch (err) {
        call.threw = String(err?.message ?? err)
        throw err
      }
