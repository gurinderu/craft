...
// shapes. Refused attempts never reach the harness's call log (the stub records only answered
// dispatches) — the registry's own `attempts` is the record of what was tried.
...
test('rust-audit (per-crate fan-out site): both registry shapes reach both crate reviews', async () => {
  for (const [shape, resolvable, expected] of [
    ...
    const scopes = calls.filter(c => c.label === 'workflow').map(c => c.argv[1].path).sort()
