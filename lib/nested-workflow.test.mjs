...
test('EXHAUSTION: both spellings refused throws loudly, naming both and carrying the LAST refusal — never a silent skip', async () => {
  const { attempts, fn } = registry([])
  await assert.rejects(
    () => nestedWorkflow(fn, 'review', {}),
    e => {
      assert.match(e.message, /neither 'craft:review' nor 'review' resolved — the nested run did NOT happen/)
      // The refusal text IS the diagnosis: its `Available:` listing is what located the live
      // failure at a consumer. The LAST refusal is the second attempt's — the bare name.
      assert.match(e.message, /last refusal: workflow\('review'\): no workflow with that name\. Available: deep-research/,
        'the exhaustion error carries the registry\'s own refusal, listing what WAS available')
      return true
    },
  )
...
for (const pin of ['rust-review', 'nix-review']) {
  ...
  test(`${pin}: EXHAUSTION — a registry with neither spelling fails the run loudly`, async () => {
    ...
    await assert.rejects(
      ...
      /neither 'craft:review' nor 'review' resolved.*last refusal:.*Available:/,
      'an unresolvable child must fail the pin loudly and carry the refusal\'s diagnosis, never return something report-shaped',
    )
  })
}
...
// The same no-relaunch pin on the nix-review SITE. The loop above already drives nix-review through
// the qualified/bare/exhaustion cases, but the most expensive mistake available — relaunching a
// nested review that FAILED on its own under the second spelling, running a whole review twice — was
// pinned only on the rust-review and rust-audit sites. nix-review inlines the same `nestedWorkflow`,
// but "the helper is correct" says nothing about the site still launching by one hard-coded spelling
// or growing its own catch: the site is a separate hard-coded call and can regress alone. So the
// property is pinned per carrier, here included.
test('nix-review: a nested run that failed on its own fails the pin — it is not relaunched', async () => {
  const attempts = []
  const fn = async name => { attempts.push(name); throw new Error('the nested review itself failed mid-run') }
  await assert.rejects(
    () => runEngine('nix-review', { args: {}, script: { '*': null, workflow: ({ argv }) => fn(...argv) } }),
    /failed mid-run/,
  )
  assert.deepEqual(attempts, ['craft:review'], 'one launch — a failed review must not be run twice under the other spelling')
})
...
test('rust-audit EXHAUSTION: neither spelling resolving is a NOT-RUN dimension and an INCOMPLETE audit', async () => {
  ...
  // NOT RUN alone is ambiguous: a name that would not resolve and a run that died render the same
  // way. The swallowed throw must reach the log BEFORE the thunk nulls it, refusal text included.
  const refusalLine = run.logs.find(l => /^review failed:/.test(l))
  assert.ok(refusalLine, 'the whole-workspace site logs what it swallowed before nulling it')
  assert.match(refusalLine, /neither 'craft:review' nor 'review' resolved/, 'the log names the resolution refusal')
  assert.match(refusalLine, /Available: deep-research/, "the last refusal's registry listing reaches the operator")
})
...
test('rust-audit: a nested review that failed on its own is NOT relaunched under the other spelling', async () => {
  ...
  // The other half of the ambiguity: a run that DIED must log its own reason — and read as a death,
  // never as a resolution refusal.
  const deathLine = run.logs.find(l => /^review failed:/.test(l))
  assert.ok(deathLine, 'the whole-workspace site logs the swallowed death before nulling it')
  assert.match(deathLine, /the nested review itself failed mid-run/, "the log carries the death's own reason")
  assert.doesNotMatch(deathLine, /neither 'craft:review' nor 'review' resolved/, 'a dead run does not read as a resolution refusal')
})

test('rust-audit (per-crate fan-out site): a swallowed failure is logged per crate — refusal and death distinguishable', async () => {
  // Refusal shape: neither spelling resolves — each crate's line carries the LAST refusal's text.
  {
    const { fn } = registry([])
    ...
    for (const crate of ['a', 'b']) {
      const line = run.logs.find(l => l.startsWith(`review:${crate} failed:`))
      assert.ok(line, `crate ${crate}: the swallowed exhaustion is logged under its own dimension`)
      assert.match(line, /neither 'craft:review' nor 'review' resolved/)
      assert.match(line, /Available: deep-research/, "the last refusal's registry listing reaches the operator")
    }
  }
  // Death shape: the launch resolved and the nested run died — the line carries ITS reason.
  {
    const boom = async () => { throw new Error('the nested review itself failed mid-run') }
    const run = await runEngine('rust-audit', {
      args: {},
      script: { scout: TWO_CRATES, '*': null, workflow: boom },
    })
    for (const crate of ['a', 'b']) {
      const line = run.logs.find(l => l.startsWith(`review:${crate} failed:`))
      assert.ok(line, `crate ${crate}: the swallowed death is logged under its own dimension`)
      assert.match(line, /the nested review itself failed mid-run/)
      assert.doesNotMatch(line, /neither 'craft:review' nor 'review' resolved/, 'a dead run does not read as a resolution refusal')
    }
  }
})
