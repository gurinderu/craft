...
//
// The QUOTING half is handled at the source: `decodeGitPath` normalises the list before anything
// asks whether a file belongs to a profile. Doing it later was not enough and looked like it was —
// a C-quoted name ends in `"`, so `detect` and the shared-suffix test both said no, and the file
// entered NO slice at all; the decoder further downstream never saw it. That also means the
// normalisation fixes the profile selection and the not-reviewed list, not just the pathspec.
//
// What is bounded rather than solved is a model that simply OMITS a line, and the bound is
// structural: the five WHOLE_DIFF_LENSES below keep the profile's glob, so a file absent from the
// list is still read by them. A drop costs the code-intrinsic pass over that file, never the whole
// review — the difference between a thinner review and a silent hole. Accepted on that basis; the
// reconciliation against git is worth doing and is not done here.
//
...
export function decodeGitPath(file) {
