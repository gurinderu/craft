...
test('a single matching journal in the window, with an agreeing branch, is found', () => {
  ...
  // The production filter (`findJournalForRun`) reads only `mtimeMs`, never birthtime — so the
  // fixture pins `runStartMs` as an explicit constant and derives the journal's `mtime` from it with
  // a comfortable positive offset, instead of reading the file's real (platform-dependent) birthtime
  // back after the fact. That real-birthtime approach previously produced `mtime < runStartMs` on
  // Linux (ext4's real creation time lands strictly after the `Date.now()` the test captured before
  // writing the file, while `utimesSync` had already pinned `mtime` to that earlier instant) — hidden
  // on macOS only because APFS pulls a file's birthtime backward to match an earlier `mtime` set via
  // `utimesSync`, which no other platform does. See the file's own measurement above.
  const runStartMs = Date.now()
  const file = plantJournal(claudeHome, project, { birthMs: runStartMs, mtimeMs: runStartMs + 2000, branch: 'feat/x' })
...
test('two candidate journals in the same window resolve to nothing, not to a guess', () => {
  ...
  const project = tmpDir('craft-proj-')
  const runStartMs = Date.now()
  plantJournal(claudeHome, project, { birthMs: runStartMs, mtimeMs: runStartMs + 2000, session: 's1', wf: 'wf1', branch: 'feat/x' })
  // A second journal, mtime within the same tolerance window — real ambiguity.
  plantJournal(claudeHome, project, { birthMs: runStartMs, mtimeMs: runStartMs + 2500, session: 's2', wf: 'wf2', branch: 'feat/x' })
  const hit = findJournalForRun({ project, runStartMs, branch: 'feat/x', claudeHome })
...
test('a unique, well-timed journal for a DIFFERENT branch is refused, not matched on uniqueness alone', () => {
  ...
  const project = tmpDir('craft-proj-')
  const runStartMs = Date.now()
  plantJournal(claudeHome, project, { birthMs: runStartMs, mtimeMs: runStartMs + 2000, branch: 'someone-elses-branch' })
...
test('no branch known on the calling side refuses a unique-in-window candidate rather than matching it', () => {
  ...
  const project = tmpDir('craft-proj-')
  const runStartMs = Date.now()
  plantJournal(claudeHome, project, { birthMs: runStartMs, mtimeMs: runStartMs + 2000, branch: 'feat/x' })
...
test('a candidate journal with no base entry (no branch to read) is refused even with a known caller branch', () => {
  const claudeHome = tmpDir()
  const project = tmpDir('craft-proj-')
  // Same portability fix as the first fixture in this file: pin runStartMs as an explicit constant and
  // derive the planted mtime from it with a positive offset, instead of reading the file's real
  // (platform-dependent) birthtime back — that pattern rejected this exact candidate on Linux, where
  // the real birthtime lands strictly after the pinned mtime, making mtimeMs - runStartMs negative and
  // the window filter reject it for "no-journal-found" instead of the intended 'branch-mismatch'.
  const runStartMs = Date.now()
  plantJournal(claudeHome, project, { birthMs: runStartMs, mtimeMs: runStartMs + 2000 }) // no branch written into the journal
  const hit = findJournalForRun({ project, runStartMs, branch: 'feat/x', claudeHome })
  assert.equal(hit.found, false)
  assert.equal(hit.reason, 'branch-mismatch')
})
...
test('a session id finds its journal under ANY project slug, including one that does not match the given project', () => {
  const claudeHome = tmpDir()
  const project = tmpDir('craft-proj-')
  // Same portability fix as the first fixture: pin runStartMs as an explicit constant and derive the
  // planted mtime from it with a positive offset, rather than reading real (platform-dependent)
  // birthtime back — the real-birthtime read made this the one test CI still reported failing on
  // Linux, where real birthtime lands after the pinned mtime and the active-bound filter rejects the
  // candidate before the session-id match is ever reached.
  const runStartMs = Date.now()
  // Deliberately planted under a slug that has nothing to do with `project` — the worktree case.
  const file = plantJournal(claudeHome, '/some/other/worktree/path', { birthMs: runStartMs, mtimeMs: runStartMs + 2000, session: 'sess-abc', branch: 'feat/x' })
...
test('a session with two workflow runs is ambiguous, and resolves to nothing rather than a guess', () => {
  const claudeHome = tmpDir()
  const now = Date.now()
  // mtime is pinned with a comfortable positive offset from `runStartMs` rather than left equal to
  // it — an exact-equality fixture is one syscall-rounding surprise away from `mtimeMs - runStartMs`
  // landing just under 0 on a platform whose `utimesSync` precision differs from this one's (see the
  // header note on the first fixture in this file for the general hazard).
  plantJournal(claudeHome, '/proj-a', { birthMs: now, mtimeMs: now + 1000, session: 'sess-two', wf: 'wf1', branch: 'feat/x' })
  plantJournal(claudeHome, '/proj-b', { birthMs: now, mtimeMs: now + 1500, session: 'sess-two', wf: 'wf2', branch: 'feat/x' })
