...
export function writeRecord(raw, { store = DEFAULT_STORE, project = process.cwd(), now = new Date() } = {}) {
  ...
  for (let bump = 0; bump < 60; bump++) {
    record = { ...raw, ...computedFields(project, bump ? new Date(now.getTime() + bump * 1000) : now) }
...
// and `head` where the payload carries them. A candidate is adoptable only when its recorded
...
// `branch`/`head` refine it for two reviews of the same repo, and are compared only when both sides
// have them (not every phase payload carries them).
export function checkpointIdentity(o) {
  const s = k => (o && typeof o[k] === 'string' ? o[k] : '')
  return { project: s('project'), branch: s('branch'), head: s('head') }
}
...
export function identityAgrees(want, have) {
  ...
  for (const k of ['branch', 'head']) {
    if (want[k] && have[k] && want[k] !== have[k]) return false
    if (have[k] === null) return false                    // the directory disagrees with itself
  }
  return true
}
...
// fail to attest to. The payload's own `project`, if it carries one, still wins — it came from the
// record.
export function writeCheckpoint(dir, phase, payload, { project = '' } = {}) {
  ...
  fs.writeFileSync(file, JSON.stringify({ phase, at: new Date().toISOString(), ...identity, ...payload }, null, 2))
  return file
}
...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date() } = {}) {
  ...
  const { file } = writeRecord(folded.length && !raw.phases ? { ...raw, phases: folded } : raw, { store, project, now })
...
export function recoverPartials({ store = DEFAULT_STORE, project = process.cwd(), now = new Date(), claudeHome } = {}) {
  ...
  for (const name of fs.readdirSync(root).sort()) {
    ...
    const runProject = dirIdentity(phases).project || project
    const runStartMs = dirStamp(name)
    let journalFindings = null
    let journalFile = ''
    try {
      const jr = findJournalForRun({ project: runProject, runStartMs, ...(claudeHome ? { claudeHome } : {}) })
      if (jr.found) {
    ...
    const base = {
      ...
      partialReason: journalFindings
        ...
        : 'run ended before it could write a final record — reconstructed from phase checkpoints',
      phases,
      ...
      ...(journalFindings ? { ledger: normalizeLedger(journalFindings), ledgerSource: 'journal', journalFile } : {}),
...
// back out of it.
function parseIndividualVerifyTarget(text) {
  const m = /FINDING:\s*\[[^\]]*\]\s*(.+?)\n\s*at\s+(.+?):(\d+)/.exec(text)
...
function parseBatchVerifyTargets(text) {
...
export function findingsFromJournal(dir) {
  ...
  const tally = verifyVerdictsFromJournal(dir)
  if (!tally.size) return findings
  const out = []
  for (const f of findings) {
    const t = tally.get(findingKey(f.file, f.line, f.title))
    if (!t) { out.push(f); continue }
    // Mirrors tierFromVotes' own majority rule in review.js (refutes > half of the panel): a finding
    // the dead run's own verification refuted more than it upheld is disconfirmed candidate
    // evidence, not an open one. This also matches what the LIVE engine persists: toLedgerEntry is
    // only ever called on confirmed/suspected findings (workflows/review.js ~3144) — a refuted
    // finding never reaches a real ledger either, so dropping it here is not a new rule, just the
    // existing one applied to a reconstruction.
    if (t.refuted > t.upheld) continue
    if (t.upheld > 0) {
      out.push({
        ...f,
        tier: 'suspected',
        why: `${f.why || ''} (the dead run's own verification upheld this finding — ${t.upheld} upheld / ${t.refuted} refuted vote(s) recovered from its transcript; not re-verified here, so it is carried as suspected rather than confirmed)`.trim(),
      })
      continue
    }
    out.push(f)
...
if (invokedDirectly) {
  ...
  try {
    if (cmd === 'write') {
      ...
      const file = writeCheckpoint(dir, flag(argv, 'phase') || payload.phase, payload, { project })
      ...
      const res = finalizeRun(raw, { store, project, dir: flag(argv, 'dir') || '', rejoin: argv.includes('--rejoin') })
