...
export function findPriorRound({ store = DEFAULT_STORE, project = process.cwd(), branch = '' } = {}) {
  ...
  const candidates = [...new Set([project, cwd])]
  ...
  for (const cand of rows) {
    ...
    return {
      ...
      head: String(rec.head || cand.head || ''),
      ledger: normalizeLedger(rec.ledger),
...
if (invokedDirectly) {
  ...
  // Resolved to an ABSOLUTE path: a row keyed "." is not attributable to any repository, and the
  // read path (prior-round) deliberately refuses to match those — so a relative --project would file
  // records this branch's own re-review could never find again.
  const project = path.resolve(flag(argv, 'project') || process.cwd())
  const parseStdin = () => {
    const raw = readStdin()
