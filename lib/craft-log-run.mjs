...
// a real id.
export function computedFields(project = process.cwd(), now = new Date(), session = '') {
  return {
...
export function writeRecord(input, { store = DEFAULT_STORE, project = process.cwd(), now = new Date(), session = '', gitId = null } = {}) {
  ...
  for (let bump = 0; bump < 60; bump++) {
    record = { ...raw, ...computedFields(project, bump ? new Date(now.getTime() + bump * 1000) : now, session) }
...
// where the payload carries it (`head` is recorded but does not decide — see `identityAgrees`).
...
// it (not every phase payload carries it). `head` is recorded here for readers but is NOT part of
// the ownership proof — the two writers spell it differently (see `identityAgrees`).
...
// `head` is NOT compared either, and that one was measured rather than reasoned about. The two
// sides of the comparison spell the field differently: `workflows/review.js` writes `head: baseRef`
// (the DIFF BASE — `origin/main`, a ref name) into every checkpoint, while its final record carries
// `head` = the run's actual HEAD sha. Run through these functions with exactly those two payload
// shapes, `finalizeRun` returned `folded: 0, kept: true`, left the `.partial` directory behind and
// filed a record with no `phases` at all — so the fold this proof was meant to guard never happened
// on a single real review; `recover` then promoted the leftover as a separate `partial: true` run.
// A field whose two writers disagree about its MEANING cannot prove ownership: it only ever proves
// which writer filled it. `project` plus `branch` are the discriminators that are spelled the same
// way on both sides, and `head` stays recorded on checkpoints (`checkpointIdentity`/`dirIdentity`
// keep it) for readers — it just no longer decides ownership. Two directories that both pass this
// weaker proof still make `findRejoinableDir` REFUSE rather than guess, which is the guard that
...
export function dirStamp(name) {
  const m = name.match(/^(\d{4}-\d\d-\d\dT\d\d-\d\d-\d\dZ)-/)
  return m ? Date.parse(m[1].replace(/T(\d\d)-(\d\d)-(\d\d)Z$/, 'T$1:$2:$3Z')) : NaN
}
...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date(), session = '', gitId = null } = {}) {
  ...
  const target = (refusedDir ? '' : dir) || (rejoin ? findRejoinableDir({ ...raw, project }, { store, now, project }) : null)
  ...
  const mine = !target || !phases.length ? true
    : !attested.project ? true
      : identityAgrees({ ...checkpointIdentity({ ...raw, project }), project }, attested)
  const folded = mine ? phases : []
  // `gitId` is forwarded, never applied here — `writeRecord` is the single choke point that applies
  // it (see the note there). The comparison above therefore sees the payload as the run described
  // it, which is incidental now rather than required: `identityAgrees` compares `project` and
  // `branch`, and git fills `branch` with the same string the caller would have.
  const { file } = writeRecord(folded.length && !raw.phases ? { ...raw, phases: folded } : raw, { store, project, now, session, gitId })
...
export const PRIOR_ROUND_NONE = { found: false, round: 0, head: '', ledger: [], ledgerCount: 0, priorFindings: 0, journalSourced: false, reason: '' }
...
export function findPriorRound({ store = DEFAULT_STORE, project = process.cwd(), branch = '', workdir = '' } = {}) {
  ...
  if (!git(['rev-parse', '--git-dir'], repoCwd, { probe: true }).ok) return none('git-unavailable')
  ...
  for (const cand of rows) {
    ...
    const mb = git(['merge-base', '--is-ancestor', String(cand.head), 'HEAD'], repoCwd, { probe: true })
    ...
    // still rejected: an older complete round is a far better base than a half-written newer one.
    const journalLedger = rec.partial && rec.ledgerSource === 'journal' && Array.isArray(rec.ledger) && rec.ledger.length
    if (rec.partial && !journalLedger) { rejected ||= 'partial-only'; continue }
...
if (invokedDirectly) {
  ...
  try {
    if (cmd === 'write') {
      const { file } = writeRecord(parseStdin(), { store, project, session, gitId })
      ...
      const res = finalizeRun(raw, { store, project, dir: flag(argv, 'dir') || '', rejoin: argv.includes('--rejoin'), session, gitId })
      ...
      try {
        ...
        out = findPriorRound({ store, project, workdir, branch: gitId.branch || flag(argv, 'branch') || '' })
      } catch {
