...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date() } = {}) {
  const target = dir || (rejoin ? findRejoinableDir({ ...raw, project }, { store, now, project }) : null)
  const phases = target ? readCheckpoints(target) : []
  // An explicit --dir is the caller naming a directory it minted itself, so it needs no proof; a
  // REJOINED one was found by search, and the search's guarantee has to hold at the moment of the
  // destructive act too — the directory can have been written to since.
  const mine = !target || !phases.length ? true
    : dir ? true
      : identityAgrees({ ...checkpointIdentity({ ...raw, project }), project }, dirIdentity(phases))
  ...
  return { file, folded: folded.length, dir: target || '', kept: Boolean(target) && !mine }
}
