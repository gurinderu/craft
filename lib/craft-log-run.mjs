...
//                                         quarantine damaged ones; dry by default (no stdin)
//
...
// failed, pretty-printing it on the way. Words get slipped; this command is the cleanup.
//
...
// write, and the result is verified line for line against the lines that parsed before. Dry by
// default, like backfill-engine — it rewrites the operator's own telemetry, so the destructive form
...
export function repairIndex({ store = DEFAULT_STORE, apply = false, now = new Date() } = {}) {
  ...
  while (i < lines.length) {
    ...
    if (compact === null) quarantined.push({ from: i + 1, to: j, lines: block })
    else {
      out.push(compact)
  ...
  if (quarantined.length) {
    ...
    fs.writeFileSync(report.quarantineFile, quarantined
      .map(q => `# index.jsonl lines ${q.from}-${q.to} — does not join into JSON; moved out by repair-index at ${at}\n${q.lines.join('\n')}`)
      .join('\n') + '\n')
  }
...
if (invokedDirectly) {
  ...
  try {
    if (cmd === 'write') {
      ...
      console.log(`${res.apply ? 'repaired' : 'would repair'} ${res.file}`)
      console.log(`${res.linesBefore} line(s) read: ${res.parsedBefore} already parse, ${res.recovered.length} pretty-printed block(s) recoverable, ${res.quarantined.length} damaged block(s)`)
      for (const r of res.recovered) console.log(`- recover lines ${r.from}-${r.to} → one line: ${r.line.length > 160 ? `${r.line.slice(0, 160)}…` : r.line}`)
      for (const q of res.quarantined) console.log(`- quarantine lines ${q.from}-${q.to} (${q.lines.length} line(s); does not join into one JSON object — NOT reconstructed)`)
