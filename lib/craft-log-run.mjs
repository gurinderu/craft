...
export function writeRecord(raw, { store = DEFAULT_STORE, project = process.cwd(), now = new Date(), session = '' } = {}) {
...
export function identityAgrees(want, have) {
  ...
  for (const k of ['branch', 'head', 'session']) {
...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date(), session = '' } = {}) {
  ...
  const { file } = writeRecord(folded.length && !raw.phases ? { ...raw, phases: folded } : raw, { store, project, now, session })
...
export function recoverPartials({ store = DEFAULT_STORE, project = process.cwd(), now = new Date(), claudeHome } = {}) {
  ...
  for (const name of fs.readdirSync(root).sort()) {
    ...
    // carries none) — `dirIdentity` already turns "two checkpoints disagree" into `null`, so a
    // poisoned value degrades to no session id here rather than a wrong one.
    ...
    try {
      const jr = findJournalForRun({
        project: runProject, runStartMs,
        ...(runSession ? { sessionId: runSession } : {}),
    ...
    const record = { ...base, ...computedFields(project, now, runSession), ts: m ? m[1] : stamp(now) }
    fs.mkdirSync(store, { recursive: true })
...
// Cheap accessor for callers that only need the seen/linked counts (recoverPartials' trust gate),
// without re-deriving them from findingsFromJournal's own call to verifyVerdictsFromJournal.
...
if (invokedDirectly) {
  ...
  try {
    if (cmd === 'write') {
      const { file } = writeRecord(parseStdin(), { store, project, session })
      ...
      const res = finalizeRun(raw, { store, project, dir: flag(argv, 'dir') || '', rejoin: argv.includes('--rejoin'), session })
