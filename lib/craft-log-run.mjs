    ...
    try {
      ...
      for (const sid of sessionCandidates) {
        const jr = findJournalForRun({
          project: runProject, runStartMs, sessionId: sid,
          ...(claudeHome ? { claudeHome } : {}),
        })
        if (jr.found) hits.push(jr)
      }
      // No session id at all (or none of them matched): fall back to the project-slug path, the
      // existing fail-open shape for "session id unusable".
      const jr = hits.length === 1 ? hits[0]
        : hits.length === 0 ? findJournalForRun({ project: runProject, runStartMs, ...(claudeHome ? { claudeHome } : {}) })
          : { found: false, reason: 'ambiguous-journals', count: hits.length }
      if (jr.found) {
