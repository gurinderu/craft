...
export function checkpointDir(record, { store = DEFAULT_STORE, now = new Date(), project = process.cwd(), rejoin = false } = {}) {
  ...
  // `.partial` disappearing under us (a concurrent sweep, a mount going away) is worth ONE recreate,
  // not an unbounded one: `bump -= 1; continue` returned to the same stamp, so while the ENOENT
  // persisted the loop never advanced and never hit the 60 bound — it spun until the harness killed
  // the logger's shell, turning a recoverable miss into a hang.
  let remakes = 0
  for (let bump = 0; bump < 60; bump++) {
