...
export function writeRecord(input, { store = DEFAULT_STORE, project = process.cwd(), now = new Date(), session = '', gitId = null } = {}) {
  ...
  // is on today). It is applied HERE, not upstream in `finalizeRun`, because that function's
  // ownership proof reads `branch`/`head` off the payload to compare against what the run's
  // checkpoints attest: filling them in before that comparison would make a directory whose
  // checkpoints carry the engine's `baseRef` in `head` disagree with its own run and stop being
  // folded in.
...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date(), session = '', gitId = null } = {}) {
  ...
  // `gitId` is forwarded, never applied here: the identity comparison above must see the payload as
  // the run described it (see the note on `writeRecord`'s `gitId`).
...
if (invokedDirectly) {
  ...
  const gitId = gitIdentity(project)
  const parseStdin = () => {
    const raw = readStdin()
  ...
  try {
    if (cmd === 'write') {
      ...
      try {
        ...
        out = findPriorRound({ store, project, branch: gitId.branch || flag(argv, 'branch') || '' })
      } catch {
