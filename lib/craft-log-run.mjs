...
export function partialChainEvidence({ store = DEFAULT_STORE, project = '', branch = '' } = {}) {
  const root = path.join(store, PARTIAL_DIR)
...
export function findPriorRound({ store = DEFAULT_STORE, project = process.cwd(), branch = '', workdir = '' } = {}) {
  ...
  const evidence = fs.existsSync(store)
    ? partialChainEvidence({ store, project: cwd, branch }).find(e => onThisHistory(e.head)) || null
    : null
  ...
  for (const cand of rows) {
    ...
    try {
      ...
      proven ||= provesRound('detail-unreadable') ? { round: cand.round, head: cand.head, findingsTotal: cand.findingsTotal } : null
      continue
    }
    ...
    if (rec.partial && !journalLedger) {
      ...
      proven ||= provesRound('partial-only')
        ? { round: Number(rec.round || cand.round || 0) || 0, head: cand.head, findingsTotal: Number(rec.findings?.total ?? cand.findingsTotal ?? 0) || 0 }
        : null
      continue
    }
