...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date(), session = '', gitId = null } = {}) {
  ...
  const target = (refusedDir ? '' : dir) || (rejoin ? findRejoinableDir(ownId, { store, now, project }) : null)
  ...
  // A directory THIS CALL was handed by `--dir` without `--rejoin` was minted by this run's own
  // first checkpoint: the `--dir` value only becomes untrustworthy once a checkpoint FAILED and the
  // engine armed `--rejoin`, which it then passes here too (that is the hazard the note above
  // describes). So `dir && !rejoin` separates "my own directory" from "a directory I searched for
  // and might be adopting". It matters because the two readings of a branch/head disagreement are
  // opposite: on a searched directory it means "not this run", and on this run's own directory it
  // means the working copy MOVED mid-run — an ordinary thing during a long review, and refusing the
  // fold for it would strand the run's own phases.
  ...
  const startId = mine && attested && (attested.branch || attested.head)
    ? { branch: attested.branch || '', head: attested.head || '' }
    : null
  ...
  const { file } = writeRecord(folded.length && !raw.phases ? { ...raw, phases: folded } : raw, { store, project, now, session, gitId: startId || gitId })
