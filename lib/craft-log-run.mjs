...
// `--dir` REJOINS the newest unfinalized directory of the same kind+name, and only mints a new one
// when there is none. Nothing about the directory's shape changes — `<ts>-<kind>-<name>`, parsed
// back by `recover` exactly as before — so leftovers already in the store stay readable.
//
// The window is what stops the cure from becoming its own disease: without it, every run following
// a dead one would adopt the corpse and finalize ONE record describing two runs. Six hours is well
// past any real review (the longest measured is about three) and well short of "yesterday".
...
export function findRejoinableDir(record, { store = DEFAULT_STORE, now = new Date() } = {}) {
  ...
  const found = fs.readdirSync(root)
    ...
    .filter(n => !fs.existsSync(path.join(store, `${n}.json`)))
    .map(n => ({ n, at: dirStamp(n) }))
    .filter(x => Number.isFinite(x.at) && Math.abs(now.getTime() - x.at) <= REJOIN_WINDOW_MS)
    // Holding no checkpoint at all, it identifies no run — mint a fresh one rather than adopt it.
    .filter(x => readCheckpoints(path.join(root, x.n)).length > 0)
    .sort((a, b) => b.at - a.at)
  return found.length ? path.join(root, found[0].n) : null
}
...
export function checkpointDir(record, { store = DEFAULT_STORE, now = new Date(), project = process.cwd(), rejoin = false } = {}) {
  const found = rejoin ? findRejoinableDir(record, { store, now }) : null
