...
export function checkpointDir(record, { store = DEFAULT_STORE, now = new Date(), project = process.cwd(), rejoin = false } = {}) {
  ...
  for (let bump = 0; bump < 60; bump++) {
    ...
    try {
      ...
      if (e && e.code === 'ENOENT') { fs.mkdirSync(path.join(store, PARTIAL_DIR), { recursive: true }); bump -= 1; continue }
      throw e
    }
  }
...
export function insideStore(dir, store) {
  if (!dir) return false
  const root = path.resolve(store, PARTIAL_DIR)
...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date() } = {}) {
  ...
  const phases = target ? readCheckpoints(target) : []
  ...
  if (target && mine && fs.existsSync(target)) {
  ...
  return { file, folded: folded.length, dir: target || '', kept: Boolean(target) && !mine, refusedDir }
}
...
if (invokedDirectly) {
  ...
  try {
    if (cmd === 'write') {
      ...
      const dir = flag(argv, 'dir') || checkpointDir(payload, { store, project, rejoin: argv.includes('--rejoin') })
      ...
      if (res.kept) console.error(`craft-log-run WARNING: ${res.dir} does not match this run's identity — left for \`recover\``)
      ...
      if (res.refusedDir) console.error(`craft-log-run WARNING: --dir ${res.refusedDir} is not inside ${path.join(store, PARTIAL_DIR)} — record written, directory neither folded nor removed`)
    } else if (cmd === 'recover') {
