...
// Atomic replace: write a temp file in the SAME directory as the target, then rename it over the
// target, so a crash mid-write cannot leave a truncated record. LEAVE NOTHING BEHIND: if either the
// write or the rename throws (a full disk, an unwritable directory, a target that is a directory), the
// temp is removed before the error propagates, so a failed enrich leaves the directory exactly as it
// found it — the atomic "all or nothing" this path exists for, extended to its own failure case
// (before, a throw between the write and the rename stranded a `.cost-*.tmp` in the store).
