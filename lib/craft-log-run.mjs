...
// round chain, not two. Not a git repo (or no git at all) → the resolved path, which is still better
// than a relative one; `--git-common-dir` failing on an otherwise-valid repo (very old git without
// `--path-format`) falls back to `--show-toplevel` rather than losing the key entirely.
export function repoKey(p = process.cwd()) {
  const abs = path.resolve(p)
  const commonDir = git(['rev-parse', '--path-format=absolute', '--git-common-dir'], abs)
  if (commonDir) return path.resolve(path.dirname(commonDir))
