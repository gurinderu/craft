...
export function writeRecord(raw, { store = DEFAULT_STORE, project = process.cwd(), now = new Date() } = {}) {
  ...
  let record = { ...raw, ...computedFields(project, now) }
  let file = path.join(store, recordFilename(record))
  for (let bump = 0; fs.existsSync(file) && bump < 60; bump++) {
    record = { ...raw, ...computedFields(project, new Date(now.getTime() + (bump + 1) * 1000)) }
