...
export function finalizeRun(raw, { store = DEFAULT_STORE, project = process.cwd(), dir = '', rejoin = false, now = new Date(), session = '', gitId = null, workdir = '' } = {}) {
  ...
  // all; review.js finalizes with `{ dir: runDir, rejoin: checkpointFailed }`, so on every run that
  ...
  // lib/run-logging.mjs); `ownDir` is now false exactly when the engine ever armed a rejoin, which is
  // exactly when its `runDir` may have been adopted.
  //
  ...
  //     STALE leftover of the same repo and branch at an older commit. Unclosed, deliberately.
  //
