...
import { indexProjection, summarizeFindings, reviewVerdict, selectPriorRound } from './run-record.mjs'
...
export function recordFromJournal(dir, { name = 'review', kind = 'workflow' } = {}) {
  ...
  return {
    ...
    scout: scout ? [{ size: scout.sizeBucket, lenses: scout.lenses, maxRounds: scout.maxRounds, verifyVotes: scout.verifyVotes }] : [],
...
// That is a chain of DECISIONS, and it silently returned "no prior round" on the two conditions the
// real store actually has — index rows written with project="." and blocks of pretty-printed JSON
// inside the JSONL. Both are handled here, deterministically; the model only carries the bytes.
export const PRIOR_ROUND_NONE = { found: false, round: 0, head: '', ledger: [], priorFindings: 0 }

export function findPriorRound({ store = DEFAULT_STORE, project = process.cwd(), branch = '' } = {}) {
  if (!branch) return PRIOR_ROUND_NONE
  const entries = readJsonl(path.join(store, 'index.jsonl'))   // malformed lines are skipped, not thrown
  // Rows exist under both the resolved absolute path and whatever string the caller passed (older
  // runs recorded project="."). Ask the pure selector once per candidate spelling and keep the newest.
  const candidates = new Set([project, path.resolve(project), '.'])
  let best = null
  for (const p of candidates) {
    const hit = selectPriorRound(entries, { project: p, branch })
    if (hit && (!best || String(hit.ts) > String(best.ts))) best = hit
  }
  if (!best || !best.head) return PRIOR_ROUND_NONE
  // A rebase/force-push leaves a run whose head is no longer on this history: not a prior round.
  const cwd = path.resolve(project)
  const mb = git(['merge-base', '--is-ancestor', String(best.head), 'HEAD'], cwd, { probe: true })
  if (!mb.ok) return PRIOR_ROUND_NONE
  const file = path.join(store, recordFilename({ ts: best.ts, kind: best.kind, name: best.name }))
  let rec
  try {
    rec = JSON.parse(fs.readFileSync(file, 'utf8'))
  } catch {
    return PRIOR_ROUND_NONE
  }
  return {
    found: true,
    round: Number(rec.round || best.round || 0) || 0,
    head: String(rec.head || best.head || ''),
    ledger: Array.isArray(rec.ledger) ? rec.ledger : [],
    priorFindings: Number(rec.findings?.total ?? best.findingsTotal ?? 0) || 0,
  }
}
...
if (invokedDirectly) {
  ...
  const project = flag(argv, 'project') || process.cwd()
  const parseStdin = () => {
    const raw = readStdin()
