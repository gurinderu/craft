...
// `.message.usage`. Tolerant by construction — a `.jsonl` transcript is a stream a run may have been
// killed mid-line, and non-assistant records (user turns, tool results) carry no usage — so a blank,
// malformed or usage-less line is skipped, never allowed to abort the sum.
...
// land simply re-sums. `total` is the four token sums; `agents` is the transcript count. Throws with
// a clear message when the run directory is missing, holds no transcripts, or the record is
// missing/unparseable — the CLI turns any throw into a non-zero exit.
export function enrichCost({ runDir, recordFile } = {}) {
  ...
  const cost = { output: 0, input: 0, cacheRead: 0, cacheWrite: 0, total: 0, agents: transcripts.length }
  for (const name of transcripts) {
    const u = sumTranscriptUsage(fs.readFileSync(path.join(runDir, name), 'utf8'))
  ...
  // Atomic: temp file in the SAME directory + rename, so a crash mid-write cannot leave a truncated record.
  const tmp = path.join(path.dirname(recordFile), `.${path.basename(recordFile)}.cost-${process.pid}-${Date.now()}.tmp`)
...
An optional \`cost{output,input,cacheRead,cacheWrite,total,agents}\` is folded in AFTER the run by
...
export function partialChainEvidence({ store = DEFAULT_STORE, project = '', branch = '', session = '' } = {}) {
  ...
  for (const name of names) {
    ...
    // invalidates the whole load. `ledgerTotal` is the count the dead run DECLARED, kept separate
    // from the array so a shortfall stays visible — see the note in reconcileChain.
    ...
    out.push({
      ...
      ledger: normalizeLedger(shards.ledger),
    ...
    if (rec.partial && !journalLedger) {
      ...
      proven ||= provesRound('partial-only')
        ? {
          ...
          ledger: normalizeLedger(shardsInRecord.ledger),
...
if (invokedDirectly) {
  ...
  try {
    if (cmd === 'write') {
      ...
      console.log(`enriched ${recordFile} — ${cost.agents} agent(s): cacheRead ${cost.cacheRead}, cacheWrite ${cost.cacheWrite}, input ${cost.input}, output ${cost.output}, total ${cost.total} tok (real cost; outputTokens is the harness pool, ~<1%)`)
    } else {
