...
// THE PREMISE OF THE SAVING IS A CLAIM ABOUT A MOMENT, NOT AN INVARIANT.
//
// The floor is raised during Verify, by a Critical that came back confirmed. The verdict is decided
// afterwards — and on a re-review `absorbAcross` folds a confirmed finding into a matching dismissed
// prior, which `rereviewVerdict` deliberately excludes. So the evidence the saving rested on can
// leave the verdict while the findings it saved stay unverified, and one of them could have been the
// difference between Approve and Warning. Nothing in the earlier assertions would notice: the tier
// is right, the denominator is right, the list is right. Only the outcome is wrong.
test('review: a saving whose premise did not survive to the verdict is reported as a coverage hole', async () => {
  const RULE = 'RUST-001'
  const ruled = f => ({ ...f, ruleId: RULE })
  const criticals = CRITICALS.map(ruled)
  const script = {
    ...scriptFor(() => CONFIRMS),
    scout: { sizeBucket: 'small', lenses: ['safety'], isLibrary: false, securitySensitive: false, intent: '', churn: [], notes: 'x' },
    lens: ({ opts, callIndex }) => ({
      lens: String(opts.label || '').split(':')[2]?.split(' ')[0] || 'safety',
      findings: callIndex === 0 ? criticals.concat(MEDIUMS) : [],
    }),
    // Every confirmed Critical matches a prior the author already DISMISSED, so all of them absorb
    // into `carried` and none reaches the verdict.
    'prior-round': {
      found: true, round: 1, head: 'abc0000', ledgerCount: criticals.length, priorFindings: criticals.length, reason: '',
      ledger: criticals.map((f, i) => ({
        stable_id: `p${i}`, severity: 'Critical', title: f.title, file: f.file, line: f.line,
        symbol: '', ruleId: RULE, why: 'w', fix: 'f', disposition: 'rejected', round: 1,
      })),
    },
  }
  const { calls, report, logs } = await runEngine('review', { args: {}, script })
  const rec = filedRecord({ calls })

  // The saving really was made — otherwise this test proves nothing about it.
  assert.equal(batchDispatches(calls), 0, 'the batches must still have been skipped on the Block premise')

  // And the premise did not hold: the verdict is not Block.
  assert.ok(!/Block/.test(rec.verdict), `the fixture must end somewhere other than Block: ${rec.verdict}`)

  // THEREFORE the skip stops being a saving. It converts into the one thing a coverage hole is —
  // a notRun entry a re-run can fix — and the run says INCOMPLETE rather than reporting a clean
  // verdict over findings nobody checked for a reason that turned out not to apply.
  assert.equal(rec.savedByFloorPremiseHeld, false, 'the record must say the premise was revoked')
  assert.ok(
    rec.notRun.some(n => /PREMISE DID NOT HOLD/.test(n)),
    `the skipped verifications must convert into notRun: ${JSON.stringify(rec.notRun)}`,
  )
  assert.match(rec.verdict, /INCOMPLETE/, 'and the recorded verdict must say the run was incomplete')
  assert.match(report, /premise did not hold/i, 'and the report must tell the reader, not only the record')

  // The fallback report renders the verdict line from the LIVE notRun, so it already says so here.
  // That is the mechanical path; the model's path is asserted by its own test below, because these
  // two reach the same line by different routes and a fixture that dies synthesis proves only one.
  // THE VERDICT LINE ITSELF. A section at the foot of the document does not take back a green line
  // at the top: a reader stops at the verdict, and rust-audit's roll-up reads that STRING — so a
  // clean line there carries the wrong-green up into the audit while the run record says INCOMPLETE.
  // Record and report disagreeing is the defect; this is the assertion that sees it.
  const verdictLine = report.split('\n').find((l, i, all) => {
    const head = all.findIndex(x => /^#+\s*Verdict\b/i.test(x.trim()))
    return head >= 0 && i > head && l.trim()
  })
  assert.match(String(verdictLine), /INCOMPLETE/, `the report's own verdict line must say so: ${verdictLine}`)
  assert.ok(logs.some(l => /premise/i.test(l) || /coverage hole/.test(l)), 'and the transcript must carry it too')
})

// The SYNTHESIZED path, which the test above cannot reach: its fixture kills synthesis on purpose,
// so it exercises the mechanical fallback. The model's report is composed from a prompt built
// BEFORE the verdict exists, so the model cannot know the premise was revoked — it writes a clean
// verdict line in good faith. If nothing amends that line afterwards, the report says Approve while
// the run record of the same run says INCOMPLETE, and rust-audit's roll-up reads the report string.
test('review: a synthesized report does not keep a clean verdict line over a revoked premise', async () => {
  const RULE = 'RUST-001'
  const criticals = CRITICALS.map(f => ({ ...f, ruleId: RULE }))
  const { calls, report } = await runEngine('review', {
    ...
    script: {
      ...
      // A live synthesis writing exactly what the prompt let it believe: a clean approval.
      synthesis: '## Verdict\n✅ Approve — the fixes closed everything that was open.\n\n## Gate\nCI',
    },
  })
  const rec = filedRecord({ calls })
  assert.equal(rec.savedByFloorPremiseHeld, false, 'the fixture must actually revoke the premise')

  const lines = report.split('\n')
  const head = lines.findIndex(l => /^#+\s*Verdict\b/i.test(l.trim()))
  const verdictLine = lines.find((l, i) => i > head && l.trim())
  assert.match(String(verdictLine), /INCOMPLETE/, `the model wrote a clean line and nothing corrected it: ${verdictLine}`)
  // And the correction is not silently duplicated on a line that already carries it.
  assert.equal(String(verdictLine).match(/INCOMPLETE/g).length, 1, 'the clause must be appended once')
})
