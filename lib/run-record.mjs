...
// NOTE FOR THE INLINE: worstVerdict is inlined into workflows/rust-audit.js, which already inlines
...
export function selectPriorRounds(indexEntries, { project, branch }) {
  ...
  for (const e of (Array.isArray(indexEntries) ? indexEntries : [])) {
    ...
    // The reader half of this decision is in workflows/review.js: a child does not read the chain.
...
//       (the recidivism/tombstone check) is only sound within one revision. That is exactly the
//       "changes what its telemetry means" case this field exists to mark, so it earns the bump.
//
...
export function engineRevisionToken(r) {
  return r && Number.isInteger(r.engineRevision) ? `r${r.engineRevision}` : 'r?'
}
