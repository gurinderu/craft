...
//       compute dropped/candidates and are untouched; `refuteRate()` below is likewise unchanged.)
//
// WHY 2 AND NOT 3, WRITTEN DOWN BECAUSE IT LOOKED LIKE A MISSED BUMP: all of the above reached trunk
// in a single merge batch, so no run was ever recorded under a narrower revision 2 — every record in
// existence predates the field entirely and reads `r?`. Bumping would have created a bucket nothing
// can ever be in. What was needed was not a new number but an honest description of the one that
// shipped. The lesson for the next reader: before bumping, ask whether any record actually carries
// the current value; parallel branches merged together are ONE engine, however the log lists them.
